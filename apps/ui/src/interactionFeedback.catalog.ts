...
export const FEEDBACK_CATALOG: Record<string, FeedbackRow> = {
  ...
  'src/components/settings/DawCatalogPanel.tsx::dawCatalogList#1': row('mount', 'instant', 'na', 'na', 'ui', 'inline', 'na', 'ok', 'The DAW catalog and its on-demand detection state (PRD daw-selection-and-acquisition.prd.md Phase 2). A failed load shows an inline error, like the other mount-time loads in Settings; the panel re-runs detection whenever it remounts (opening or returning to the DAW Integration category).'),
  'src/components/settings/DawCatalogPanel.tsx::dawCatalogOpenDownloadPage#1': row('click', 'os-dialog', 'pending', 'pending', 'ui', 'toast', 'na', 'ok', 'Opens the vendor\'s official download page in the default browser (never an installer); the button reads "Opening…" and every entry\'s button is disabled while it runs (usePendingAction), so a second click cannot open a second tab. Nothing here changes after success except the browser tab - detection itself only updates on the Phase 3 "Check again" action, not yet built.'),
