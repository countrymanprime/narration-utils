...
export const FEEDBACK_CATALOG: Record<string, FeedbackRow> = {
  ...
  'src/components/stages/useStageRecommendations.ts::stageRecommendations#1': row('mount', 'file-io', 'na', 'na', 'ui', 'inline', 'na', 'ok', 'Reads every chapter\'s suggestion when Home opens and again after an import, a finished check, a status changed by hand and Check now (Q5); each row says Checking… until the first read answers and Check now is busy meanwhile; a failure reads "Couldn\'t check" in every row, with the reason above the table and a chip on the card.'),
