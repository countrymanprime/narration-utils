...
describe('DeliveryPage', () => {
  it('names the profile the project is judged against, with every rule and its source, before anything is measured', async () => {
    ...
    expect(screen.getByText('2 not checked by the app')).toBeTruthy();
    ...
    expect(within(rules).getByText('Measured: 192 kbps+ CBR')).toBeTruthy();
  ...
  it('opens a file against the profile, rule by rule, with the requirement and its verification', async () => {
    ...
    expect(format?.textContent).toContain('This is a WAV render; measure the MP3 you upload to check its format.');
