...
/** What made a sync run: the consent itself, a DAW link (or a different .rpp chosen), an import, a project attach, or an Undo. */
export type ChapterSyncTrigger = 'consent' | 'daw-link' | 'import' | 'attach' | 'undo';
 ...
 * something or found a new unmatched track.
 */
...
export interface ChapterSyncApi {
  ...
  /** `chaptersync:state`: sent after each link path (a DAW link, an import, an attach) and each sync or Undo. */
