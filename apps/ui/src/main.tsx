...
// `?mockUpdate=available|found|downloading|download-fails|failed|current|development` boots the mock host in that update state ("Version 0.2.7 is available", "could not reach
...
const mockUpdate = (['available', 'found', 'downloading', 'download-fails', 'failed', 'current', 'development'] as const).find(
  (seed) => seed === mockParams.get('mockUpdate'),
);
const mockInitial = {
