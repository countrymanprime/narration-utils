...
// windowsDevices are names Windows treats as devices whatever their extension.
...
func extractArchive(staging string, file File) ([]ExtractedFile, error) {
	...
	for _, entry := range reader.File {
		...
		if declared > uint64(limit) { //nolint:gosec // G115: limit is a catalog size, never negative
