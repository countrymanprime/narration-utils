...
####
## The Narration Utils installer (docs/adr/0082). `wails build -nsis` compiles it with makensis. Wails writes wails_tools.nsh again on
## every build (it is generated and not checked in) and uses this file as it finds it, so only what must differ from the Wails default
## is here:
##
...
##
## Wails passes -DARG_WAILS_AMD64_BINARY=<the built program>. To try the installer without Wails, run makensis in this folder with
## that define after one `wails build -nsis` has written wails_tools.nsh.
####

# Defaults for wails_tools.nsh. A value given with makensis -D (Wails passes -DWAILS_INSTALL_SCOPE=user and
# -DREQUEST_EXECUTION_LEVEL=user for `-installscope user`) is kept, and is never defined twice.
