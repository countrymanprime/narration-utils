...
- The file protocol between the app and REAPER (the session folder under REAPER's resource path, the command files the Lua bridge reads, and the paths it opens from a command), the arguments a DAW starts the app with (`--daw REAPER` or `--daw Audacity`, `--project-folder`, `--session-dir`), and the local `/media` route that plays a project's audio.
