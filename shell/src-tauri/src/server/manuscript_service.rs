...
type EntityIndex = HashMap<EntityKey, Vec<String>>;
...
static MANUSCRIPT_CACHE: LazyLock<Mutex<HashMap<PathBuf, (Stamp, Value)>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static NOTES_CACHE: LazyLock<Mutex<HashMap<PathBuf, (Stamp, Value)>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
static ENTITY_INDEX_CACHE: LazyLock<Mutex<HashMap<PathBuf, (Stamp, EntityIndex)>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));
...
impl ManuscriptService {
    ...
    pub fn chapters(&self) -> Result<Value, String> {
        ...
        Ok(Value::Array(
            data["chapters"]
                ...
                .unwrap_or_default()
                .iter()
                .map(|chapter| chapter_payload(chapter, &status, Some(&paragraphs)))
                .collect(),
        ))
    }
    pub fn paragraphs(&self, chapter_id: &str) -> Result<Value, String> {
        ...
        Ok(Value::Array(
            data["paragraphs"]
                ...
                .unwrap_or_default()
                .iter()
                .filter(|paragraph| paragraph["chapterId"].as_str() == Some(chapter_id))
                .map(|paragraph| paragraph_payload(paragraph, &entities))
                .collect(),
        ))
    }
    pub fn reader(&self) -> Result<Value, String> {
        ...
        let status = notes
            ...
            .unwrap_or_default();
        let entities = self.entity_occurrence_index();
        let chapters = Value::Array(
            data["chapters"]
                .as_array()
                .cloned()
                .unwrap_or_default()
                .iter()
                .map(|chapter| chapter_payload(chapter, &status, None))
                .collect(),
        );
        let paragraphs = Value::Array(
            data["paragraphs"]
                .as_array()
                .cloned()
                .unwrap_or_default()
    ...
    pub fn set_chapter_status(&self, chapter_id: &str, status: &str) -> Result<Value, String> {
        if !matches!(
            status,
            "not_started" | "recording" | "editing" | "proofing" | "finalized"
        ) {
            return Err(format!("Unknown chapter status: {status}"));
        }
        let data = self.load()?;
        let chapter = data["chapters"]
            .as_array()
            .and_then(|chapters| {
                chapters
                    .iter()
            ...
            .ok_or_else(|| "Unknown manuscript chapter.".to_string())?;
        let mut notes = self.load_notes();
        let object = notes.as_object_mut().expect("notes object");
        object
            .entry("chapterStatus")
            .or_insert_with(|| json!({}))
            .as_object_mut()
            .expect("status object")
            .insert(chapter_id.to_string(), json!(status));
        let chapter_status = notes["chapterStatus"]
            .as_object()
            .cloned()
            .unwrap_or_default();
    ...
    pub fn note_list(&self, chapter_id: Option<&str>) -> Value {
        let notes = self.load_notes();
        Value::Array(
            notes["notes"]
                .as_array()
                ...
                .unwrap_or_default()
                .into_iter()
                .filter(|note| chapter_id.is_none_or(|id| note["chapterId"].as_str() == Some(id)))
                .collect(),
        )
    }
    pub fn note_create(
        ...
        if anchor_start.is_some_and(|value| value < 0)
            || anchor_end.is_some_and(|end| anchor_start.is_none_or(|start| end < start))
        {
            return Err("Invalid note anchor.".to_string());
        }
        let data = self.load()?;
        let paragraph = data["paragraphs"]
            .as_array()
            .and_then(|items| {
                items.iter().find(|item| {
                    item["id"].as_str() == Some(paragraph_id)
                        && item["chapterId"].as_str() == Some(chapter_id)
                })
            })
            .ok_or_else(|| "Unknown manuscript paragraph.".to_string())?;
    ...
    pub fn note_delete(&self, note_id: &str) -> Result<(), String> {
        let mut notes = self.load_notes();
        notes["notes"]
            .as_array_mut()
            .expect("notes array")
            .retain(|note| note["id"].as_str() != Some(note_id));
        self.save_notes(&notes)
    }

    pub fn reader_state(&self) -> Value {
        self.load_notes()["readerState"].clone()
    }
    ...
    #[allow(clippy::too_many_arguments)] // Matches the established HTTP payload fields one-for-one.
    pub fn create_bookmark(
        ...
        if !matches!(kind, "chapter" | "line" | "note") {
            return Err("Unknown bookmark kind.".to_string());
        }
        let mut notes = self.load_notes();
        let bookmarks = notes["readerState"]["bookmarks"]
            .as_array()
            .cloned()
            .unwrap_or_default();
        ...
        let mut updated = bookmarks;
        updated.push(bookmark.clone());
    ...
    pub fn delete_bookmark(&self, bookmark_id: &str) -> Result<(), String> {
        ...
        let bookmarks = notes["readerState"]["bookmarks"]
            ...
            .unwrap_or_default()
            .into_iter()
            .filter(|b| b["id"].as_str() != Some(bookmark_id))
            .collect();
    ...
    fn guide_path(&self) -> Option<PathBuf> {
        self.project
            .as_ref()
            .map(|project| project.join("ManuscriptGuide/manuscript_guide.json"))
    }
    fn entity_occurrence_index(&self) -> HashMap<(String, String), Vec<String>> {
        ...
        if let Ok(mut cache) = ENTITY_INDEX_CACHE.lock() {
            cache.insert(path, (current, index.clone()));
        }
        index
    }
    fn notes_path(&self) -> Result<PathBuf, String> {
        Ok(self
            .project
            .as_ref()
            .ok_or_else(|| "Save the REAPER project first.".to_string())?
            .join("narration-utils/manuscript-notes.json"))
    }
    fn load_notes(&self) -> Value {
        let Ok(path) = self.notes_path() else {
            return empty_notes();
        };
        let Some(current) = stamp(&path) else {
            return empty_notes();
        ...
        if let Ok(mut cache) = NOTES_CACHE.lock() {
            cache.insert(path, (current, notes.clone()));
        }
        notes
    }
    fn save_notes(&self, notes: &Value) -> Result<(), String> {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .map_err(|error| format!("Could not create manuscript sidecar: {error}"))?;
        }
        fs::write(
            &path,
            serde_json::to_vec_pretty(notes).expect("notes JSON serializes"),
        )
        .map_err(|error| format!("Could not write manuscript sidecar: {error}"))?;
        if let Ok(mut cache) = NOTES_CACHE.lock() {
            cache.remove(&path);
        }
        Ok(())
...
fn empty_notes() -> Value {
    json!({"notes": [], "chapterStatus": {}, "readerState": {"activeChapter": null, "activeSourceLine": null, "bookmarks": [], "expandedChapters": null}})
}
fn normalize_notes(raw: Value) -> Value {
    let object = raw.as_object().cloned().unwrap_or_default();
    let lower = object
        .into_iter()
        .map(|(key, value)| (key.to_lowercase(), value))
        .collect::<BTreeMap<_, _>>();
    json!({"notes": lower.get("notes").cloned().unwrap_or_else(||json!([])), "chapterStatus": lower.get("chapterstatus").cloned().unwrap_or_else(||json!({})), "readerState": lower.get("readerstate").cloned().unwrap_or_else(|| empty_notes()["readerState"].clone())})
}
fn paragraph_payload(
    paragraph: &Value,
    entities: &HashMap<(String, String), Vec<String>>,
) -> Value {
    let key = (
        paragraph["chapterId"]
            .as_str()
            .unwrap_or_default()
            .to_string(),
        paragraph["id"].as_str().unwrap_or_default().to_string(),
    );
    ...
    if let Some(paragraphs) = paragraphs {
        value["paragraphIds"] = Value::Array(
            paragraphs
                ...
                .map(|paragraph| json!({"id":paragraph["id"],"index":paragraph["index"]}))
                .collect(),
        );
    }
    value
...
fn build_entity_occurrence_index(
    guide_path: &std::path::Path,
) -> HashMap<(String, String), Vec<String>> {
    let mut index: HashMap<(String, String), Vec<String>> = HashMap::new();
    let Ok(text) = fs::read_to_string(guide_path) else {
        return index;
    };
    ...
    let mut add_occurrences = |entity_id: &str, occurrences: &Value| {
        for occurrence in occurrences.as_array().into_iter().flatten() {
            let (Some(chapter), Some(paragraph)) = (
                occurrence["chapterId"].as_str(),
                occurrence["paragraphId"].as_str(),
            ) else {
                continue;
            };
            let ids = index
                .entry((chapter.to_string(), paragraph.to_string()))
                .or_default();
    ...
    for entity in guide["entities"].as_array().into_iter().flatten() {
        ...
        for alias in entity["aliases"].as_array().into_iter().flatten() {
            if alias.is_object() {
                add_occurrences(entity_id, &alias["occurrences"]);
            }
        }
    }
    index
...
mod tests {
    ...
    fn project(label: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("narration-utils-{label}-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        dir
    }

    fn seed_manuscript(project: &std::path::Path) -> (String, String) {
    ...
    fn reader_state_round_trips_and_an_explicit_empty_expanded_list_is_not_treated_as_unset() {
        ...
        let saved = service
            .save_reader_state(
                Some("c-0001".to_string()),
                Some(3),
                Some(vec!["c-0001".to_string()]),
            )
        ...
        let unchanged = service
            .save_reader_state(Some("c-0002".to_string()), None, None)
            .expect("save succeeds");
        assert_eq!(unchanged["expandedChapters"], serde_json::json!(["c-0001"]));

        let cleared = service
            .save_reader_state(None, None, Some(vec![]))
            .expect("save succeeds");
    ...
    fn duplicate_bookmarks_are_deduped_and_delete_removes_by_id() {
        ...
        assert_eq!(
            ...
            "identical bookmark should be deduped, not re-created"
        );
