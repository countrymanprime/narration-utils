...
impl Default for TranscriptRun {
    fn default() -> Self {
        Self {
            ...
            diff: String::new(),
            summary: String::new(),
            marker_export_phase: "idle".into(),
            marker_export_message: String::new(),
            marker_export_added: 0,
            marker_export_skipped: 0,
            started: None,
            manifest: None,
            output: None,
            progress: None,
            ...
            log_offset: 0,
            pending_options: std::collections::BTreeMap::new(),
        }
    }
}
...
impl AppState {
    fn compare(&self) -> (String, String) {
        (
            self.config.compare_python.clone(),
            self.config.compare_backend.clone(),
        )
    }
    ...
    pub async fn transcript_start(
        ...
        let project = {
            ...
            let project = self
                ...
                .project_folder
                .clone()
                .filter(|project| super::manuscript_canonical::exists(project))
                .ok_or_else(|| {
                    ApiError::bad_request("Save the REAPER project and import a manuscript first.")
                })?;
            let run_id = format!(
                "{}000",
            ...
            if let Some(parent) = hints_path.parent() {
                std::fs::create_dir_all(parent).map_err(|error| {
                    ApiError::bad_request(format!(
                        "Could not prepare Transcript Compare storage: {error}"
                    ))
                })?;
            }
            std::fs::write(
                &hints_path,
                options.get("hints").map(|value| value.trim()).unwrap_or(""),
            )
            .map_err(|error| {
                ApiError::bad_request(format!("Could not write vocabulary hints: {error}"))
            })?;
            *run = TranscriptRun {
                phase: "preparing".into(),
                run_id: run_id.clone(),
                message: "Preparing the selected REAPER audio…".into(),
    ...
    pub async fn transcript_cancel(&self) {
        let progress = {
            let mut run = self.transcript.lock().await;
            if matches!(run.phase.as_str(), "preparing" | "need_chapter") {
    ...
    pub async fn transcript_reset(&self) -> Result<(), ApiError> {
        {
            ...
            if matches!(run.phase.as_str(), "preparing" | "running") {
                return Err(ApiError::bad_request(
                    "Cancel the active comparison before starting a new one.",
                ));
            }
    ...
    pub async fn transcript_jump(&self, row_id: &str) -> Result<(), ApiError> {
        let (run_id, ok) = {
            ...
            (
                run.run_id.clone(),
                run.rows.iter().any(|row| row["id"] == row_id),
            )
        };
        if !ok {
            return Err(ApiError::bad_request(
                "That discrepancy is no longer available.",
            ));
        }
        let mut bridge = self.bridge.lock().await;
    ...
    pub async fn transcript_export_markers(&self) -> Result<(), ApiError> {
        let (run_id, output, pending) = {
            ...
            if run.phase != "success" || run.run_id.is_empty() || run.output.is_none() {
                return Err(ApiError::bad_request(
                    "Run a comparison in this session before exporting its markers.",
                ));
            }
            if run.marker_export_phase == "exporting" {
                return Err(ApiError::bad_request(
                    "Marker export is already in progress.",
                ));
            }
            let pending = run
                .rows
                .iter()
                .filter(|row| row["markerState"] == "pending")
                .count();
            if pending == 0 {
                return Err(ApiError::bad_request(
                    "There are no new markers ready to export.",
                ));
            }
        ...
        let misread = self
            .settings
            .effective("TranscriptCompare", "color_misread", "FF4040")
            .0;
        let skipped = self
            .settings
            .effective("TranscriptCompare", "color_skipped", "FFC000")
            .0;
        let extra = self
            .settings
            .effective("TranscriptCompare", "color_extra", "40A0FF")
            .0;
        {
            let mut run = self.transcript.lock().await;
            run.marker_export_phase = "exporting".into();
            run.marker_export_message = format!(
                "Exporting {pending} marker{} to REAPER…",
                if pending == 1 { "" } else { "s" }
            );
            ...
            let _ = bridge.send(
                ...
                &[&run_id, &output, &misread, &skipped, &extra],
            );
        }
        self.changed();
        Ok(())
    ...
    pub async fn transcript_add_equivalence(&self, row_id: &str) -> Result<String, ApiError> {
        let project = self.config.project_folder.clone().ok_or_else(|| {
            ApiError::bad_request("Select a single-word MISREAD result to add an equivalence.")
        })?;
        let run = self.transcript.lock().await;
        ...
        if invalid {
            return Err(ApiError::bad_request(
                "Select a single-word MISREAD result to add an equivalence.",
            ));
        }
        ...
        let mut file = std::fs::OpenOptions::new()
            ...
            .open(&path)
            .map_err(|error| {
                ApiError::bad_request(format!("Could not update equivalences file: {error}"))
            })?;
        writeln!(file, "{doc_text}, {audio_text}").map_err(|error| {
            ApiError::bad_request(format!("Could not update equivalences file: {error}"))
        })?;
    ...
    pub async fn transcript_suggest_hints(&self) -> Result<String, ApiError> {
        ...
        let accepted_lower: std::collections::HashSet<String> =
            accepted.iter().map(|value| value.to_lowercase()).collect();
        let candidates = self
            .guide()
        ...
        Ok(candidates
            .into_iter()
            .filter(|candidate| !accepted_lower.contains(&candidate.to_lowercase()))
            .collect::<Vec<_>>()
            .join(", "))
    }
    ...
    fn vocab_hints_path(&self) -> Option<std::path::PathBuf> {
        self.config
            .project_folder
            .as_ref()
            .map(|project| project.join("TranscriptCompare/vocab_hints.json"))
    }

    pub fn transcript_hints(&self) -> Vec<String> {
        let Some(path) = self.vocab_hints_path() else {
            return Vec::new();
        };
    ...
    pub fn transcript_save_hints(&self, accepted: Vec<String>) -> Result<(), ApiError> {
        let path = self
            .vocab_hints_path()
            .ok_or_else(|| ApiError::bad_request("Select a manuscript first."))?;
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|error| ApiError::bad_request(format!("Could not save hints: {error}")))?;
        }
    ...
    fn persist_last_comparison(&self, run: &TranscriptRun) {
        ...
        if let Ok(mut cache) = self.last_completed_json.try_lock() {
            *cache = Some(text);
        }
    }
    ...
    pub async fn handle_bridge_event(&self, fields: Vec<String>) {
        ...
        match (tag.as_str(), rest.len()) {
            ("COMPARE_PREPARED", n) if n >= 5 => {
                let (run_id, manifest, manuscript, track, diff_path) = (
                    &rest[0],
                    &rest[1],
                    rest[2].clone(),
                    rest[3].clone(),
                ...
                let should_launch = {
                    let mut run = self.transcript.lock().await;
                    if run_id != &run.run_id || run.phase != "preparing" {
                        false
                    } else {
                ...
                if should_launch {
                    self.launch_backend(&manuscript, &track).await;
                }
            }
            ...
            ("COMPARE_INSPECTED", n) if n >= 4 => {
                ...
                if run_id == run.run_id && run.phase == "inspecting" {
                    ...
                    run.summary = if existing_count > 0 {
                        format!("{} {existing_count} already marked.", rest[1])
                    } else {
                        rest[1].clone()
                    };
                    run.message =
            ...
            ("COMPARE_EXPORTED", n) if n >= 3 => {
                let run_id = rest[0].clone();
                let (added, skipped): (i64, i64) =
                    (rest[1].parse().unwrap_or(0), rest[2].parse().unwrap_or(0));
                let mut run = self.transcript.lock().await;
                if run_id == run.run_id && run.marker_export_phase == "exporting" {
                    ...
                    run.marker_export_message = format!(
                        "Exported {added} marker{}; skipped {skipped} existing.",
                        if added == 1 { "" } else { "s" }
                    );
                    self.persist_last_comparison(&run);
                    drop(run);
            ...
            ("ERROR", _) => {
                let message = rest
                    .first()
                    .cloned()
                    .unwrap_or_else(|| "REAPER integration failed.".to_string());
                let mut run = self.transcript.lock().await;
                if run.marker_export_phase == "exporting" {
    ...
    async fn launch_backend(&self, manuscript: &str, track: &str) {
        ...
        let output = self
            .config
            .session_dir
            .join(format!("results_{}.txt", run.run_id))
            .to_string_lossy()
            .to_string();
        let progress = self
            .config
            .session_dir
            .join(format!("progress_{}.txt", run.run_id))
            .to_string_lossy()
            .to_string();
        let log_path = self
            .config
            .session_dir
            .join(format!("log_{}.txt", run.run_id))
            .to_string_lossy()
            .to_string();
        let mut args = Vec::new();
        if !compare_backend.is_empty() {
            args.push(compare_backend);
        }
        args.extend([
            "--manifest".to_string(),
            run.manifest.clone().unwrap(),
            ...
            output.clone(),
            "--diff-out".to_string(),
            run.diff_path.clone().unwrap_or_default(),
            "--model".to_string(),
            run.pending_options
                .get("model")
                .cloned()
        ...
        if let Some(chunk) = run.pending_options.get("chunk").filter(|value| {
            value
                .trim_start_matches('-')
                .chars()
            ...
            args.push(if workers == "Auto" || workers.is_empty() {
                "0".to_string()
            } else {
                workers
            });
    ...
    pub async fn poll_backend(&self) {
                ...
                if let Some(last) = text.lines().last() {
                    if let Some((stage, rest)) = last.split_once('|') {
                        if !rest.is_empty() {
                            ...
                            run.message = if detail.is_empty() {
                                stage.to_string()
                            } else {
                                detail.to_string()
                            };
        ...
        if let Some(log_path) = &log_path {
            if let Ok(bytes) = std::fs::read(log_path) {
                let mut run = self.transcript.lock().await;
                let offset = usize::try_from(run.log_offset)
                    .unwrap_or(usize::MAX)
                    .min(bytes.len());
                if offset < bytes.len() {
        ...
        {
            let mut bridge = self.bridge.lock().await;
            let _ = bridge.send("inspect_compare_results", &[&run_id, &output]);
        }
        let mut run = self.transcript.lock().await;
...
pub fn spawn_bridge_loop(state: std::sync::Arc<AppState>) {
    tokio::spawn(async move {
        loop {
            ...
            for line in events {
                state
                    .handle_bridge_event(super::bridge::decode_fields(&line))
                    .await;
            }
...
mod tests {
    ...
    fn project(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "narration-utils-transcript-{label}-{}",
            std::process::id()
        ));
    ...
    fn state(project: PathBuf, compare_python: String, compare_backend: String) -> Arc<AppState> {
        let session_dir = std::env::temp_dir().join(format!(
            "narration-utils-transcript-session-{}-{}",
...
"#).unwrap();
    ...
    async fn start_requires_an_imported_manuscript() {
        ...
        let error = state
            .transcript_start(Default::default())
            .await
            .expect_err("must be rejected");
        assert!(
            error.message.contains("import a manuscript"),
            "unexpected message: {}",
            error.message
        );
    ...
    async fn start_writes_hints_and_sends_prepare_compare_over_the_bridge() {
        ...
        let hints = std::fs::read_to_string(project.join("TranscriptCompare/vocabulary_hints.txt"))
            .unwrap();
    ...
    async fn compare_marker_event_defaults_missing_optional_fields() {
        ...
        state
            .handle_bridge_event(vec![
                "COMPARE_MARKER".to_string(),
                "run-1".to_string(),
                "row-1".to_string(),
                "MISREAD".to_string(),
                "name".to_string(),
        ...
        assert_eq!(
            row["audioContext"], "audioword",
            "must default to audioText"
        );
    ...
    async fn full_compare_lifecycle_runs_through_a_fake_backend_to_inspecting() {
        ...
        state
            .handle_bridge_event(vec![
                "COMPARE_PREPARED".to_string(),
                run_id.clone(),
                manifest,
                project
                    .join("narration-utils/manuscript/manuscript.json")
                    .to_string_lossy()
                    .to_string(),
        ...
        for _ in 0..100 {
            state.poll_backend().await;
            if state.transcript.lock().await.phase != "running" {
                break;
            }
        ...
        assert_eq!(
            ...
            "inspecting",
            "snapshot: {:?}",
            state.transcript.lock().await.snapshot()
        );
        ...
        let sent_inspect = std::fs::read_dir(&commands_dir).unwrap().any(|entry| {
            std::fs::read_to_string(entry.unwrap().path())
                .unwrap_or_default()
                .contains("inspect_compare_results")
        });
        assert!(
            sent_inspect,
            "expected an inspect_compare_results bridge command"
        );
    ...
    async fn add_equivalence_requires_a_single_word_misread_row() {
        ...
        assert!(
            state.transcript_add_equivalence("r1").await.is_err(),
            "wrong kind must be rejected"
        );
        assert!(
            state.transcript_add_equivalence("r2").await.is_err(),
            "multi-word text must be rejected"
        );
        let message = state
            .transcript_add_equivalence("r3")
            .await
            .expect("valid single-word MISREAD succeeds");
        assert_eq!(message, "Added equivalence: alpha = beta");
        let contents =
            std::fs::read_to_string(project.join("TranscriptCompare/equivalences.csv")).unwrap();
    ...
    fn hints_round_trip_and_dedupe_case_insensitively() {
        ...
        let state_dir = std::env::temp_dir().join(format!(
            "narration-utils-transcript-hints-session-{}",
            std::process::id()
        ));
        ...
        assert_eq!(
            state.transcript_hints(),
            vec!["Alice".to_string(), "Rabbit".to_string()]
        );
        let _ = std::fs::remove_dir_all(project);
    }
}
