...
pub fn decode_fields(line: &str) -> Vec<String> {
    line.trim_end_matches(['\r', '\n'])
        .split('|')
        .map(|field| percent_decode_str(field).decode_utf8_lossy().into_owned())
        .collect()
}
...
impl BridgeClient {
    pub fn new(session_dir: impl Into<PathBuf>) -> Result<Self, String> {
        ...
        fs::create_dir_all(session_dir.join("commands"))
            .map_err(|error| format!("Could not create bridge command directory: {error}"))?;
        Ok(Self {
            session_dir,
            counter: 0,
            event_offset: 0,
    ...
    pub fn send(&mut self, action: &str, fields: &[&str]) -> Result<PathBuf, String> {
        let target = self
            .session_dir
            .join("commands")
            .join(format!("{:08}.cmd", self.counter));
        self.counter += 1;
        let temporary = target.with_extension("tmp");
        let mut values = vec![PROTOCOL_VERSION.to_string(), action.to_string()];
        values.extend(fields.iter().map(|value| (*value).to_string()));
        let refs = values.iter().map(String::as_str).collect::<Vec<_>>();
        fs::write(&temporary, format!("{}\n", encode_fields(&refs)))
            .map_err(|error| format!("Could not write bridge command: {error}"))?;
        fs::rename(&temporary, &target)
            .map_err(|error| format!("Could not activate bridge command: {error}"))?;
        Ok(target)
    }
    pub fn read_events(&mut self) -> Vec<String> {
        ...
        let offset = usize::try_from(self.event_offset)
            .unwrap_or(usize::MAX)
            .min(bytes.len());
...
mod tests {
    ...
    fn matches_python_quote_safe_empty_for_unicode_and_protocol_delimiters() {
        assert_eq!(
            encode_fields(&["hello world", "A|B", "é", "~_.-"]),
            "hello%20world|A%7CB|%C3%A9|~_.-"
        );
        assert_eq!(
            decode_fields("hello%20world|A%7CB|%C3%A9|~_.-\n"),
