...
[Studio UI Primitives](../prds/studio-ui-primitives.prd.md) Phase 1 ([ADR 0360](../adr/0360-studio-primitives-land-as-flat-leaf-files-after-one-token-batch-and-capability-gating-is-a-primitive.md)) adds every colour token the ten new primitives need, in one phase that runs alone, so no later phase touches `styles.css`, `paletteContrast.test.ts` or `tokenContrast.ts`.

- **Meter zones (`LevelMeter`):** `--meter-floor`, `--meter-body`, `--meter-hot` and `--meter-over` are named for their role, not new hues. Each is a derived alias — `--meter-floor: var(--non-text)`, `--meter-body: var(--ok)`, `--meter-hot: var(--warn)`, `--meter-over: var(--danger)` — declared once, the same way `--row-alt` is: it needs no dark or booth override of its own, because it resolves through whichever theme's aliased token is active. Each still gets its own `PAIRS` row (as a mark, 3:1 on `--surface`) so a later phase that breaks the alias fails the guard on the meter specifically, not only on the token it borrowed from.
- **Badge fills (`StatusBadge`):** `--badge-ok-fill`, `--badge-warn-fill`, `--badge-info-fill`, `--badge-danger-fill` and `--badge-experimental-fill` are each a 14% tint of the tone colour (`color-mix(in srgb, var(--<tone>) 14%, transparent)`), declared once and checked as a text pair (4.5:1) against that tone's own `-text` colour on `--surface`. 14%, not the entity badges' 18% ([ADR 0059](../adr/0059-text-colours-meet-wcag-aa-with-two-text-levels-a-non-text-token-and-derived-on-tint-text.md)): at 18%, `--danger-text` on an 18% `--danger` tint measures 4.29:1 in the dark theme. 14% holds 4.5:1 or more for every tone, in light, dark and booth.
...
- **The booth surface (`FocusShell`, Q1):** `[data-surface='booth']` is a scoped block, not a third app theme, so it applies inside `FocusShell` regardless of the narrator's light/dark preference — the booth is a place, not a preference. It lists only the tokens booth mode actually changes (`--bg`, `--surface`, `--surface-2`, `--surface-3`, `--border`, `--text`, `--text-muted`, `--non-text`, plus `--font-size-booth-script` for the larger script size a later phase reads); everything else, including the meter zones and badge fills above, falls through from the dark theme because they are derived from tokens the booth block does not override. `src/tokenContrast.ts` parses the block (`boothTokens`, `countBoothBlocks`) and layers it over dark as a third map (`parseThemes` returns `{ light, dark, booth }`), and `paletteContrast.test.ts` runs every declared pair over booth as well as light and dark, not only the new ones: `THEME_NAMES` is `['light', 'dark', 'booth']`.
