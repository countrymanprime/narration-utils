...
- **What is not wrapped.** The alias combobox in `GuideDetail.tsx` (domain matching and an active index), the six native `<select>`s, `Button`, `NavButton`, `Heading`, `Panel`, `Highlight` and `ErrorBoundary` stay as they are; a page that needs a new widget gets a new primitive.
