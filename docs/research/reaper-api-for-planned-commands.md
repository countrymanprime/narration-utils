...
## FX: `list_fx_chains`, `apply_fx_chain`, `list_fx`, `add_take_fx` (read, then write)

Serve Edit and Proof Workspace Phases 8 and 9 (EP8, EP9). **The owner's rule (2026-09-25, [ADR 0234](../adr/0234-fx-chains-go-on-tracks-and-a-passage-of-a-take-gets-one-plug-in-at-a-time.md)): an FX chain goes on a track or the master track only; a passage of a take gets single plug-ins, one per request, never a chain.**
...
| `MediaTrack track = reaper.GetMasterTrack(ReaProject proj)` | "Get a MediaTrack-object of the MasterTrack". `apply_fx_chain` takes `master` (or the master track's GUID) for it; `CountTracks`/`GetTrack` do not list it. |
| `integer index = reaper.TrackFX_AddByName(MediaTrack track, string fxname, boolean recFX, integer instantiate)` | "Specify a negative value for instantiate to always create a new effect … Returns -1 on failure or the new position in chain on success. This allows also adding .FXChain-files. Just provide the full path+filename to the parameter fxname." `apply_fx_chain` passes the chain's full path, `recFX` false (the track's own FX, not its input FX) and -1. REAPER saves chains as `.RfxChain`. Whether a chain with several plug-ins answers the first new index, and whether a missing plug-in inside it is reported, is only known from a run (spike SF of the workspace PRD). |
| `integer count = reaper.TrackFX_GetCount(MediaTrack track)` | "returns the number of trackfx in the FXChain of track": before and after, so the command reports how many FX the chain added. |
| `boolean retval, string name, string ident = reaper.EnumInstalledFX(integer index)` | "Enumerates installed FX … for video-processor(index=0) and fx-container(index=1), retval ident is "Video processor" or "Container" respectively." This is how a single plug-in is told apart from a chain: `list_fx` lists these names without the container (which holds a chain) and the video processor, and `add_take_fx` accepts only a name it lists, so never a chain file or a path. |
| `MediaItem right = reaper.SplitMediaItem(MediaItem item, number position)` | "The original item becomes the left-hand split, the function returns the right-hand split (or NULL if the split failed)". `add_take_fx` splits at the passage's end first and then at its start, so the piece in the middle is the second call's return value (or the original item when the passage starts at the item's start). |
| `integer index = reaper.TakeFX_AddByName(MediaItem_Take take, string fxname, integer instantiate)` and `TakeFX_GetCount(take)` | "Adds or queries the position of a named FX in a take." `add_take_fx` passes the listed plug-in name and -1, and checks the count grew by exactly one. Sending it again for the same piece adds another plug-in without splitting again. |

`apply_fx_chain` is one undo block ("Narration Utils: apply FX chain <name> to <track>") and touches no item. `add_take_fx` is one undo block ("Narration Utils: add take FX <name>") holding both splits and the add. The pass checks one Undo in REAPER removes a chain from a track and rejoins a split item, what a split does to the item's take markers and extension data (spike S0 says line ids survive a split), and whether REAPER's default split crossfade changes the passage's edges.
...
| FX | None loads an `.RfxChain` or lists the `FXChains` folder; they add plug-ins by name with `TakeFX_AddByName(take, name, -1)` and danishaft refuses a negative return. | `add_take_fx` does the same for one listed plug-in and checks exactly one was added; `apply_fx_chain` refuses a negative return and checks `TrackFX_GetCount` grew. Loading a chain by path has no outside precedent, so the verification pass (row A8) is its only evidence. |
| Splitting | danishaft splits the end first, then the start, skips a split within an epsilon of the item's edges, and refuses when `SplitMediaItem` returns nil; TwelveTake reports a nil split. | `add_take_fx` does the same, inside its one undo block. |
