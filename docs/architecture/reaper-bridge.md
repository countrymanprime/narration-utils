...
| `narration_cleanup.lua`, `narration_compare.lua`, `narration_line_identity.lua`, `narration_pickups.lua`, `narration_render.lua`, `narration_project_state.lua`, `narration_retake_lanes.lua`, `narration_take_review.lua`, `narration_navigation.lua`, `narration_track_state.lua`, `narration_transport.lua`, `narration_workspace.lua`, `narration_regions.lua` | The commands, one file per feature (the cleanup launchers; Transcript Compare; manuscript line identity; the pickup list; per-chapter render configuration; the live project-change indicator; the retake-lane pick; take creation; going to and looping a finding, and adding its approved marker; the live transport and track state; arming and recording; the workspace's take and FX actions; chapter and credits regions). |
...
`narration_ui_bridge.lua`'s tick loop also appends a `PROJECT_STATUS` event every `HEARTBEAT_INTERVAL_SECONDS` (1.5s), independent of any command: `PROJECT_STATUS||<rpp>|<unsaved>|<changeCount>`, with an empty run ID and `<rpp>` exactly what `EnumProjects(-1, '')` returns (the empty string, never `nil`, for an unsaved project - confirmed on a real REAPER 7.80, [spike S6](../research/reaper-spike-s6-daw-reachability.md)). `<changeCount>` is `GetProjectStateChangeCount(0)`, REAPER's edit counter, appended for DAW chapter-track auto-sync Phase 4 so the host can tell the project changed before it is saved; it is optional in `wire.go` (an older script sends three fields) and empty on a REAPER without the call. The empty run ID is deliberate: `events.go`'s fan-out already delivers a run-less event to every subscriber regardless of `Owns` ("Routing" above), so this is a broadcast with no new delivery mechanism. `apps/desktop/internal/daw.Reachability` subscribes to it on the same `bridge.Client` `transcript.Service` uses (`configureLocked`), tracks the freshness of the last heartbeat (5s timeout) and exposes `Reachable()`, `Matches(linkedPath)` (case-insensitive, path-normalized) and `ChangeCount()`; chapter sync's watcher (`apps/desktop/chaptersync_watch.go`, [ADR 0210](../adr/0210-chapter-sync-watches-the-saved-rpp-and-reapers-edit-counter-and-sends-reaper-nothing.md)) reads `ChangeCount()` only while `Matches` names the watched `.rpp`, to say REAPER has unsaved edits, and sends nothing; `dawfacts.go`'s `dawLinkFacts` uses `Reachable()` and `Matches` to compute Bootstrap's `dawReachable`/`dawProjectMatches` facts ([ADR 0092](../adr/0092-reaper-executable-discovery-heartbeat-mechanism-and-script-plus-project-launch-are-resolved.md), project-workspace-and-daw-link PRD Phase 7, W10). The Lua harness's `Session:events()` filters `PROJECT_STATUS` out for every other test file's sake (it would otherwise be a stray trailing event in every exact event-list assertion); `Session:all_events()` includes it, used only by `reachability_test.lua`.
...
| `stamp_item_lines`, `read_line_ids` | see [manuscript line identity](manuscript-line-identity.md) | `LINES_*` |
| `create_regions` | `run_id`, `payload_path` (`start\|end\|title` rows), `RRGGBB` or empty, `update` (`1` or `0`) | `REGIONS_CREATED\|run\|created\|existing\|invalid\|updated\|ambiguous\|failed`, or `ERROR`. Experimental |
...
| `chapter_track_state` | `run_id`, `track_guid` (empty for the transport only) | `TRACK_STATE\|run\|guid\|playState\|editCursor\|playPosition\|rpp\|unsaved\|changeCount\|thisArmed\|armedCount\|recInput\|inputDevice`, a `TRACK_ITEM\|run\|itemGuid\|takeGuid\|position\|length\|sourceOffset\|playrate\|sourceFile` per item (at most 1000), then `TRACK_STATE_END\|run\|listed\|total`; `TRACK_STALE\|run\|guid` when the track is gone, or `ERROR`. Experimental (below) |
| `arm_only` | `run_id`, `track_guid` | `ARMED\|run\|guid\|disarmed\|changed`, `TRACK_STALE`, or `ERROR`. Experimental |
| `record_start` | `run_id`, `track_guid` (the one armed track) | `RECORD_STARTED\|run\|guid\|position` once REAPER reports recording, `TRACK_STALE`, or `ERROR` (a refusal, or REAPER did not start); later `RECORD_ENDED\|run\|restored\|kept` if the narrator stops it in REAPER. Experimental |
| `record_stop` | `run_id` | `RECORD_STOPPED\|run\|restored\|kept`, or `RECORD_NOT_OURS\|run` when no recording this bridge started is running. Experimental |
| `set_active_take` | `run_id`, `item_guid`, `take_guid` | `ACTIVE_TAKE_SET\|run\|itemGuid\|takeGuid\|changed`, `ITEM_STALE\|run\|guid\|reason`, or `ERROR`. Experimental |
| `list_fx_chains` | `run_id` | `FX_CHAIN\|run\|name` per chain, then `FX_CHAINS_LISTED\|run\|count\|truncated`. Experimental |
| `apply_fx_chain` | `run_id`, `track` (a track GUID, or `master`), `chain` (relative to `FXChains`) | `FX_CHAIN_APPLIED\|run\|name\|track\|added`, `TRACK_STALE`, or `ERROR`. Experimental |
| `list_fx` | `run_id` | `FX_PLUGIN\|run\|name` per installed plug-in, then `FX_PLUGINS_LISTED\|run\|count\|truncated`. Experimental |
| `add_take_fx` | `run_id`, `item_guid`, `take_guid`, `source_start`, `source_end`, `plugin` (a `list_fx` name) | `TAKE_FX_ADDED\|run\|name\|itemGuid\|takeGuid\|splits`, `ITEM_STALE`, or `ERROR`. Experimental |
| `close` | none | none; the loop stops |

### Experimental commands (D38)

The commands of stack S28 are built in full and tested by the harness, but what REAPER does with their calls is checked only by [the verification pass](../operations/reaper-verification-pass.md), which has not run yet. Until it confirms a command, the host refuses to send it unless the narrator has turned its capability on: its own `DAW.capability.<name>` setting (`auto` by default), or, for a capability left on `auto`, the **Experimental REAPER actions** setting (`DAW.experimental_reaper_actions`, off by default). `apps/desktop/internal/bridge/actions.go` (`Actions`) asks its gate before writing any command file, and the gate is the DAW port resolver's answer (`dawport/reaper.Gate`, [ADR 0304](../adr/0304-bridge-actions-asks-a-per-command-gate-and-the-daw-ports-resolver-answers-it-from-the-per-capability-settings.md)). A command leaves its `experimentalCommands` list, and its capability's declaration moves to Supported, in the PR that records its pass ([ADR 0230](../adr/0230-reaper-commands-built-before-the-verification-pass-are-refused-by-the-host-while-the-experimental-switch-is-off.md)). The calls each command makes are in [the ReaScript calls behind the planned commands](../research/reaper-api-for-planned-commands.md). No host binding calls `Actions` yet: the bindings arrive with the UI that uses them.

### Live transport and track state (`narration_track_state.lua`)

`chapter_track_state` ([ADR 0231](../adr/0231-chapter-track-state-is-one-read-only-answer-of-the-transport-the-arms-the-input-device-and-one-tracks-items.md), Proposed) is the one read-only live read the resume prompt, the Read Aloud control bar and the teleprompter's input detection share: the play state bits, the edit cursor and play position, the saved path and unsaved flag, the edit counter, whether the named track is armed and how many tracks are, the track's record input and REAPER's input device, then the track's items with their active takes. It opens no undo block and sets nothing. A track GUID that does not resolve answers `TRACK_STALE` and nothing else; `bridge.Actions.ChapterTrackState` also refuses an answer for a GUID it did not ask about. Its first host consumer is `ReadAloudReaperState(chapterId)` (`apps/desktop/readaloudreaper.go`, [ADR 0249](../adr/0249-read-aloud-asks-reaper-once-whether-the-chapters-linked-track-is-the-one-track-armed.md)): it asks about the chapter's one confirmed link and answers `ready`, `not_armed`, `other_armed`, `several_armed`, `no_link`, `recording_elsewhere` or `unavailable`, on demand, never on a timer.

### Recording in REAPER (`narration_transport.lua`)

`arm_only`, `record_start` and `record_stop` ([ADR 0232](../adr/0232-arming-for-a-recording-remembers-the-narrators-arms-makes-no-undo-point-and-the-app-stops-only-the-recording-it-started.md), Proposed; owner decision D28) are the write side of the Read Aloud control bar's "Record in REAPER". `arm_only` arms one track and disarms the others without an undo point, remembering the narrator's arms the first time; they are put back when a recording the app started stops, by `record_stop` or by the narrator in REAPER (a defer watch answers `RECORD_ENDED` on the start's run), except an arm the narrator changed meanwhile. `record_start` records only when REAPER is stopped and the named track is the one armed track, presses `CSurf_OnRecord` once, and answers once `GetPlayState` reports recording (up to 30 defer cycles). `record_stop` stops only the recording this bridge started. None calls `Main_OnCommand`.

### The workspace's take and FX actions (`narration_workspace.lua`)

`set_active_take` ([ADR 0233](../adr/0233-the-app-may-make-a-take-active-by-guid-in-one-undo-step.md)) makes a take active by GUID in one undo block. FX follow the owner's rule ([ADR 0234](../adr/0234-fx-chains-go-on-tracks-and-a-passage-of-a-take-gets-one-plug-in-at-a-time.md)): **an FX chain goes on a track or the master track only**, and **a passage of a take gets one installed plug-in at a time**, never a chain. `list_fx_chains` lists the narrator's chains under `<resource path>/FXChains` by relative name and `apply_fx_chain` adds one to a track (`TrackFX_AddByName`), resolved by name inside `FXChains` on the Lua side; `list_fx` lists the installed plug-ins (`EnumInstalledFX`, without the FX container and the video processor) and `add_take_fx` splits a passage of the active take out at its edges and adds one listed plug-in to the middle piece, in one undo block. Nothing here calls `Main_OnCommand`.

### Regions for chapters and credits (`narration_regions.lua`)

`create_regions` ([ADR 0235](../adr/0235-one-create-regions-command-serves-chapter-and-credits-regions-and-replaces-create-chapter-regions.md)) replaced `create_chapter_regions`, keeping its payload, duplicate rule and undo block, which the scripted REAPER 7.80 checklist verified. It serves the chapter regions and the credits regions alike; with the update flag, the one region holding a row's title moves to the row's bounds, keeping its number and colour.
...
| `*_test.lua` | Characterization tests: `protocol_test` (the loop), `registry_test` (the registry and the feature-file convention), `compare_test`, `line_identity_test`, `pickups_test`, `render_test` (`configure_chapter_render`, including the never-calls-`Main_OnCommand`/`RENDER_STATS` check), `cleanup_test` (`launch_cleanup_tool`: the allow-list, lookup by name, and that a launch is one `Main_OnCommand` and no edit), `retake_lanes_test` (`pick_retake_lane`: one lane value in one undo block and nothing else, and the refusals), `navigation_test` (`navigate_item`, `loop_context`, `stop_loop`, `ping`), `track_state_test` (`chapter_track_state`: the transport, the arms, the items, the stale track and that nothing changes), `transport_test` (`arm_only`, `record_start`, `record_stop`: every refusal, the remembered arms, a stop in REAPER, a late start), `workspace_test` (`set_active_take`, `list_fx_chains`, `apply_fx_chain`, `list_fx`, `add_take_fx`: the stale rules, chains only on tracks, one plug-in per passage request, the splits and the one undo block), `regions_test` (`create_regions`), `finding_marker_test` (`add_finding_marker`), `launcher_test` (the launcher from an installed-bundle layout), `common_test` (the helpers). |
| `mutations.json`, `mutations.py` | Each entry breaks one guard in the Lua source (a stale GUID resolving to another item, an overwrite without asking, a spurious undo point, a widened duplicate window) and the suite must fail. A mutation that survives, or whose text is no longer in the source, fails the run. Each mutation runs its feature's own test file first (`narration_X.lua` → `X_test.lua`) and stops at the first failing file, so the check stays fast as mutations are added; one that no test catches still runs every file. |
