...
| `narration_compare.lua`, `narration_line_identity.lua`, `narration_pickups.lua`, `narration_render.lua`, `narration_project_state.lua`, `narration_take_review.lua`, `narration_navigation.lua` | The commands, one file per feature (Transcript Compare; manuscript line identity and chapter regions; the pickup list; per-chapter render configuration; the live project-change indicator; take creation; going to and looping a finding). |
...
- **Consumers today:** the Transcript Compare service subscribes to `COMPARE_*`/`ERROR`, `apps/desktop/internal/lineidentity` to `LINES_*`/`ERROR`, `apps/desktop/internal/pickups` to the pickup tags (`PICKUPS_IMPORTED`, `PICKUPS_EXPORTED`, `PICKUPS_COUNTED`, `PICKUP_NEXT`, `PICKUP_RESOLVED`) and `ERROR`, and `apps/desktop/internal/renderconfig` to `RENDER_CONFIGURED`/`ERROR`, each for its own run; `bridge.Navigator` subscribes to the navigation answers (`NAVIGATED`, `LOOP_STARTED`, `LOOP_STOPPED`, `PONG`, `FINDING_STALE`, `ERROR`) of the requests it has in flight ([going to and looping a finding](reaper-navigation.md); `configureLocked` constructs it on the same client for the Review page's bindings). The teleprompter integration subscribes the same way when it lands.
