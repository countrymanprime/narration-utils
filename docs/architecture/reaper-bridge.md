...
| `narration_compare.lua`, `narration_line_identity.lua`, `narration_pickups.lua`, `narration_render.lua`, `narration_project_state.lua`, `narration_take_review.lua`, `narration_navigation.lua` | The commands, one file per feature (Transcript Compare; manuscript line identity and chapter regions; the pickup list; per-chapter render configuration; the live project-change indicator; take creation; going to and looping a finding, and adding its approved marker). |
