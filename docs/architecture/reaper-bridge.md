...
`chapter_track_state` ([ADR 0231](../adr/0231-chapter-track-state-is-one-read-only-answer-of-the-transport-the-arms-the-input-device-and-one-tracks-items.md), Proposed) is the one read-only live read the resume prompt, the Read Aloud control bar and the teleprompter's input detection share: the play state bits, the edit cursor and play position, the saved path and unsaved flag, the edit counter, whether the named track is armed and how many tracks are, the track's record input and REAPER's input device, then the track's items with their active takes. It opens no undo block and sets nothing. A track GUID that does not resolve answers `TRACK_STALE` and nothing else; `bridge.Actions.ChapterTrackState` also refuses an answer for a GUID it did not ask about.
