...
| `apply_fx_chain` | `run_id`, `item_guid`, `take_guid`, `source_start`, `source_end`, `chain` (relative to `FXChains`) | `FX_CHAIN_APPLIED\|run\|name\|itemGuid\|takeGuid\|splits\|added`, `ITEM_STALE`, or `ERROR`. Experimental |
...
`set_active_take` ([ADR 0233](../adr/0233-the-app-may-make-a-take-active-by-guid-in-one-undo-step.md)) makes a take active by GUID in one undo block. `list_fx_chains` and `apply_fx_chain` ([ADR 0234](../adr/0234-an-fx-chain-is-applied-to-a-passage-as-take-fx-on-a-split-out-piece-and-is-named-only-inside-fxchains.md), Proposed) list the narrator's chains under `<resource path>/FXChains` by relative name, and apply one to a passage of the active take: split at its edges and the chain added as take FX to the middle piece, in one undo block. A chain is resolved by name inside `FXChains` on the Lua side and must be one the listing holds; nothing here calls `Main_OnCommand`.
