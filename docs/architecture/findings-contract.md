...
A finding that carries a REAPER item GUID can also be gone to and looped in REAPER through `FindingsGoTo(id)`, `FindingsLoop(id)` and `FindingsStopLoop()`, with `FindingsReaperStatus()` saying whether REAPER is there (`apps/desktop/bindings_navigation.go`; [going to and looping a finding](reaper-navigation.md#from-the-review-page)). They too take only the id: the host reads `source` and `time_range`'s source times from the store. A finding whose `evidence.members` lists several reads, each with its own `item_guid`, `take_guid`, `source_start` and `source_length` (take review's pickup and duplicate_read groups), is gone to and looped one read at a time with `FindingsGoToRead(id, read)` and `FindingsLoopRead(id, read)`, `read` being the index in `members` ([ADR 0124](../adr/0124-take-review-groups-are-reviewed-on-the-review-page-each-read-is-navigated-by-its-index-and-the-scan-is-a-cancellable-job.md)); the page checks `members` against `takeReviewEvidenceSchema` (`apps/ui/src/api/schemas/takeReview.ts`), and `tests/fixtures/contracts/findings-list-take-review.json` pins it. A `take_comparison` finding (analyzer `take-comparison`, take review Phase 10, [ADR 0165](../adr/0165-a-take-comparison-is-one-finding-per-group-over-its-one-span-built-from-the-saved-project-and-never-ranked.md)) keeps its group's reads in the same `members` order, so the same two bindings go to and loop its reads; each member adds whether it was compared (or why not), its word statuses and divergences over the group's span, and its per-category take metrics. It has `confidence: null` and no suggested action: a comparison ranks nothing. The page checks it against `takeComparisonEvidenceSchema`, and `tests/fixtures/contracts/findings-list-take-comparison.json` pins it. `FindingsAddMarker(id)` adds one take marker in REAPER for a finding whose review status is `accepted`, named like Transcript Compare's markers, in one undo point ([adding the approved marker](reaper-navigation.md#adding-the-approved-marker), [ADR 0123](../adr/0123-the-approved-marker-is-one-take-marker-for-an-accepted-finding-named-like-transcript-compares-in-one-undo-point.md)).
...
- A finding can suggest one action but cannot execute it. The REAPER adapter owns execution and undo blocks, and the narrator confirms each action: the approved marker is sent only for an accepted finding, after a confirm.
...
- The dashboard can navigate a valid REAPER finding and safely mark a stale one unavailable. (Built in Phases 6 and 7; the check in REAPER from the page is pending with the owner.)
