...
| **The asset list and the install job** (`AssetsList`, `AssetsInstall*`, `AssetsVerify`, [ADR 0077](../adr/0077-every-asset-install-is-one-job-with-real-bytes-a-second-start-joins-it-and-one-hook-follows-it.md), [ADR 0079](../adr/0079-every-downloadable-asset-is-listed-installed-verified-and-removed-through-one-registry-of-providers.md)) | `assetCatalogSchema`, `assetInstallJobSchema` and `assetVerifyResultSchema` behind `parseWire`; golden files written by a Go test | `apps/desktop/bindings_assets.go`, `apps/ui/src/api/schemas/assets.ts` |
