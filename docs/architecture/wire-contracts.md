...
| **Live events** (`transcript:state`, `teleprompter:event`, `teleprompter:state`, `system:attached`, `system:notice`) | `parseWire` per event; a bad one is dropped and counted | `wailsClient.ts`, `wire/liveHealth.ts` |
...
- **`parseWire(schema, payload, { boundary, payload })`** (`apps/ui/src/api/wire/parseWire.ts`) validates a value and throws a `WireError`; `parseWireJson` does the same for the text a string binding returns. It is typed on **Standard Schema** (`wire/standardSchema.ts`), not on Zod, so no caller depends on the library, and a remote JSON payload (the in-app update manifest, D14) uses it unchanged. Only files under `apps/ui/src/api/` import `zod` (the `zod-only-in-api` rule of `.dependency-cruiser.mjs`).
