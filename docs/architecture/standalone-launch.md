...
- **"Create new" only makes the folder.** `ProjectCreate` runs `os.MkdirAll` on the chosen path and then `ProjectSwitch`. The `narration-utils/` sidecar folders (`manuscript/`, settings and so on) are created lazily by whichever service first writes to them (for example `apps/desktop/internal/manuscript/service.go:332,366`), and nothing scaffolds anything in REAPER: the DAW is `Standalone`.
