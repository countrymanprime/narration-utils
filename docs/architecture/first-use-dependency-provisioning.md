...
- **Delivered.** One catalog per kind (`config/{tts,whisper,spacy,moonshine,dictionary}-assets.json`) with pinned URLs, sizes and SHA-256; the
...
- **Derived files.** A manager may give the install a `Derive` step ([ADR 0136](../adr/0136-an-asset-may-keep-only-a-file-built-at-install-from-its-verified-archive.md)): it runs in the staging folder after the archive is checked and unpacked, builds what the install keeps from what was unpacked and removes the rest, and the manifest records the kept files with their own hashes, so `State`, `Ready` and `Verify` read them like any unpacked file. The dictionary is the one user: the 72 MB Open English WordNet JSON release becomes one 17 MB lookup index (`internal/dictionary`), and a release that is not in the shape the builder reads fails the install (`ErrBadContent`) with nothing kept.
...
start its speech phonemizer (the data a freeze silently loses), loads the five approved catalogs, writes the asset cache and checks
