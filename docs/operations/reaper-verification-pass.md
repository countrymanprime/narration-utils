...
| A7 | `list_fx_chains` | Send it. Add a second chain in a nested folder and a file that is not a chain; send it again. | `FX_CHAIN` lists `Voice/Test EQ.RfxChain` (forward slashes), then the new one; the other file is not listed; no path outside `FXChains` appears. Record whether REAPER follows a junction inside `FXChains`. |
| A8 | `apply_fx_chain` | Apply `Voice/Test EQ.RfxChain` to 1.0–2.0 s of source time on the trimmed item; read the item list back; press Undo; apply to the whole item; apply a name that is not listed and `../reaper.ini`. | Two splits and one ReaEQ on the middle piece's active take (`TakeFX_GetCount` 1), in one undo point "Narration Utils: apply FX chain Voice/Test EQ.RfxChain"; the left and right pieces have no FX; the item's line-id extension data is on all three pieces; one Undo rejoins the item and removes the FX. The whole-item case splits nothing. The last two are refused before anything changes. Record REAPER's split crossfade and what happened to take markers. |
