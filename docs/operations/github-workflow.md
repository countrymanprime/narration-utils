...
| Documentation | `docs/` | Not mirrored anywhere. The **wiki is disabled** on purpose: it is an unreviewed second copy that drifts from the docs that change with the code. A curated public site is generated from `docs/` on every build and never kept as a copy ([`tools/docs-site`](ci-and-releases.md#the-public-docs-site), [ADR 0083](../adr/0083-the-public-docs-site-is-built-by-mkdocs-with-the-material-theme-straight-from-docs-and-a-reviewed-include-list.md)); Pages is not enabled yet (owner settings below). |
...
| GitHub Pages (Settings > Pages) | **Not enabled** (the API answers 404). **Owner step:** Settings > Pages > Build and deployment > Source = **GitHub Actions** (or `gh api -X POST repos/countrymanprime/narration-utils/pages -f build_type=workflow`). Nothing else: the workflow ([`pages.yml`](../../.github/workflows/pages.yml), [what it does](ci-and-releases.md#the-pages-workflow)) brings its own `github-pages` environment on first deploy. Until it is enabled, every push to `main` shows a red `deploy` job in the `Pages` run (the `build` job passes); after enabling, re-run the latest `Pages` run or start it from the Actions tab, and the docs site is at `https://countrymanprime.github.io/narration-utils/` with the Storybook at `.../storybook/`. Optional hardening once the environment exists: Settings > Environments > `github-pages` > Deployment branches and tags > `main` only (the workflow's own `if` already refuses other refs). No agent enables it |
...
| Required status checks | None, by owner decision D11: the docs follow the live rulesets. `zizmor`, `security.yml`, CodeQL and dependency review are advisory too. If the owner ever adds one, the safe first choice is `Docs / Links (offline)` (Settings > Rules > `Pull Request` > Require status checks to pass): it runs on every pull request, docs-only ones included, so it never stays pending ([the docs link check](ci-and-releases.md#the-docs-link-check)) |
