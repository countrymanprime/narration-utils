...
| GitHub Pages (Settings > Pages) | **Not enabled** (the API answers 404). **Owner step:** Settings > Pages > Build and deployment > Source = **GitHub Actions** (or `gh api -X POST repos/countrymanprime/narration-utils/pages -f build_type=workflow`). Nothing else: the workflow ([`pages.yml`](../../.github/workflows/pages.yml), [what it does](ci-and-releases.md#the-pages-workflow)) brings its own `github-pages` environment on first deploy. Until it is enabled, every push to `main` shows a red `deploy` job in the `Pages` run (the `build` job passes); after enabling, re-run the latest `Pages` run or start it from the Actions tab, and the site is at `https://countrymanprime.github.io/narration-utils/`. Optional hardening once the environment exists: Settings > Environments > `github-pages` > Deployment branches and tags > `main` only (the workflow's own `if` already refuses other refs). No agent enables it |
