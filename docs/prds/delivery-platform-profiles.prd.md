...
**Extension (2026-09-26, benchmark recommendation 4):** the [audiobook studio benchmark](../research/audiobook-studio-benchmark.md) recommendation 4 ("Make QC trustworthy, then broaden it... Measure MP3 levels. ... Put delivery findings on the Review page. Add a book-level checklist and a book-wide spread") adds Phases 8 to 10 below: measuring levels on the encoded MP3 (not just its container), ingesting `delivery_qc` findings into the shared findings store the review-dashboard PRD built so they appear on the Review page instead of only the Delivery page, and a book-wide spread chart of loudness/RMS/noise-floor across every measured file. Citations for the extension are `file:line` on `main` at `48a882d`; everything above this line is unchanged from the original PRD. New Open Questions are numbered P11 onward, continuing the original P1-P10. Part of the [agent train](../operations/agent-train.md) (stream N-D2, wave 3), see #509.
...
**Extension questions (2026-09-26, recommendation 4):**

- [ ] **P11. How are MP3 levels measured, given `internal/measure` only reads WAV (ADR 0025's "no third runtime" and Phase 6's container-only MP3 check, ADR 0237)?** (A) Decode the MP3 to PCM using the same catalogued encoder/decoder binary this train's [render-encode-master](render-encode-master.prd.md) PRD adds (`Encoder` port, per docs), then run the existing WAV-path measurement functions unchanged on the decoded samples. (B) Write a second, independent MP3 decoder in Go. (C) Leave MP3 levels `not_checked` permanently and only ever measure the pre-encode WAV. Recommendation: (A). Writing a second decoder duplicates real engineering risk (a lossy-codec decoder is far larger than a frame-header parser) that the render-encode-master PRD's own encoder dependency already has to solve; reusing its decode path (the same binary, run in reverse) means one dependency, one license record, not two. This closes DX-2 of the diagnostics PRD (per docs) and turns `acx.rms`/`acx.peak`/`acx.noise_floor` from `not_checked` (Phase 1's own "WAV render judged for rules that apply to the uploaded MP3" risk row) into measured, once Phase 8 lands.
- [ ] **P12. What does resolving or dismissing a `delivery_qc` finding on the Review page do, given Delivery findings are file-level, not manuscript-position-level?** (A) "Go to" a delivery finding opens the Delivery page filtered to that file and rule, rather than seeking REAPER (there is no manuscript position for a level or format violation); accept/dismiss/defer behave exactly like every other finding category (evidence-versioned, per the review-dashboard PRD's own merge semantics). (B) Delivery findings get a distinct, simpler review affordance with no navigation at all. Recommendation: (A), so a narrator working the Review page's unified list never has to remember that one category behaves differently; "go to" degrading to "open the right page instead of the right timestamp" is already how [Character Continuity Review](character-continuity-review.prd.md) frames a finding whose evidence is audio-range rather than a script position (per docs).
- [ ] **P13. What does the book-wide spread show, and where does it render?** (A) A per-rule spread (min/max/median, one point per measured file) for the numeric rules only (RMS, peak, noise floor, once P11 lands MP3 levels), on the Delivery page below the per-file table, using the existing chart-is-feature-local convention (`studio-ui-primitives.prd.md`'s own "Charts... Feature-local or a later PRD" stance, per docs) rather than a new shared chart primitive. (B) A dedicated new page. Recommendation: (A); matches the benchmark's own mock 05 placement ("a book-wide spread of loudness" on the Master, QC and delivery page) and needs no new primitive or nav entry.
...
| 8 | MP3 level measurement (P11) | Decode an MP3 through the render-encode-master PRD's `Encoder`/decode path to PCM; run the existing `measure` WAV-path functions on the decoded samples; `acx.rms`/`acx.peak`/`acx.noise_floor` turn from `not_checked` to measured for MP3 inputs; DX-2 closes | pending | 9 | 6; [render-encode-master](render-encode-master.prd.md) Phase 1 (`Encoder`) | `Encoder` (`provider-ports.prd.md`, via [render-encode-master](render-encode-master.prd.md)) | - |
| 9 | Delivery findings on the Review page (P12) | A `deliveryprofile` adapter turning `not_met`/`not_measurable` rule results into `delivery_qc` findings in the shared `findings.Store` (mirroring the Transcript Compare and Manuscript Guide adapters, review-dashboard PRD Q4/Q7, per docs); "go to" opens the Delivery page filtered to the file/rule; accept/dismiss/defer with the same evidence-version semantics as every other category | pending | 8, 10 | 1 (finding IDs already profile-scoped); review-dashboard-and-findings-adoption (delivered) | none (consumes the existing findings store; no DAW/provider port) | - |
| 10 | Book-wide spread (P13) | A per-rule spread (min/max/median across measured files) for RMS, peak and noise floor, rendered on the Delivery page below the per-file table; includes MP3-sourced values once Phase 8 lands | pending | 9 | 1; soft on 8 for MP3 files | UI: existing Delivery page components (no new primitive; charts stay feature-local per `studio-ui-primitives.prd.md`) | - |
...
- **Phase 8 (extension).** Needs [render-encode-master](render-encode-master.prd.md) Phase 1's `Encoder` (decode side) to exist first; until then MP3 levels stay `not_checked` exactly as Phase 6 left them. Tests: a decoded-then-measured MP3 matches the pre-encode WAV's own measurement within a small, documented tolerance (lossy encoding is not lossless); a corrupt or truncated MP3 reports `not_measurable`, never a fabricated value.
- **Phase 9 (extension).** Tests mirror the review-dashboard PRD's own adapter tests (Q4/Q7, per docs): a `not_met`/`not_measurable` rule result becomes exactly one finding per rule per file; a rule that returns to `met` on a later measurement resolves the finding (carried-forward semantics, matching how a resolved Manuscript Guide entity drops out of the default list); dismissing a finding never mutates the profile or the measurement.
- **Phase 10 (extension).** Tests: the spread reads only measured values (never `not_checked` rows); an empty measured set shows "no measurements yet," never a zero or a guess; adding an MP3-sourced value (once Phase 8 lands) changes the spread with no code change to this phase's own rendering.
...
Phase 0 needs the owner and can run beside Phase 1's types (numbers are data). Phases 5 and 6 are independent measurement additions and can run in parallel with 3 and 4. Phase 7 waits on the chapter mapping. **Extension:** Phase 8 waits on render-encode-master's `Encoder`; Phase 9 is independent of Phase 8 (it ingests whatever rule results already exist, `not_checked` included, and simply gains more measured ones once Phase 8 lands) and can start as soon as the review-dashboard findings store is available (already delivered); Phase 10 depends only on Phase 1 and improves automatically as Phase 8 adds MP3-sourced values.
...
| 8 | `internal/deliveryprofile/*`, `internal/measure/*` (decode-then-measure path) | [render-encode-master](render-encode-master.prd.md) Phase 1 (`Encoder`, decode side) |
| 9 | new adapter in `apps/desktop/internal/deliveryprofile` or `internal/findings`, `apps/ui/src/components/review/*` | The review-dashboard PRD's own adapter files (Transcript Compare, Manuscript Guide) — same store, additive adapter, not a shared-file edit if each adapter stays its own file |
| 10 | `apps/ui/src/components/delivery/*` | Same collisions as Phase 3 (shell layout, Settings PRDs) |
...
| MP3 levels (proposed, 2026-09-26, P11) | Decode through render-encode-master's `Encoder`, then reuse the existing WAV measurement path | A second, independent Go MP3 decoder | One decode dependency (already needed for encoding) instead of two; avoids duplicating real DSP risk |
| Delivery findings' "go to" (proposed, 2026-09-26, P12) | Opens the Delivery page filtered to the file/rule, not a REAPER seek | A distinct, navigation-less review affordance | Delivery findings have no manuscript position; matches the pattern of other position-less finding categories |
| Book-wide spread placement (proposed, 2026-09-26, P13) | On the Delivery page, no new chart primitive | A dedicated new page or a shared chart primitive | Matches the benchmark's own mock placement; `studio-ui-primitives.prd.md` keeps charts feature-local |
...
**Extension research (2026-09-26):** the benchmark's own recommendation 4 and its mock 05 caption ("a book-wide spread of loudness... outputs for each platform"); the review-dashboard PRD's delivered findings-store adapter pattern (Q4, Q7, per docs), reused rather than re-invented for `delivery_qc`; `internal/measure/mp3header.go` (ADR 0237, per docs) confirmed to read the container only, so MP3 level measurement (P11) genuinely needs a decode path this extension does not build itself, borrowing it from [render-encode-master](render-encode-master.prd.md) instead.

---

*Generated: 2026-09-24*
*Status: DRAFT - open questions P1 to P10 need the owner*
*Extended: 2026-09-26 - Phases 8-10 and Open Questions P11-P13 added for benchmark recommendation 4; take their recommendation per D22 until the owner says otherwise*
...
Concept mock copied from [the audiobook studio benchmark](../research/audiobook-studio-benchmark.md#3-what-the-ideal-looks-like-concept-mocks) for Phase 10, **not yet owner-approved as a build spec** — kept marked **concept** until the owner approves it on [#510](https://github.com/countrymanprime/narration-utils/issues/510), per the agent-train wave-0 rule. It differs from the images above (2026-09-24, owner-approved): those cover Phases 1-7 as built; this one is new, for the extension only.
...
*Book-wide spread (concept)* (`11-book-wide-spread-concept.webp`) — the per-rule min/max/median spread Phase 10 adds below the per-file table. A Mockup check table will be added to Phase 10's own PR once the owner approves it and building against it begins.
