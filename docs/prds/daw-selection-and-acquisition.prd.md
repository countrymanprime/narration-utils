...
- [x] **A1. Where does this surface live?** Options: (a) a step in first-run/onboarding before or alongside the project picker; (b) a Settings panel only, reached on demand; (c) both — a first-run prompt that can be dismissed and revisited from Settings. Recommendation: (c), since a narrator mid-project should not be blocked by a DAW they already have. Adopted; built in Phase 2.
...
*Status: Phase 1 delivered (this stack's first PR). Phases 2-4 pending, sequential.*
