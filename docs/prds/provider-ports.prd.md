...
- **P7 (lane K). Ports: `TtsEngine` and `PronunciationSource` (Python).**
  ...
  - Built: `TtsDescriptor` adds `asset_kind` and has no modes; `ENGINES` ("voice engine") is empty until P8. `tts_conformance.run(engine, voice_path=, workdir=, text=, check_missing_voice=)` checks the WAV is readable and not empty, that blank text raises a `ValueError` with a message and leaves nothing at or beside the destination, and that a path with no voice fails to load. `PronunciationDescriptor` has two modes, `pronounce` and `browse` (the `BrowserLookup` role, asked for with `source.browser()`), and a `browser_host` a `browse` source must declare; `refuse(mode)` gives the refusal. `SOURCES` is a `SourceRegistry` whose `fallback_order(platform)` is its `pronounce` sources in registration order, empty until P8. `pronunciation_conformance.run(source, known=, unknown=)` checks a new `{ipa, source, confidence}` dict per call (confidence `high`, `medium` or `low`), that a miss is a `ValueError` and not a `NotSupportedError`, and that every browser URL is https on the declared host, with no credentials or port, different per name, and kept there by hostile names. The modules are imported as `narration_common.ports.tts` and `.pronunciation`, so `ports/__init__.py` is unchanged. Validate: `libs/python/tests/test_ports_tts.py`, `test_ports_pronunciation.py`.
