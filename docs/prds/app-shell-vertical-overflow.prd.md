...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Anchor escaped content and gate it | Shell root and page area `relative`, `height: 100%` chain, `TableHeader` and take comparison anchored; the kit's vertical-overflow and escaped-absolute checks (0.3.6, opt-in, on here) with kit tests; re-run the sweep at 8 sizes and record it | pending | - | Q1, Q2, Q3 | - |
