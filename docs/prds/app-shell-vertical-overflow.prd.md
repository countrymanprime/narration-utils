...
| ADR | Likely one: "the document never scrolls; the shell's page area is the only page scroller, and the suite fails an escape" | A `design-system.md` line only | It is a rule every future page must follow and the suite enforces; `architecture-decision-records` decides at merge |
