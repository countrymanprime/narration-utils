...
- **No size cap exists on imports.** `docxEntry` reads a whole zip entry with `io.ReadAll` (`docx.go:26`); the [threat model](../architecture/threat-model.md) (row 6a) lists it as a gap, filed as [#239](https://github.com/countrymanprime/narration-utils/issues/239). An EPUB is also a zip, so the same hazard applies from day one.
...
- [x] **F3. Options.** None in the first delivery, so `ManuscriptImportPreview` keeps its signature and `hostAPIVersion` does not bump. Reconsider only if a real file needs an override (for example "one line per paragraph"). Recommendation: none. **Adopted (D22): built as recommended in Phase 1; `hostAPIVersion` unchanged.**
- [x] **T1. Charset when the file is not UTF-8.** Order: BOM (UTF-8, UTF-16 LE/BE), valid UTF-8, then Windows-1252 with a notice. Recommendation: this order, no prompt. **Adopted (D22): built as recommended in Phase 1 (`decodeTxt`, `txt.go`).**
- [x] **T2. Hard-wrapped lines.** (a) Always join a block's lines with a space, as Markdown does; (b) detect: if most lines in the file are short and of similar length join them, otherwise keep line breaks (verse, addresses), and treat a file with no blank lines and long lines as one paragraph per line. Recommendation: (b), reported in `Notices`, because Gutenberg text and notes-app text differ. **Adopted (D22): built as recommended in Phase 1 (`prepareTxtBlocks`); the exact "short and similar length" band (45-100 characters, 70% majority) is an implementation judgement call recorded in [ADR 0095](../adr/0095-txt-import-decodes-by-bom-utf-8-windows-1252-and-a-chapterless-file-becomes-one-narration-chapter.md).**
- [x] **T3. Chapter headings in plain text.** Recommendation: conservative. A block of one line, or two lines where the second is a subtitle, that matches `isNarrativeMarker` (chapter, part, book, prologue, epilogue, afterword) or is a bare numeral, roman numeral or number word. A `Contents` block is a reference paragraph, never a list of chapters. ALL-CAPS lines alone are not headings. **Adopted (D22): built as recommended in Phase 1 (`classifyTxtHeading`).**
- [x] **T4. Chapterless input.** Recommendation: one `narration` chapter titled from the file name, with a notice. This is the first case where a format diverges from Word and Markdown (whose chapterless files become `opening`); decide whether to fix those the same way in a later change. Record in the ADR. **Adopted (D22): built as recommended in Phase 1, recorded in [ADR 0095](../adr/0095-txt-import-decodes-by-bom-utf-8-windows-1252-and-a-chapterless-file-becomes-one-narration-chapter.md). Whether Word/Markdown's own chapterless files should change the same way is left open, not decided here.**
- [x] **T5. Gutenberg boilerplate and underscore italics.** Strip text outside `*** START OF ... ***` and `*** END OF ... ***` when both are present; turn word-bounded `_italic_` into an italic span (`alice_raw.txt:48`). Recommendation: both, each reported as a notice; `snake_case` and lone underscores stay literal (as `markdown_inline.go` already does). **Adopted (D22): built as recommended in Phase 1 (`stripGutenbergBoilerplate`, `appendTxtItalics`).**
- [x] **E1. Where chapters come from.** (a) The EPUB nav/NCX table of contents, falling back to spine documents; (b) spine documents only; (c) `h1`/`h2` headings. Recommendation: (a). A TOC entry may point into the middle of a file, and a chapter may span several files, so a chapter is the run of blocks from one TOC target to the next, in reading order. **Adopted (D22): built as recommended in Phase 2 (`epubChapterStarts`, `epub.go`), recorded in [ADR 0101](../adr/0101-epub-import-reads-nav-then-ncx-for-chapters-caps-entries-and-refuses-drm.md).**
- [x] **E2. TOC depth.** Every depth starts a chapter, in order (parity with DOCX, where every heading is a chapter today), or top level only. Recommendation: every depth; a `Part` entry with no body text is an empty group the model already tolerates. **Adopted (D22): built as recommended in Phase 2; ADR 0101.**
- [x] **E3. Emphasis carried by CSS classes.** Semantic `<i>`, `<em>`, `<b>`, `<strong>`, `<u>` map to spans. Exporters often write `<span class="italic">` and put the style in a stylesheet. Recommendation: semantic tags in Phase 2; single-class `font-style`/`font-weight` rules from the book's own stylesheets in Phase 3; anything else is text without a span and a count in `Notices`. **Adopted (D22): semantic tags built in Phase 2 (`collectInline`, `epub_xhtml.go`); CSS-class emphasis (inline `<style>` and linked stylesheets, single-class/single-declaration rules only) built in Phase 3 (`epub_css.go`, `parseEPUBCSSClasses`); anything else counted in `Notices` (`unresolvedClasses`).**
- [x] **E4. Footnotes and non-linear items.** Drop note references (`epub:type="noteref"`) so "the end.1" never reaches the narrator; keep note bodies as `reference`. Skip spine items with `linear="no"` and say so. Recommendation: both. **Adopted (D22): built in Phase 3 - inline `epub:type="noteref"` markers are dropped (`epub_xhtml.go`, `walkInline`); a spine `itemref linear="no"` is skipped and counted in a notice (`epub.go`, `parseOPF`'s `nonLinear`). Note *bodies* are classified `reference` only when they occupy their own whole spine document carrying a matching `epub:type` (`footnotes`/`endnotes`/etc, via E4's own classification pass) - an inline note body embedded within an otherwise-narration file is not separately reclassified; see ADR 0102's Consequences.**
- [x] **E5. Limits.** Proposed: TXT at most 32 MB; EPUB at most 64 MB on disk, 16 MB per text entry and 128 MB of text in total when decompressed; images are never read. A shared capped reader that DOCX can adopt later. Recommendation: these numbers, revisited after the first real files. **Adopted (D22): built as recommended in Phase 2 (`epubMaxFileSize`, `epubMaxEntrySize`, `epubMaxTotalText`, `epub.go`); the numbers are an implementation judgement call recorded in [ADR 0101](../adr/0101-epub-import-reads-nav-then-ncx-for-chapters-caps-entries-and-refuses-drm.md), not yet revisited against a real file (F1 still unsupplied). DOCX has not yet adopted the shared capped reader (Could, out of this phase's scope).**
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Plain text import | Accepted-formats list, TXT importer, UI copy and union, fixtures, tests | complete | - | - | - |
| 2 | EPUB reading order and text | Container, OPF, spine, nav/NCX TOC, XHTML to paragraphs and spans, DRM, limits | complete | - | 1 | - |
| 3 | EPUB structure and repairs | `epub:type` classification, glued headings, CSS-class emphasis, notes, non-linear items | complete | - | 2 | - |
...
| TXT charset order (Phase 1, T1) | BOM, then valid UTF-8, then Windows-1252 with a notice, no prompt | Prompting the user; guessing without a notice | PRD's own recommendation (D22); [ADR 0095](../adr/0095-txt-import-decodes-by-bom-utf-8-windows-1252-and-a-chapterless-file-becomes-one-narration-chapter.md) |
| TXT hard-wrap detection thresholds (Phase 1, T2) | A candidate line is 45-100 characters; wrapped when at least 70% of candidates fall in that band; a single, no-blank-line block of long lines becomes one paragraph per line | A fixed line-length cutoff; always joining, as Markdown does | Implementation judgement call within the PRD's own recommended shape, documented rather than left as magic numbers ([ADR 0095](../adr/0095-txt-import-decodes-by-bom-utf-8-windows-1252-and-a-chapterless-file-becomes-one-narration-chapter.md)), the same pattern as ADR 0089's 80% TOC-match threshold |
| TXT chapterless fallback (Phase 1, T4) | One `narration` chapter titled from the file name, with a notice; Word/Markdown's own chapterless behavior is unchanged | Making Word/Markdown match TXT too, in this PRD | PRD's own recommendation (D22); out of this PRD's scope to also change the other two importers |
| EPUB chapter source (Phase 2, E1/E2) | Nav (EPUB 3) then NCX (EPUB 2) table of contents, every depth, falling back to spine documents starting at their own first `h1`/`h2` | Spine documents only; headings only | PRD's own recommendation (D22); [ADR 0101](../adr/0101-epub-import-reads-nav-then-ncx-for-chapters-caps-entries-and-refuses-drm.md) |
| EPUB emphasis scope (Phase 2, E3) | Semantic tags (`i`/`em`, `b`/`strong`, `u`) only; CSS-class emphasis stays Phase 3 | Reading the book's stylesheets in Phase 2 too | PRD's own recommendation (D22), phased as proposed |
| EPUB footnotes and non-linear items (Phase 2, E4) | Deferred to Phase 3, which builds the `epub:type` classification pass this needs | Building an isolated noteref/linear check in Phase 2 | The PRD's own phase table already scopes this to Phase 3 (Solution Detail's Classification paragraph) |
| EPUB size limits (Phase 2, E5) | 64 MB EPUB on disk, 16 MB per decompressed entry, 128 MB decompressed text total, a shared capped reader | A fixed per-file cap only; no archive-wide budget | PRD's own recommendation (D22); the exact numbers are an implementation judgement call recorded in [ADR 0101](../adr/0101-epub-import-reads-nav-then-ncx-for-chapters-caps-entries-and-refuses-drm.md), the same pattern as ADR 0095's TXT thresholds |
| EPUB chapterless fallback (Phase 2, not an open question) | Same T4 divergence as TXT: one `narration` chapter titled from the file name, with a notice | Leaving a chapterless EPUB as all-`opening` | Required by the Success Metrics' own "No un-narratable import" row, which names EPUB explicitly; [ADR 0101](../adr/0101-epub-import-reads-nav-then-ncx-for-chapters-caps-entries-and-refuses-drm.md) |
| EPUB `epub:type` → `contentKind` mechanism (Phase 3, Architecture notes' "Classification" paragraph) | An explicit `kindOverrides map[string]string` parameter on `newDraft`, keyed by title, forces the kind; the real on-page title is kept | Renaming a section's title to a canonical string the text classifier already recognizes | The PRD left this open with no recommendation ("decide in Phase 3 with `change-impact-scan`"); recorded per D22 as [ADR 0102](../adr/0102-epub-content-kind-overrides-by-title-rather-than-canonical-title-renaming.md) (**Proposed** - owner review requested) |
| EPUB CSS-class emphasis scope (Phase 3, E3) | Single-class selector, single-declaration `font-style`/`font-weight` rules only, from inline `<style>` and linked stylesheets (cached once per href for the book); anything else counted in `Notices` | Full CSS cascade/specificity resolution; multi-declaration rules | PRD's own recommendation (D22); a full CSS engine is out of proportion to "the book's own stylesheets" wording |
| EPUB note-body classification scope (Phase 3, E4) | A note body is classified `reference` only via the whole-document `epub:type` pass (its own spine file carries `epub:type="footnotes"`/`"endnotes"`/etc); an inline note body embedded within an otherwise-narration file is not separately reclassified | Parsing individual `<aside epub:type="footnote">`/`<li epub:type="footnote">` elements out of their surrounding document | Real exporters (Calibre, Vellum) put notes in their own file; the inline-aside shape is rarer and deferred rather than guessed at without a real fixture (F1 still unsupplied) - recorded as a known limitation in ADR 0102's Consequences rather than silently built and left untested |
