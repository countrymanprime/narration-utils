...
- **Bindings.** Mapping list, confirm and clear, plus a currentness read for the UI's basis label. Each binding phase bumps `hostAPIVersion` in `apps/desktop/app.go`, `apps/desktop/app_test.go` and `apps/ui/src/hostApi.ts` and regenerates `apps/ui/wailsjs/go/main/Host.{js,d.ts}`. Service pointers follow the accessor pattern of `host-binding-data-race.prd.md`.
