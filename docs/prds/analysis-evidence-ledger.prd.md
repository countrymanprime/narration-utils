...
- [x] **Q4. What makes a signal "current" in v1?** Options: (A) a `complete` chapter-level record whose fingerprints equal the current ones; (B) A, or every item's cache entry present and current even without a chapter record; (C) any item-level union. Recommendation: A. It has one clear rule to test and to explain in the evidence view; B lets a signal become `met` from parts nobody ran together (different parameter hashes, different scopes). Revisit B only if re-running a whole chapter after a one-item edit proves too slow, which the per-item cache should already prevent. **Adopted (A).** `evidence.EvaluateFingerprints`/`EvaluateChapter` (Phase 6, `staleness.go`) implement exactly this: `StateCurrent` only when the latest *complete* record's fingerprint, analyzer version, parameter hash and confirmed track all still equal the current ones; a `partial`/`failed` record, or one whose analyzer/params/mapping differ, is never `current` even when the audio itself is unchanged.
...
- [x] **Q6. What is the mapping's shape and identity?** Options: (A) `trackGuid -> chapterId` in a new `narration-utils/chapter-track-map.json`, keyed by manuscript `documentId`, holding chapter title and confirmedAt beside the id; (B) store in `settings.json`; (C) store in REAPER track extension data. Recommendation: A. B is strings-only and tool-scoped; C needs Lua (recorded decision) and is per-project-file. Because chapter ids reset on re-import, the map is derived state cleared by `resetDerived`, and the stored chapter title lets the app re-suggest the link after a re-import instead of asking again (Q9). **Adopted (A).** `evidence.MappingStore` (`apps/desktop/internal/evidence/mapping.go`, Phase 5) is exactly this shape, keyed by `documentId`, with `chapterTitle` and `confirmedAt` on each `TrackMapping`.
- [x] **Q7. Where does the mapping confirm UI live?** Options: (A) an inline prompt wherever a check needs it (Home row "Check recording", the evidence view) plus a full list on the Tracks page; (B) the Tracks page only; (C) Home only. Recommendation: A. The need appears while checking a chapter, but the whole picture (which tracks are unlinked, which links point at missing tracks) belongs on the Tracks page, which already lists tracks (`docs/utilities/tracks.md`). SR Phase 9 adds the hint there. **Adopted (A).** `MappingConfirm` (`apps/ui/src/components/mapping/MappingConfirm.tsx`, Phase 7) is the reusable inline prompt - built now, used by the Tracks page's `Chapter links` list (`apps/ui/src/components/tracks/ChapterLinksTable.tsx`) and ready to drop beside a future check, since Home's own "Check recording" row does not exist until the recording-coverage-analysis PRD lands. The Tracks page list shows every chapter's state (`linked`, `unlinked`, `missing_track`), not just the one being checked.
- [x] **Q8. How is an unsaved project labeled?** Options: (A) always show "saved project, file modified <time>"; (B) A plus a warning when the `.rpp` is older than the newest ledger record or than a source file; (C) also poll REAPER for its change count (Lua). Recommendation: B. The warning is cheap and catches the common case (record, edit, run check without saving); C is the Phase 8 Could and needs a running REAPER, which the standalone launch does not have. **Adopted (B).** `evidence.BuildBasis` (Phase 6, `staleness.go`) always sets `EvaluationBasis.Label` to "saved project, file modified `<time>`" and sets `Stale` (the warning) when the saved `.rpp`'s `ModTime` is older than the newest ledger record for the chapter (any analyzer, not just the one asking) or than the confirmed track's newest source file `ModTime`, both computed by `EvaluateChapter` in the same pass as the current fingerprint. C (a live REAPER change counter) is untouched, Phase 8's Could.
...
- [x] **Q12. Is the live change counter worth having?** Options: (A) never; (B) a Could that reads `GetProjectStateChangeCount` through the file bridge when REAPER is running, shown as "project changed since this check"; (C) required. Recommendation: B, after everything else ships. Per docs it is coarse (any change increments it) and needs Lua with a manual checklist, and nothing in the MVP depends on it. **Adopted (B), deferred (D22; stream B5, 2026-09-25).** It stays a Could, built after the signal PRDs ship, as the recommendation says. Phase 8 is skipped by design for now (see its status). The shared plumbing it would reuse already exists: `project_state`, `projectstate.ChangedSince` and their UI contract (reaper-automation-follow-through Phase 13), and the comparison baseline pattern (`TranscriptState.projectChangeCount`).
...
| 6 | Staleness evaluator | `current`, `stale` or `never` with reasons and recomposable flag, saved-project warning, basis builder for SR | complete | 7 | 2, 3, 4, 5 | - |
| 7 | Mapping confirm UI and close-out | Inline prompt, Tracks page list, states, docs, screenshots, ADR, `docs/utilities/tracks.md` update | complete | - | 5, 6 | - |
| 8 | Live change counter (Could) | Bridge read of the project change count, "changed since this check" label, manual REAPER checklist | deferred (skipped by design, Q12) | - | 6, RF (bridge plumbing) | - |
...
**Phase 6 - Staleness evaluator** (complete)
...
- **Delivered**: `evidence.EvaluatorState` (`current`/`stale`/`never`), `evidence.EvaluatorReason` (the twelve values in the data model), `evidence.EvaluatorResult`, `evidence.EvaluationBasis` and `evidence.ChapterEvaluationRequest` (`apps/desktop/internal/evidence/staleness.go`). `evidence.EvaluateFingerprints` is the pure Q4 comparison (never touches disk); `evidence.EvaluateChapter(ledger, mapping, req)` is the one call a signal PRD or SR makes - it resolves the chapter's confirmed track (`mapping.List`, D5's "consumers treat two links as unknown" implemented as `StateNever` with no reason when a chapter has zero or more than one confirmed track), reports `mapped_track_missing` when the confirmed track no longer exists in the parsed project, computes the chapter's current fingerprint (`ComputeChapterFingerprint`, which skips an unsupported item or one whose source cannot be identified rather than erroring - the safe default per the PRD's "any doubt errs to stale"), finds the newest *complete* ledger record for (analyzerId, chapterId) (a newer `partial`/`failed` record is skipped, never read as current), and returns the comparison plus `evidence.BuildBasis`'s Q8 label and warning. One implementation decision beyond the PRD's literal ledger-record shape (Solution Detail's `fingerprint{trackFingerprint, itemFingerprints, analysisKeys}`): `LedgerFingerprint` gained a fourth, additive map, `ItemFacts` (`ledger.go`), a plain-value snapshot of exactly the fields `ItemFingerprint` already hashes (position, length, mute, active-take index, a hash-summary of the active take's `SourceIdentity`) - needed because a hash cannot be inverted, and the edit-type table's specific reasons (`item_moved` vs `item_trimmed` vs `item_muted` vs `take_switched` vs `source_changed`) cannot be told apart from the opaque `AnalysisKey`/`ItemFingerprint` hashes alone. It is backward compatible (`ledgerSchemaVersion` unchanged at 1; a record from Phases 3-5 simply decodes with a nil `ItemFacts` map, and the evaluator falls back to the coarser but still-correct generic reason for that comparison branch). Coverage 92.8% (floor 92, unchanged).

**Phase 7 - Mapping confirm UI and close-out**
- **Scope**: An inline mapping prompt reusable from Home rows and evidence views, a Tracks page list showing linked, unlinked and missing-track states, `state-catalog.ts` rows and drivers, stories for any new primitive, `docs/utilities/tracks.md` and `docs/guides/using-the-app` updates with screenshots, ADRs for the fingerprint and ledger decisions (`architecture-decision-records`), and `feature-cleanup`.
...
- **Status: deferred, skipped by design (stream B5, 2026-09-25).** Q12's adopted answer is "a Could, after everything else ships", and the signal PRDs that would show the label (RC, ER, PS) have not shipped their check pages' basis lines yet. Building it now would mean a live `project_state` round trip at the start of every recording check, for a label no page shows. That would also be the first time a live REAPER value entered a ledger record, next to D6's rule that the live state is never the basis. Everything it needs exists: `project_state` (Lua, harness-tested), `internal/projectstate` with `ChangedSince`, the `ProjectState*` bindings with their UI contract, and the pattern of storing a baseline count with a run (`TranscriptState.projectChangeCount`, follow-through Phase 13). When a signal PRD asks for it, the phase is: store the count in `LedgerRecord` at run time, and have the page compare it after a `projectStateCheck()`.
...
| Currentness rule (delivered Phase 6) | A `complete` chapter record at equal fingerprints, analyzer version, parameter hash and confirmed track (`evidence.EvaluateFingerprints`/`EvaluateChapter`) | Union of item cache entries (Q4 option B) | One explainable rule; matches Q4's adopted recommendation |
| Ledger record shape extended with `ItemFacts` (Phase 6) | `LedgerFingerprint` gained a fourth map, a plain-value snapshot of the same fields `ItemFingerprint` already hashes, so the evaluator can name *why* an item changed | Sub-classify from the opaque hashes alone; store nothing extra and report only a generic "item changed" reason | A hash cannot be inverted, so distinguishing `item_moved`/`item_trimmed`/`item_muted`/`take_switched`/`source_changed` (the edit-type table's own Evaluator column) needs a plain-value fact beside the hash; kept additive and backward compatible (schema version unchanged, old records decode with a nil map and fall back to a coarser reason) rather than reworking Phase 3's already-complete record shape |
| Basis warning scope (delivered Phase 6, Q8) | The newest ledger record across *any* analyzer for the chapter, not only the one currently asking | Scope the warning to the asking analyzer's own records only | Q8's "record, edit, check without saving" case is about any evidence recorded after the last save, not specifically this analyzer's; a narrower scope would miss a warning after a different analyzer's run |
