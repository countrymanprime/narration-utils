...
| 2 | The owner's screens | Home table, Manuscript card heading, Chapters & Search, import review on the primitive; new visual states and driver assertions; aria update; design-system entry; ADR | partial — pending: the Home chapter table (`AudiobookEstimatePanel.tsx`) is not migrated (owned by a concurrent stream sharing this table's other cells); Manuscript's card heading (via `ReaderCard`, landed together with manuscript-credits-card-parity.prd.md and manuscript-chapter-header-alignment.prd.md), Chapters & Search (`ChapterNav.tsx`) and the import review's section rows (`ImportReview.tsx`) are complete, with the guard ceiling lowered for both; design-system entry and ADR 0191 done; no new visual state was needed since the existing `manuscript/*` states and the new `manuscript/chapter-header-columns` state already exercise the primitive on every chapter card | with 3 | 1 | - |
...
| Host and sidecar names (Phase 4, ADR 0239, Proposed) | One helper per language (`chaptername.Name`, `chapter_display_name`) pinned by a Go-written fixture; display text moves to the rule; the `": "` joins that are matching keys (Transcript Compare's candidates and explicit selection, the take comparison title, `takecompare_job.go`) stay | Replace every joiner, keys included | "Matching is unchanged" is this phase's own success signal; a key changes only with every side of the match and a migration |
