...
- [x] **8. Kit changes and release cadence.** Phase 3 (scaffold parity) and Phase 6 (axe) change `tools/ui-atlas-kit` and need a version bump; the settings mobile layout PRD's Phase 2 also bumps the kit (delivered as 0.3.3). Options: bundle into one kit release; or serialize. Recommendation: serialize, settings PRD first (it is smaller), then these. **Decided (D22): serialize.** Phase 3 bumps the kit to 0.3.2 in this stack; the settings mobile layout stack (S12, delivered as 0.3.3) and Phase 6 take the next versions after it.
