...
| I: failure-mode corpus | Constructed cases (I1, 2026-09-23: no longer real manuscripts), misreads listed with counts, each kept as a fixture | Phase 4 write-up in a new `docs/research/import-heading-misreads.md` (the brief that would have held it is removed) |
...
- [x] **B1. Default for build after import.** Options: (a) off; (b) on. Recommendation: (a) until the measured build time is short enough and the user has run it on a real book. **Decided: (b), per owner decision D8**, which explicitly overrides this PRD's own recommendation: build after import is on by default (`ManuscriptGuide.build_after_import` defaults `true`). Measured in Phase 3's completion (p50 of 5 runs each, Windows 11 dev venv, `en_core_web_sm` 3.8.0 passed as a model folder as the host does; the host's own `manuscript.Service` preview and commit with the checked suggestions seeded through `seedCharacterCandidates`, then `guide.Service.Build`):

  | Manuscript | Import (preview + commit) | Seeding | Build | Import then build |
  | --- | --- | --- | --- | --- |
  | `tests/fixtures/alice.docx` (808 paragraphs, 4 chapters, no suggestions) | 0.05 s | - | 6.1 s | 6.2 s |
  | Full Alice (Gutenberg text, 26,000 words, 12 chapters plus a 3-name cast list) | 0.05 s | 1.6 s (3 suggestions) | 5.9 s | 7.5 s |
  | Novel-length (the full text three times, 79,000 words, 36 chapters plus the cast list) | 0.1 s | 2.2 s (3 suggestions) | 14.7 s | 16.8 s |
  | Novel-length, rules-only (no language model) | 0.1 s | 0.6 s (1 suggestion) | 1.2 s | 1.9 s |

  The import is not the cost; the build is, at roughly 5 s of Python and model start-up plus about 1 s per 10,000 words. The 3 seeded characters were present after every build (`manual` entities kept by `merge_locked`). The default stays on (D8): a 15 s build on a novel is well inside what a progress dialog with "Continue in background" (added in Phase 3's completion, as the Story Bible page's rebuild already had, ADR 0076) makes acceptable. Not measured: the frozen release sidecar, and a book longer than 80,000 words.
...
| 3 | Build after import | Setting, per-import checkbox, `useWorkJob` extraction, chained build, failure isolation, duration measurement, visual states | complete: setting, checkbox, chained build and failure isolation; `useWorkJob` shared by Guide and Home (import and chained build); the chained build can continue in the background; import then build measured (B1: 6.2 s on the Alice fixture, 16.8 s on a 79,000-word manuscript, p50 of 5); `home/import-build-running` and `home/import-build-failed` added to the visual suite and their PNGs reviewed at every viewport with `import-confirm` (the checkbox) and `storybible/rebuild-running` | 1, 4, 6, 9 | - | - |
| 4 | Import heading failure modes | Document the title/subtitle heuristic's failure modes from constructed cases in a new `docs/research/import-heading-misreads.md` (docs only; not an evidence gate, I1) | complete: 23 constructed cases (5 controls, 18 misreads in 8 failure modes) in [the heading misreads note](../research/import-heading-misreads.md), fixtures in `tests/fixtures/heading-misreads/` pinned by `TestHeadingMisreadFixtures`; finding for Phase 5: a per-heading line toggle alone fixes only the 3 two-line-title cases (F1) | all | - | - |
| 5 | Import subtitle override | Global default plus per-heading `subtitleOverrides` (I2), commit path, review UI, API bump, visual states | complete: "Read a heading's second line as its subtitle" plus a Subtitle checkbox per row; off joins the line to the title or returns a plain-text line to the body ([ADR 0135](../adr/0135-a-subtitle-turned-off-in-the-import-review-joins-the-title-or-returns-to-the-text-by-where-its-line-came-from.md)); `ManuscriptImportCommit` takes `subtitleOverrides`, `hostAPIVersion` 29; `TestHeadingMisreadOverrides` fixes the 3 F1 cases and the F3 case from `cases.json` and leaves every case unchanged with no override; visual states `home/import-review-subtitles-off` and `home/import-review-text-subtitle`. F2 and F4 to F8 are out of reach of a line switch: #387; the dropped EPUB `<h2>` is bug #388 | 6, 9 | 4 (constructed cases as fixtures) | - |
...
| 7 | Dictionary backend | Asset catalog entry, Go reader, `SystemLookup` binding, tests, API bump | complete: `config/dictionary-assets.json` pins `english-wordnet-2025-json.zip` (SHA-256 downloaded and verified, matches GitHub's digest), registered as the `dictionary` provider in the one registry; the install builds a 17 MB lookup index from the verified release (ADR 0136, Proposed); `internal/dictionary` reads it from Go per lookup; `SystemLookup` answers `asset_required` (never downloads) or `ok`; p95 1.0-1.5 ms on the real release (budget 250 ms); `hostAPIVersion` 36; attribution in the notices and the provenance table | 4, 9 | 6 (soft: release-readiness Phase 3) | - |
| 8 | Dictionary UI | "Look up" action in `SelectionMenu`, overlay panel, states, Playwright and PNG review | complete: "Look up" in the selection menu for a one-word selection (the host's own `Normalize` rule); the answer opens a `SlideOver` ("Look up: <word>", the design system's peek-at-something-else panel) with each part of speech, numbered definitions, examples, synonyms and antonyms, all rendered as text (a test feeds it markup), and the CC BY 4.0 attribution under every answer, found or not; `asset_required` opens the standard first-use question (`AssetInstallPrompt`), asked as "Repair the dictionary?" when the index failed its check, and nothing downloads before Download; a successful download answers the lookup. Visual states `manuscript/word-lookup-definition`, `-not-found`, `-not-installed`, `-damaged` and the one-word `selection-popup`, PNGs reviewed at desktop, small-desktop and tablet (the suite has no phone viewport, ADR 0037); aria snapshots for the panel and the question. No `hostAPIVersion` change | 4, 9 | 7 | - |
...
| 4 | new `docs/research/import-heading-misreads.md`, `tests/fixtures/heading-misreads/` and its pinning test `apps/desktop/internal/importer/heading_misreads_test.go` | None |
