...
| --- | --- | --- |
| `native-notifications.md` | Not implemented, but the brief's central research question is answered: Wails v2.16.0 (`shell/go.mod:7`) already ships a cross-platform notification API | No notification code in `shell/` or `shared/ui/src` (grep, only `Toast`). The Wails module (local module cache, `pkg/runtime/notifications.go:28-66`) exposes `InitializeNotifications`, `IsNotificationAvailable`, `RequestNotificationAuthorization`, `SendNotification` and action/category variants. Windows backend (`internal/frontend/desktop/windows/notifications.go:56-158`) uses go-toast, writes an HKCU `Software\Classes\CLSID\<GUID>\LocalServer32` key on initialize (`:76-77`), and always reports available and authorized (`:152-158`), so "unavailable" cannot be detected on Windows. Wails v2 has no window-focus query (`pkg/runtime/window.go` has `WindowIsMinimised`/`WindowIsNormal` only), so "the user is not looking" must be decided in the webview (`document.hasFocus()`). The brief's `runtime.exec` PowerShell fallback is unnecessary, and so is its worry that Windows gates notifications behind a consent prompt (see N2). The only channel today is the single in-app toast: one string, fixed 2.4 s (`shared/ui/src/components/layout/Toast.tsx:8-9`), gone whether or not the user saw it (details in `interaction-feedback-audit.prd.md` Evidence 3); the OS notification is meant for "you were looking at something else", alongside the toast, never instead of it. |
| `story-bible-concurrent-build.md` | Not implemented; every building block exists | No `build_after_import`-style setting anywhere. `GuideBuild` job with real progress: `shell/app.go:837-893` (ADR 0015). Post-commit hook already used to seed characters: `shell/bindings.go:286-296`, `seedCharacterCandidates` `:305-330` (sequential Python spawns, one per checked candidate). `merge_locked` keeps `manual` entities across a rebuild: `tools/manuscript-guide/core/manuscript_guide.py:673-679`; a unit test covers it (`tests/test_manuscript_guide.py:456`). Project switch is refused while a build runs: `shell/app.go:471-478`. The brief's premise is that the Go importer parses a full manuscript in well under a second, so a second manual step has no good reason to exist (per the brief, not re-measured here; build duration is unmeasured, Phase 3 records it). The name "concurrent" is misleading: seeding and building both rewrite `manuscript_guide.json` from separate processes with no lock, so build must be sequenced after seeding, never overlapped. |
| `import-settings-workflow.md` | Not implemented; one claim is stale | No `subtitleOverrides` field anywhere. The review dialog has exactly three controls: Markdown heading level (`shared/ui/src/components/home/Home.tsx:217`), per-section content kind (`:243`), character candidate checkboxes (`:275`). The preview payload (`shared/ui/src/api/contracts/manuscript.ts:71-78`) carries `chapterTitles`, `sections`, `characterCandidates` only, so the UI has no subtitle data to edit. The brief says `docx.go` splits "first line = title, remaining = subtitle unconditionally"; the split now lives in `shell/internal/importer/headings.go:90-97` (`headingParts`, called at `docx.go:262` and `markdown.go:91`) and also repairs glued headings (ADR 0013). The value lands in `Paragraph.ChapterSubtitle` (`shell/internal/importer/model.go:13`), which the brief named as the field an override should feed before commit. The brief itself says which settings matter needs "real manuscripts with more classification failures"; none are recorded. |
...
- **Only `choice`/`color` settings validate today** (`shell/app.go:715-724`), so an on/off setting is a two-value `choice`; the UI `kind` union also has `'text'` (`shared/ui/src/api/contracts/system.ts:7`). Settings categories `ManuscriptGuide` ("Story Bible") and `Piper` ("TTS") already exist with global and project scope (`Settings.tsx:18-20`).
...
- Choosing REAPER-side or Lua changes - none of the five needs `shared/reaper`.
...
| I: override correctness | Every recorded misread fixable, zero regressions in `shell/internal/importer` fixtures | Go importer tests plus Vitest |
...
| P: override survives rebuild and locked-entry rules | Yes; `edit()` rejects a change on a locked entry (ADR 0007) | Python tests in `tools/manuscript-guide/core/tests/` |
...
- [ ] **N1. Where is "the user is not looking" decided?** Options: (a) webview decides with `document.hasFocus()` and calls a new `SystemNotify` binding (host API bump); (b) host always notifies on job completion (no focus query exists in Wails v2.16, so it would notify while the user watches); (c) frontend reports focus changes to the host. Recommendation: (a). Consequence: Phase 1 bumps `hostAPIVersion` 5 to 6 (three places) and completion is detected wherever the job poller runs (page-scoped until the interaction audit's phase 5 lands).
- [ ] **N2. Default and consent.** Options: (a) on by default, opt-out in Settings, sent only when unfocused; (b) off by default, opt-in; (c) on with a one-time in-app explanation. Wails needs no OS consent prompt on Windows (`RequestNotificationAuthorization` returns true, `windows/notifications.go:156-158`) but initialization writes a per-user registry key. Recommendation: (a), initialize lazily on the first qualifying send, and document the registry key in the Settings copy.
- [ ] **N3. Which events notify?** Options: (a) Story Bible build only; (b) build, model/voice downloads, Transcript Compare, import with chained build; (c) everything that toasts. Recommendation: (b), and only when the job took about 10 s or more so fast jobs stay quiet; that threshold is also how the brief's "manuscript import complete, if it ever becomes a longer-running operation" is handled (import qualifies only if it is ever that slow, or when it chains a build).
- [ ] **N4. Toast identity on Windows.** The AppID is the executable base name (`windows/notifications.go:66,88`), so dev builds show `narration-utils-shell.exe`-style names. Options: (a) accept for dev, verify the installed build shows "Narration Utils"; (b) set explicit app data. Recommendation: (a), verify on an installed build; TBD - needs a manual check of clicking a toast (it relaunches the exe, which the single-instance handler at `shell/app.go:405-429` should turn into a window raise).
- [ ] **B1. Default for build after import.** Options: (a) off; (b) on. Recommendation: (a) until the measured build time is short enough (say under 10 s on a typical manuscript) and the user has run it on a real book; revisit with the Phase 3 numbers.
- [ ] **B2. Where the setting lives.** Options: (a) Settings Story Bible category default (global and project scope) plus a per-import checkbox pre-filled from it; (b) global only; (c) per-import only. Recommendation: (a), as the brief suggests.
- [ ] **B3. Who chains the build.** Options: (a) UI: after import success and bootstrap refresh, call `api.guideBuild()` and show a second phase in the same import `WorkDialog` (the brief's "chained second phase in the same dialog"; no binding change); (b) host: `ManuscriptImportCommit` gains a `buildStoryBible` parameter and `runCommit` starts the build (survives navigation, changes a binding, API bump). Recommendation: (a) for the MVP; revisit if leaving Home mid-import proves common.
- [ ] **I1. Is there evidence to justify I?** Options: (a) the user supplies 3 or more manuscripts with wrong title/subtitle splits and Phase 4 documents them; (b) defer I until such manuscripts appear; (c) build it now on speculation. Recommendation: (a) or (b); not (c). Please say whether such manuscripts exist.
- [ ] **I2. Override granularity, if built.** Options: (a) per-heading in the review list; (b) one global toggle; (c) both, global default plus per-heading. Recommendation: (c), starting from the heuristic's current guess.
- [ ] **D1. Cloud or local dictionary?** Options: (a) local dataset only, provisioned as an asset after a user Download; (b) cloud API (needs an ADR exception, key management, offline behavior, privacy statement); (c) local default with an opt-in cloud lookup. Recommendation: (a). Do not assume (b) or (c).
- [ ] **D2. Scope.** Options: (a) single-word definitions plus synonyms and antonyms from one dataset, US English; (b) add phrases; (c) definitions only. Recommendation: (a). Candidates for the Phase 6 spike (all unevaluated in the repo and licensed per their own upstream terms, to be verified at artifact level): Princeton WordNet, Open English WordNet, Wiktionary-derived extracts (share-alike terms are not a class in the policy table, so treat as unclear until reviewed). Sizes: TBD.
- [ ] **P1. What does "provider" mean?** Options: (a) a second local engine (for example MFA G2P, a multi-artifact install per `local-dependency-evaluation.md:249-252`); (b) a narrator-edited project pronunciation dictionary; (c) a hosted IPA API (excluded unless D1-style decision); (d) no provider at all: make the preview speak narrator-authored phonemes or respellings. Recommendation: (b) plus (d) first, because the preview ignores IPA today; providers later.
- [ ] **P2. GPL exposure check.** `phonemizer` and eSpeak NG are GPL upstream (per upstream, unverified here) and are imported in-process by the frozen Guide sidecar; the policy asks for a license review before distributing a combined program (`local-dependency-evaluation.md:84`). Options: (a) schedule the review before adding any further engine; (b) leave as is. Recommendation: (a).
- [ ] **X1. Sequencing against the host-binding fix.** Phases 1 (`SystemNotify`), 5 (commit signature) and 7 (lookup binding) add bindings while `host-binding-data-race.prd.md` rewrites nearly all of `shell/bindings.go`. Options: (a) land the accessor phase first and write new bindings on it; (b) write new bindings with an inline snapshot under `h.mu.RLock` and rebase. Recommendation: (a) if its Phase 1 merges first; otherwise (b), never reading service fields directly.
...
Phases 1 to 3 (N and B) plus the three docs-only phases 4, 6 and 9 that de-risk the rest. I, D and P implementation phases wait for evidence or decisions.
...
- **I (if built)**: the review lists each multi-line heading with its guessed title and subtitle and a toggle; commit applies the overrides.
...
- **Lock ordering**: every new binding snapshots service pointers and releases before calling into a service (see `host-binding-data-race.prd.md`).
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Notifications: host sender and setting | Go wrapper, `SystemNotify` binding, `General.notifications` choice, contract, mock, API bump 5 to 6, tests | pending | 3, 4, 6, 9 | - | - |
| 2 | Notifications: completion wiring | App-level unfocused-only helper next to existing toasts for build, installs, comparison, import; 10 s threshold; visual states | pending | 4, 6, 9 | 1 (soft: interaction-feedback-audit phase 5) | - |
| 3 | Build after import | Setting, per-import checkbox, `useWorkJob` extraction, chained build, failure isolation, duration measurement, visual states | pending | 1, 4, 6, 9 | - | - |
| 4 | Import evidence gate | Review 3+ real manuscripts, list heading and subtitle misreads in a new `docs/research/import-heading-misreads.md` (docs only) | pending | all | - | - |
| 5 | Import subtitle override | Preview subtitle data, `subtitleOverrides`, commit path, review UI, API bump, visual states | pending | 6, 9 | 4 (must show real failures) | - |
| 6 | Dictionary spike | Dataset candidates, license and provenance gate, size, lookup engine, provisioning shape; dependency record and ADR (docs only) | pending | all | - | - |
...
| 9 | Pronunciation spike | Provider meaning, Piper phoneme input, GPL review, accuracy on real names; ADR (docs only, may run scratch code out of tree) | pending | all | - | - |
...
- **Success signal**: seeded and extracted entities coexist; build failure leaves import success visible; a test pins the Guide build flow before the hook is extracted (none exists today; also needed by `interaction-feedback-audit.prd.md` Evidence 1b) and stays green after the extraction.

**Phase 4 - Import evidence gate**
- **Goal**: decide whether I is worth building.
- **Scope**: run the import preview on at least 3 real manuscripts (from the user), tabulate headings the heuristic split wrongly in `docs/research/import-heading-misreads.md`, and record where the split lives now (`headings.go:90-97`, not `docx.go`) so the removed brief's stale claim is not reintroduced.
...
- **Scope**: catalog entry and provisioning (selecting never downloads; registered as a provider in release-readiness Phase 3's aggregated catalog, not a parallel one), reader package, binding, tests, API bump.
...
| --- | --- | --- |
| 1 | new Go notifier file, `shell/{app.go,bindings.go,app_test.go}`, `shared/ui/src/{hostApi.ts,api/wailsClient.ts,api/mockApi.ts,api/contracts/system.ts}`, `shared/ui/wailsjs/go/main/Host.*`, `Settings.tsx` copy | Every phase or PRD that adds a binding (host API version), the host-binding fix (broad `bindings.go` edits: sequence it first or write the new binding on its accessor), other `fieldSchemas` editors (teleprompter, diagnostics PRDs) |
| 2 | `Guide.tsx`, `GuideDetail.tsx`, `Transcript.tsx`, `TeleprompterPage.tsx`, `Home.tsx`, `App.tsx` | interaction-feedback-audit phases 3 and 5 (same completion sites), teleprompter-engines phase 4 (`useAssetInstall`), release-readiness Phase 1 (install loops and job shape in `GuideDetail.tsx`, `Transcript.tsx`, `TeleprompterPage.tsx`), dialog modality work |
| 3 | `Home.tsx`, `Guide.tsx` (hook extraction), `shell/app.go` `fieldSchemas`, `Settings.tsx`, `mockFixtures.ts`, visual catalog | Phase 2 here, audit phase 5, other `fieldSchemas` editors |
| 4 | new `docs/research/import-heading-misreads.md` only | None |
| 5 | `shell/internal/{importer,manuscript}`, `shell/{bindings.go,app.go}`, `shared/ui/src/api/*`, `Home.tsx`, `hostApi.ts`, Wails bindings | Phase 3 (`Home.tsx`), any binding or API bump |
| 6 | `docs/research/local-dependency-evaluation.md`, ADR | ADR numbering; `local-dependency-evaluation.md` edits by other PRDs |
| 7 | `shell/internal/assets` and a new lookup package, `shell/{bindings.go,app.go}`, `shared/config/*-assets.json` (new), API files | Any binding phase; release-readiness Phase 3 (aggregated asset catalog and provider registry: register the dictionary as a provider there rather than adding a parallel catalog) |
...
| 10 | `tools/manuscript-guide/core/manuscript_guide.py` and tests, `shell/internal/guide/service.go`, `shell/bindings.go` | Host-binding fix phases, audit phase 4 (`GuideEdit` batching touches the same functions), `manuscript_guide.py` edits |
...
Cross-cutting: every phase re-checks `docs/adr/` numbering immediately before writing an ADR (take the next free ADR number at merge time; 0027 at d5cc994); every phase that adds or changes a binding bumps `hostAPIVersion` in `shell/app.go:33`, `shell/app_test.go:40-41` and `shared/ui/src/hostApi.ts:2` and regenerates `Host.{js,d.ts}` (whichever PR lands second increments again; check `hostAPIVersion` at merge time); every `shared/ui` phase runs `visual-catalog-sync`, the Playwright visual suite with PNG review of `shared/ui/screenshots/app/<page>/<state>/<viewport>.png` at all four viewports, and `doc-screenshot-sync` (user-facing changes also update the matching page of the split guide under `docs/guides/using-the-app/`, for example `home.md`, `settings.md`, `story-bible.md`, `manuscript.md`; `shared/ui/src/docsGuide.test.ts` guards its links and screenshot embeds); primitives or `styles.css` changes also run the atlas and `design-spec-guard`; every phase follows `CLAUDE.md`: plan, `change-impact-scan`, TDD, `full-verification-gate` (`pnpm check`, not `check:fast`), `design-spec-guard`, `feature-cleanup`. Nothing merges without the user.
...
| Order and MVP | N, B first; I evidence-gated; D and P spike-first | Build in brief order | Value/risk; two briefs have blocking unknowns (proposed) |
| Notification decision point | Webview decides focus; `SystemNotify` binding (proposed) | Host always notifies | No focus query in Wails v2.16 |
| Chain location for B | UI-chained for MVP (proposed) | Host-chained | No binding change, natural failure isolation |
| On/off settings | Two-value `choice` (proposed) | New `toggle` kind | Avoids a schema and contract change |
| Order of the five | Recommendations under Open Questions | See there | Pending the user's answers |
