...
| 2 | REAPER and Audacity adapters | `dawport/reaper` wraps the bridge types; `dawport/audacity` declares all `NotYetAvailable`; registry; both pass `dawporttest.Run` | complete: `dawport/reaper` (declaration checked against `experimentalCommands` by parsing `bridge/actions.go`), `dawport/audacity`, and the `Kind`-keyed registry whose factories open their own transport from a `dawport.Env` ([ADR 0303](../adr/0303-a-daw-adapter-factory-is-given-the-launchs-environment-and-opens-the-engines-transport-itself.md), `feat/dawport-p2-adapters`) | no | 1 | |
| 3 | Per-capability toggles | `DAW.capability.<name>` rows (append-only in `config/defaults.json`, `settings/store.go`), the resolver reads them, `experimental_reaper_actions` mapped; `bridge.Actions` gating delegates to the resolver | complete: a `DAW.capability.<name>` row per capability, `auto` by default; `dawport.SettingsToggles`/`SettingsExperimental` read them and the old switch; `Resolver.Allowed` answers `bridge.Actions`' new per-command `Gate` through `dawport/reaper.Gate`, and the host gates its own `Actions` over `reaper.Declaration()` until P5a ([ADR 0304](../adr/0304-bridge-actions-asks-a-per-command-gate-and-the-daw-ports-resolver-answers-it-from-the-per-capability-settings.md), `feat/dawport-p3-capability-toggles`) | no | 2 | |
