...
- The page is standalone: chapter picker plus a typed microphone name (`shared/ui/src/components/teleprompter/TeleprompterPage.tsx:256-282`). It is a separate nav page, and the reader does not use `ParagraphView`, so it has no entity or note marks (ADR 0024 decision 1; `ReaderText.tsx:57-83` renders plain words only).
- No flags exist. The tracker deliberately ignores heard words that fit nothing nearby and steps over up to `MAX_SKIP = 2` script words (`tools/manuscript-teleprompter/core/script_tracker.py:20-26,34,73-87`). The only events it emits are `position` (with `jump`/`skipped` only for restarts and skips ahead, `script_tracker.py:98-127`). The frontend event union has no flag type (`shared/ui/src/api/contracts/teleprompter.ts:21-27`) and `reduceEvent` ignores unknown types (`readerModel.ts:111-122`).
- No seek and no host-to-sidecar channel other than `--stop-file` (`live_asr.py:486-494,628-664`). There is no `--control-file`, `--start-word` or `--locate` (grep of `tools/manuscript-teleprompter` found none).
- The host relays every valid JSON line verbatim (`shell/internal/teleprompter/service.go:209-231`), so new event types need no Go relay change. But the snapshot keeps only the last `script` and `position` (`service.go:219-227`), so any flag state would be lost when a view mounts mid-session.
...
- The modal primitive cannot host a full-screen reader: `Dialog` caps width at `max-w-[70vw]` and height at `80dvh` (`primitives/Dialog.tsx:20-24`, ADR 0001), and it has no Escape, focus trap or initial focus (`Dialog.tsx` has no key handling; the retired UI-defects register recorded this as its defect 2, now owned by `dialog-modality-and-workdialog-a11y.prd.md`).
- Host API version lives in three places, all currently 5: `shell/app.go:33`, `shared/ui/src/hostApi.ts:2`, `shell/app_test.go:40`.
...
| Visual verification | Every new state reviewed as PNG at desktop, small-desktop, tablet, mobile | `shared/ui/screenshots/app/<page>/<state>/<viewport>.png`, per CLAUDE.md |

## Open Questions

- [ ] **Microphone picker overlap with PRD 2.** PR #35 puts the microphone list in its Phase 1; PRD 2 owns enumeration UX and settings. Options: (a) PRD 2 lands first and the modal consumes its picker; (b) both PRDs share one `MicrophoneField` seam - Phase 2 here extracts the current typed field into that component, PRD 2 swaps its internals for a picker, REAPER detection (Phase 11) feeds it a `suggestedDevice`; (c) this PRD builds the picker. Recommendation: (b). No ordering dependency, one component, no duplicated scope. This PRD owns only "detect what REAPER uses" and the confident-match preselect.
...
- [ ] **Full-size dialog and modality scope.** Decided by the sibling PRD, not here: `dialog-modality-and-workdialog-a11y.prd.md` owns Dialog focus trap, Escape, initial focus, inert siblings and the modality ADR for every dialog (its Phases 1 and 2). This PRD's Phase 1 only adds the `size="full"` variant and depends on that PRD's Phase 2, so the reading modal is never a dialog that a stray Escape or Tab can escape, and no second modality ADR exists. Remaining question for this PRD: Escape must not silently stop a live session (ties to the close-behavior question); the dialog PRD's Escape rule (close only when `onClose` exists) is what the reader modal supplies or omits.
...
- **Live REAPER state.** New read-only Lua command `chapter_track_state` returning track GUID, item ends, play state, edit cursor, and a device read (spike 2). Protocol per `manuscript-line-identity.md`: `1|<command>|<args>` in `commands/NNNNNNNN.cmd`, results via `events.log` (`shell/internal/bridge/bridge.go:66-108`); report "uncertain outcome" on timeout instead of guessing (research doc 4.6). Lua stays out of the live listening loop entirely (the boundary the shipped design record drew): DAW reads are one-shot commands at chapter open or on demand, and the only mutation is the narrator-triggered punch.
...
- **Host API.** Seek, resume lookup, punch and any device call add bindings: bump `hostAPIVersion` in `shell/app.go:33`, `shared/ui/src/hostApi.ts:2`, `shell/app_test.go:40` and regenerate `shared/ui/wailsjs/go/main/Host.{js,d.ts}`. New services snapshot pointers under `RLock` like `teleprompterService` (`app.go:649-655`; the pattern is owned by `host-binding-data-race.prd.md`, which superseded the retired `host-binding-concurrency.md` brief). This PRD plans host API 5 to 6 (Phase 3), and `review-dashboard-and-findings-adoption.prd.md` also plans 5 to 6 then 6 to 7; whichever PR lands second increments again; check `hostAPIVersion` at merge time.
- **Dialog.** Phase 1 adds only a `size="full"` variant to `Dialog`; focus trap, Escape, initial focus, inert siblings and the modality ADR belong to `dialog-modality-and-workdialog-a11y.prd.md` (its Phase 2), which this PRD depends on. The reader modal keeps ADR 0002's button placement.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Dialog full-size variant | `Dialog` gains `size="full"` only; stories and atlas coverage. Modality (Escape, focus trap, initial focus, inert) is `dialog-modality-and-workdialog-a11y.prd.md` Phase 2 | pending | 3, 6, 8 | dialog PRD Phase 2 | - |
...
| 5 | Story bible and notes marks | Offset-to-word mapping, rail tabs, `EntitySummary`/`NotesPanel` reuse, no cursor or scroll change on click | pending | 3, 6, 8 | 2 | - |
...
| 7 | Flags UI | Marks, heard-text tooltip, review panel, dismiss, "Punch from here" placeholder, key entries | pending | 8 | 2, 6 (sequence after 4 and 5); `component-a11y-meter-tooltip-field-heading-panel.prd.md` Phase 1 (Tooltip reachability); `palette-contrast-wcag-aa.prd.md` Phase 4 (Highlight colours) | - |
...
- **Goal**: a dialog that can host a full-screen reader. It is modal because it builds on `dialog-modality-and-workdialog-a11y.prd.md` Phase 2; this phase does not plan its own modality work or ADR.
...
- **Scope**: `tools/manuscript-teleprompter/core/control_channel.py`, `script_tracker.py` `reset_to`, `live_asr.py` argument wiring, tests; Go service append-only control file plus `TeleprompterSeek` binding; version bump in three places plus regenerated Wails bindings; contract, `wailsClient.ts`, mock; ADR for the control channel.
...
- **Scope**: offset-to-word mapping in `readerModel`, `Highlight` reuse (existing kinds), rail tabs reusing `EntitySummary` and `NotesPanel`, sharing annotation helpers from `ParagraphView` (change-impact-scan: the Manuscript reader consumes them).
...
- **Scope**: marks (misread wavy underline, extra-word caret, skipped dotted underline; new `Highlight` kinds or a documented equivalent, ADR 0016/0017, stories, atlas with no debt growth), heard-text tooltip via `TooltipTarget`, panel with dismiss, key entries for every mark, "Punch from here" disabled until Phase 12. Depends on `component-a11y-meter-tooltip-field-heading-panel.prd.md` Phase 1 (a flag wrapped in `TooltipTarget` is reachable by keyboard and its tooltip text is attached to the mark) and on `palette-contrast-wcag-aa.prd.md` Phase 4 (the `Highlight` kinds and their text colours meet WCAG AA); build the flag marks on those, not on today's `Tooltip` and `Highlight`.
...
| 5 | `readerModel.ts`, `ReaderText.tsx`, `manuscript/ParagraphView.tsx`, `EntitySummary.tsx`, `NotesPanel.tsx`, `ReadAloudDialog.tsx` | Anything touching the Manuscript reader |
| 6 | `script_tracker.py`, new `flags.py`, `live_asr.py`, `replay.py`, `contracts/teleprompter.ts`, `teleprompterMock.ts`, `teleprompterRecording.json`, `spikes/record_mock_stream.py`, maybe `shared/python/narration_common`, `tools/transcript-compare/core/compare.py` | Phase 3 and PRD 2 phases 1 and 6 (`live_asr.py`), PRD 2 phase 7 (contract) |
| 7 | `ReaderText.tsx`, `readerModel.ts`, `Highlight.tsx` and stories, atlas debt list, tooltip use | Phases 4, 5; other `Highlight` users; must follow `component-a11y-meter-tooltip-field-heading-panel.prd.md` Phase 1 (Tooltip reachability, `TooltipTarget` on a `<mark role="button">`) and `palette-contrast-wcag-aa.prd.md` Phase 4 (Highlight colours; both edit `Highlight.tsx`) |
| 8 | `shell/internal/tracks/*` (+testdata), new matcher file, `shell/{bindings.go,app.go,app_test.go}`, `hostApi.ts`, Wails bindings, `docs/utilities/tracks.md` | Any Tracks page work; diagnostics PRD Phase 8 and the review dashboard's chapter grouping (consumers of the mapping); host API version |
| 9 | new `locate.py`, `live_asr.py`, `shell/internal/teleprompter/*`, bindings and version | 3, 6 (`live_asr.py`), PRD 2 phases 1 and 6 |
| 10 | `ReadAloudDialog.tsx`, visual catalog, mock | 2, 4 |
| 11 | `shared/reaper/narration_ui_bridge.lua`, `shell/internal/bridge`, teleprompter service, checklist doc | Any Lua work (no tests; needs the user's REAPER); the `shared/reaper` bridge is shared with Transcript Compare |
| 12 | `narration_ui_bridge.lua`, sidecar alignment mode, settings files (`shell/app.go`, `defaults.json`, `Settings.tsx`, `mockFixtures.ts`), UI | PRD 2 phases 3 and 7 (same `fieldSchemas` map and `Settings.tsx`) |
| 13 | `AppShell.tsx`, `App.tsx`, `App.test.tsx`, all doc screenshots, roadmap files, guide | Everything; do this alone |

Cross-cutting: every phase re-checks `docs/adr/` numbering immediately before writing an ADR; every phase that adds a binding bumps `hostAPIVersion` in `shell/app.go`, `shared/ui/src/hostApi.ts`, `shell/app_test.go` and regenerates `Host.{js,d.ts}`; every `shared/ui` phase runs `visual-catalog-sync`, the Playwright suite with PNG review at all four viewports, `doc-screenshot-sync`, and (for primitives or `styles.css`) the atlas and `design-spec-guard`; every phase follows CLAUDE.md: plan, `change-impact-scan`, TDD, `full-verification-gate` (`pnpm check`, not `check:fast`), `design-spec-guard`, `feature-cleanup`. Phases 11 and 12 add Lua: nothing automated proves them, so the user must run the manual REAPER checklist before they count as verified.
...
**Technical Context**
- Reused, verified: streaming supervisor and Wails events (ADR 0022), tracker and event contract (ADR 0021), reader model and paced cursor (ADR 0024), `Highlight` (ADR 0016), `Tooltip`/`TooltipTarget`, `EntitySummary`/`NotesPanel`, `.rpp` parser (`tracks`), Lua file bridge and its command protocol, the layered settings files.
