...
- [x] **"Punch from here" on an extra-word flag: before or after the inserted words.** Options: (a) word before the insertion; (b) word after. Recommendation: (a). The cursor plus pre-roll should land ahead of the first unwanted audio; the extras sit between the two words, so the word after can fall inside them.
  **Answered 2026-09-23:** (a) the word before the insertion.
- [x] **Pre-roll default length.** Options: 2 s, 3 s, 5 s, or read REAPER's own pre-roll preference (not investigated; TBD - needs research). Recommendation: 3 s as an assumption, a layered setting (`Teleprompter.punch_preroll_seconds`, global and project scope), range 0-10 s, revisit after the first real punch trials.
  **Answered 2026-09-23:** read REAPER's own pre-roll preference, overriding the 3 s recommendation. Phase 12 checks how to read it in REAPER (run only with the owner's go-ahead); if it cannot be read, the fallback is the layered setting `Teleprompter.punch_preroll_seconds` (0-10 s, default 3 s).
- [x] **Where per-viewer reading preferences (text size, rail state) live.** Options: (a) browser storage like the microphone name today; (b) global settings. Recommendation: (a) for pure UI preferences; hardware and engine choices belong in settings (see PRD 2), so the two PRDs use one rule: UI layout in browser storage, machine facts in settings.
  **Answered 2026-09-23:** (a) text size and rail state in browser storage; machine facts (microphone, engine, model) in global settings. The "like the microphone name today" example is stale: the microphone moved to global settings.
...
| Must | Suspected misread, extra, skipped and restart flags, reviewable, dismissible, never editing anything | 6, 7 |
...
| Should | Pre-roll read from REAPER's own preference, with a layered setting as the fallback | 12 |
...
| Must | Live flags persisted as suspected `transcript_discrepancy` findings (owner decision 2026-09-23) | 7 |
| Won't | Arm/record, time-selection auto-punch, trailing Whisper pass, standalone page removal before acceptance | later / 13 |
...
6. Closing the modal stops the session (with a confirm while live) and nothing is written to the manuscript or audio; the session's suspected flags stay as unreviewed findings.
...
| 4 | Seek UI | "Start here" / "Go back to here" on a word; back-seek follows via `jump`; visual states | complete | 6, 8 | 2, 3 | - |
...
| 6 | Live flag events | `flag` events in the sidecar (misread, extra, skipped, restart) with precision gates, replay false-flag report, contract types, mock recording regenerated | pending | 1, 2, 5, 8 (3 with rebases) | - | - |
| 7 | Flags UI and persistence | Marks, heard-text tooltip, review panel, dismiss, "Punch from here" placeholder, key entries; flags written as suspected `transcript_discrepancy` findings through `internal/findings` (new binding, host API bump) | pending | 8 | 2, 6 (sequence after 4 and 5); tooltip reachability is delivered ([ADR 0049](../adr/0049-hints-are-base-ui-tooltips-that-meet-wcag-1-4-13-and-info-icons-are-buttons.md)), and this phase adds the inline (flowing text) mode to `TooltipTarget`, which the component accessibility PRD deferred here; `palette-contrast-wcag-aa.prd.md` Phase 4 (Highlight colours) | - |
| 8 | Chapter-track match and recorded end | Go matcher (with number-word merge), `SOFFS`/`PLAYRATE` parse, recorded end, binding | complete | 1, 2, 3, 4, 5, 6, 7 | - | - |
| 9 | Tail-audio locate | Sidecar `--locate`, Go orchestration (cut tail, run, return word index and sentence), binding | complete ([ADR 0111](../adr/0111-the-resume-point-comes-from-transcribing-the-recorded-tail-and-placing-it-with-the-tracker.md); `TeleprompterLocate`, host API 32; measured on synthetic-voice tails, a real narrator's recording is a manual check) | 4, 5, 7 | 8 | - |
...
| 12 | Punch and roll | Live anchors from a play-position bridge command (harness-tested), offline word-time alignment as fallback, Lua `punch_to`, REAPER pre-roll read with a fallback setting, "Punch from here", ADR; manual REAPER checklist | pending | - | 7, 9, 11; spike 1 | - |
...
- **Goal**: the sidecar reports suspected misreads, extras, skips and restarts with a measured false-flag rate.
- **Scope**: `flags.py`, tracker alignment output, `replay.py` extended to count flags per reading, contract types, regenerated `teleprompterRecording.json` via `record_mock_stream.py`, decision on normalization sharing and on snapshot retention, Go relay unchanged. A restart (`jump: "restart"`) also emits a flag of its own class (owner decision 2026-09-23). Design point for this phase: choose the findings category a restart flag maps to when Phase 7 persists it (candidate `pickup`, consistent with take review's `evidence.kind: restart` to category `pickup`, since `Validate` rejects unknown categories). Precision over recall still applies to restarts: a recovery the tracker cannot place confidently is not flagged.
- **Success signal**: replay report on the 3 existing real readings plus new ones meets the false-flag target or the class stays off by default; `extra` stays behind a toggle until it does.

**Phase 7 - Flags UI and persistence**
- **Goal**: review flags without leaving the modal, and keep them reviewable after it closes.
- **Scope**: marks (misread wavy underline, extra-word caret, skipped dotted underline; new `Highlight` kinds or a documented equivalent, ADR 0016/0017, stories, atlas with no debt growth), heard-text tooltip via `TooltipTarget`, panel with dismiss, key entries for every mark, "Punch from here" disabled until Phase 12. Depends on the delivered tooltips ([ADR 0049](../adr/0049-hints-are-base-ui-tooltips-that-meet-wcag-1-4-13-and-info-icons-are-buttons.md): a hint is hoverable, persistent and closes on Escape), adds the inline (flowing text) mode to `TooltipTarget` (a flag is a `<mark role="button">` inside running text, so the trigger must render as the mark and flow with the line; the component accessibility PRD's question 8 deferred it here because nothing else uses it), and on `palette-contrast-wcag-aa.prd.md` Phase 4 (the `Highlight` kinds and their text colours meet WCAG AA); build the flag marks on those, not on today's `Tooltip` and `Highlight`. Skipped and misread render by default; extra sits behind a toggle until Phase 6's replay rate meets the target. Persistence (owner decision 2026-09-23): each flag is written as a `transcript_discrepancy` finding (restart flags under the category Phase 6 chose) with status `unreviewed`, labelled suspected, through `apps/desktop/internal/findings` (the shared findings contract, not a parallel data model); this needs a new host binding (host API bump, contract, mock and golden payload per the wire-contract rule). When REAPER is recording concurrently the finding anchors to the take being recorded (GUIDs first, project time as fallback, per `daw-integration.md`); the anchor it carries when no take is known is a design point for this phase. Transcript Compare over the recorded take stays authoritative, and a dismiss in the panel sets the finding's status rather than deleting it.
- **Success signal**: every flag reviewable and dismissible; flags survive closing the modal as unreviewed, suspected findings; nothing edits text or audio; visual states at four viewports.
...
- **Scope**: word-to-time from live anchors first (owner decision 2026-09-23): a read-only play-position bridge command in `narration_ui_bridge.lua` with a harness test in `integrations/reaper/tests` first ([ADR 0066](../adr/0066-the-lua-bridge-is-tested-by-a-harness-under-lua-5-4-and-reaper-api-behaviour-is-checked-in-reaper.md)), polled by the host during the live session and written to the anchors file (word index, play position minus ASR latency; anchors at or after word N dropped after a punch at N). Each poll costs about 50-100 ms per call through the file bridge, so the poll rate is bounded and the anchor's accuracy is stated, not assumed; which play position it samples is spike 1. Offline alignment over the resolved range is the fallback when no anchor covers the word. Lua `punch_to` (with the tracker seeking to the same word afterwards; on an extra-word flag the target is the word before the insertion). Pre-roll: read REAPER's own pre-roll preference; how to read it is a REAPER API check run only with the owner's go-ahead (standing rule); if it cannot be read, fall back to the layered setting `Teleprompter.punch_preroll_seconds` (global and project scope, 0-10 s, default 3 s) through `fieldSchemas` at `apps/desktop/app.go:673-679`, `config/defaults.json`, `Settings.tsx`, `mockFixtures.ts`. UI showing resolved time, its source (anchor or alignment) and pre-roll before moving. ADR (cursor mutation, no undo block for a cursor-only punch as with `jump_to_compare_marker`, and the anchor source).
- **Success signal**: harness tests for the play-position and `punch_to` commands pass under `pnpm check`; manual REAPER checklist with a known click; only cursor and view change.
...
| Control channel | Sentinel file the sidecar tails (delivered in Phase 3, [ADR 0104](../adr/0104-seek-reaches-a-running-teleprompter-session-through-a-tailed-control-file.md)) | Loopback socket | Same pattern as `--stop-file` |
...
| Chapter-to-track matcher location (settled) | Go port in Phase 8 with parity cases, replacing the exact-title placeholder in `internal/evidence/mapping.go`; ADR 0100's `chapter-track-map.json` is the manual override | Python sidecar mode; wait for the DAW Project Scan matcher | Settled by the placeholder and ADR 0100; one matcher shared by every consumer |
| Closing the modal during a live session (settled) | Closing stops the session, with a "Stop and close" confirm while live (PR #352) | Keep the session running and reopen | Settled by Phase 2; no orphaned microphone capture |
| Live flag after the session | Written as `transcript_discrepancy` findings, status `unreviewed`, labelled suspected, through `internal/findings` (new host binding) | Session-only flags cleared on close | Owner decision 2026-09-23, overriding the recommendation; flags stay reviewable after the session, and Transcript Compare stays authoritative |
| Flag classes shown by default | Skipped and misread on; extra behind a toggle until its replay false-flag rate meets the target | All three on | Owner decision 2026-09-23; extra is the most speculative class |
| Restarts as flags | Restarts are flagged too, as their own class; findings category chosen in Phase 6 (candidate `pickup`) | Flag only skipped, misread and extra | Owner decision 2026-09-23, overriding the recommendation; precision over recall still applies |
| Undo block for a cursor-only punch | No undo block, as `jump_to_compare_marker`; recorded in Phase 12's ADR | Force an undo point | Owner decision 2026-09-23; moving the cursor changes no project data |
| Word-to-time source for the first punch | Live anchors first (REAPER play position polled through the file bridge during the session), offline alignment as fallback | Offline alignment only; Transcript Compare timing | Owner decision 2026-09-23, overriding the recommendation; PR #35's order |
| "Punch from here" on an extra-word flag | The word before the insertion | The word after | Owner decision 2026-09-23; the cursor lands ahead of the unwanted audio |
| Pre-roll default | Read REAPER's own pre-roll preference; if unreadable, `Teleprompter.punch_preroll_seconds` (0-10 s, default 3 s) | Fixed 2 s, 3 s or 5 s default | Owner decision 2026-09-23, overriding the 3 s recommendation; one pre-roll for REAPER and the app |
| Per-viewer reading preferences | Text size and rail state in browser storage; microphone, engine and model in global settings | Everything in global settings | Owner decision 2026-09-23; UI layout per viewer, machine facts in settings |
| Confirming a flag later | Deferred | Recheck on click; trailing confirmation pass | Owner decision 2026-09-23; revisit after Phases 6-7 measure false-flag rates |
