...
| S07 | [test-flakiness-and-visual-suite-stability](test-flakiness-and-visual-suite-stability.prd.md) (1-5) | 6 (axe) after the palette stack; 7 stays a deferred spike. Delivered before S06 (reordered: the flaky `ProjectPicker` focus test failed about 4 in 10 full-gate runs and slowed every later stack). |
...
| S05 | partial | #70 to #79 (issue #69, which stays open). Delivered: [verification-and-code-health-tooling](verification-and-code-health-tooling.prd.md) phases 1 to 8, 10 and 11: Black dropped, golangci-lint v2, the coverage ratchet ([ADR 0043](../adr/0043-coverage-is-a-ratchet-on-logic-directories-not-a-blanket-80-percent.md)), Hypothesis, fast-check and Go fuzz tests ([ADR 0044](../adr/0044-property-and-fuzz-tests-are-deterministic-in-the-gate.md)), Knip at zero ([ADR 0045](../adr/0045-dead-code-is-gated-at-zero-with-reasoned-ignores.md)), Go and Python import rules ([ADR 0046](../adr/0046-architecture-rules-taken-from-adrs-are-lint-and-test-rules.md)) and Playwright traces. Remaining, and they run when the primitives stack S10 has landed (the S13 tail): phase 9 (dependency-cruiser and ESLint rules for `primitives/` layering, ADR 0016, the Base UI import guard, its ADR) and phase 12 (aria snapshots of the Base UI dialogs, slide-overs and navigation, after phase 11 and the Base UI dialog and drawer phases). |
| S07 | partial | #81 to #85 (issue #80, which stays open). Delivered: [test-flakiness-and-visual-suite-stability](test-flakiness-and-visual-suite-stability.prd.md) phases 1 to 5: the `ProjectPicker` focus flake fixed at its cause (5 of 48 runs failed under load, 174 of 174 pass) and the full frontend suite hardened (Testing Library `asyncUtilTimeout` 5 s, `testTimeout` 15 s; 60 of 60 consecutive full runs), the visual drivers wait for their page and can be stress-tested with `UI_CPU_THROTTLE`, the kit scaffold gets `clickNav` (kit 0.3.2), a `-cpu 1,4` Go guard (`test-schedules`) and the `Elapsed` assertion fix, and `ci-and-releases.md` matches the live settings (no required status check, D11) and the job names that exist. Remaining, both tail phases: phase 6 (axe on app states) runs in the S13 tail, after the palette stack S12 and a kit slot; phase 7 (pixel baseline spike) is deferred until 50 consecutive stable CI runs after this stack merges. No new ADR. |
| S01, S06, S08-S24 | queued | |
