...
- **Delivered as**: `apps/ui/src/components/editing/{EditingCheckPanel,EditingCandidateRow,editingCheckText}.tsx` (a `SlideOver`, opened from `StageEvidence.tsx`'s new "Open editing check" - threaded beside the existing "Open recording check" and disambiguated by `stageText.ts`'s new `causeText()`/`EDITING_CAUSE_TEXT` - and from `ChapterLinksTable.tsx`'s new per-row "Editing check…"). Pure consumption throughout: no new host binding, wire schema or `hostAPIVersion` bump. Per-class state is read from `StageRecommendations()`, filtered to the three editing signal ids, and only ever shown for a chapter whose status is `editing` (D3); every other case falls back to a candidate count from `EditingCandidates` with a plain note (ADR 0266). The job has no live event yet (Phase 5's own note), so the panel polls `EditingState()` every 500 ms while running, keeping candidates listed under the progress bar rather than swapping to a `WorkDialog`. Hear reuses `useRangePlayer` (the take-review audition dialog's own hook) over a candidate's source-relative range; Accept/Dismiss/Defer call `FindingsReview` with the shown `evidence_version`, showing the ADR 0120 stale-evidence message on refusal; Go to/Loop/Add marker in REAPER reuse `ReaperControls.tsx` verbatim. Candidates are ordered earliest-first within a fixed class order (silence, click, breath). The mock (`stagesMock.ts`, `editingMock.ts`, `mockApi.ts`) learned the same D3 gate and a shared-findings-store `editingCandidates` (so a seeded candidate is reviewable the same way a Review-page finding is) without changing any pre-existing test's behavior. Nine catalog states across `home`/`tracks` (`editing-check-{never-checked,running,partial,complete-candidates,complete-clean,stale,settings-unset,unsupported,unmapped}`) were driven and screenshotted; this sandbox's Chromium-version mismatch (a pinned-vs-preinstalled headless-shell revision gap) blocked the automated Playwright run's console-error gate on an unrelated pre-existing `/favicon.ico` 404 (reproduced on the untouched baseline `home / default` state too), so verification was done by direct Playwright scripting against the same mock build instead, reviewing all 9 states plus tablet/small-desktop widths (no sideways overflow) by eye. A guide section, not a new page: `tracks.md#editing-check`, cross-linked from `home.md`'s Stage suggestions section (whose own stale "Editing and Proofing have no check yet" text this phase also corrects); no new screenshots were embedded, for the same environment reason. `pnpm --dir apps/ui test` (2656 tests), `tsc --noEmit`, `eslint` and `prettier --check` are green; no new primitive, so no atlas run was needed.
