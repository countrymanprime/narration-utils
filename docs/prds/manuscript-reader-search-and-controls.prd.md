...
- **Search runs per keystroke in Go over the whole file.** `SearchBar.tsx:13` fires `onChange` on every keystroke into `runSearch` (`shared/ui/src/components/manuscript/Manuscript.tsx:221-234`), then `manuscriptSearch` (`wailsClient.ts:132`, `shell/bindings.go:345-347`) to `Service.Search` (`shell/internal/manuscript/reader.go:59-78`). `Search` calls `Load()` (`service.go:302-321`), which re-reads and unmarshals `manuscript.json` every time, lower-cases every paragraph, and returns every match with the whole paragraph as `excerpt` (`reader.go:71-74`): no cap, no offset, no title or note search. No debounce hook exists (`src/hooks/` holds only `useTextSelection`). Latency is unmeasured (`interaction-feedback-audit.prd.md:40,67`).
...
- **Contents on the page.** The reader renders every chapter (`Manuscript.tsx:335`); Contents is `reference` (`shell/internal/importer/model.go:94`), which `isListableChapter` hides only in the panel (`state.ts:7`, `ChapterNav.tsx:38`). The page can open on it (`Manuscript.tsx:101`), "Expand all" loads it (`:317`), and `#p` links resolve against unfiltered chapters (`:188`). [ADR 0005](../adr/0005-reference-material-excluded-from-chapter-nav.md) says the continuous reader deliberately does not filter and the backend never filters; hiding Contents in the reader reverses that half and needs a superseding ADR. Totals already exclude it (`AudiobookEstimatePanel.tsx:63`, `shell/app.go:596`).
...
- Controls bar depends on a shared `IconButton` (see [ui-primitives-and-headless-library.prd.md](ui-primitives-and-headless-library.prd.md) Phase 1); until it exists, the bar can reuse the pasted class string.
