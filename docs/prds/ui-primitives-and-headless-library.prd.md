...
| 4 | Menus and disclosure | `DropdownMenu` (`Menu`), `Disclosure` (`Collapsible`) and `Switch` were delivered by S10a; **4a** `Tabs` and `ToggleGroup` ([ADR 0054](../adr/0054-tabs-and-toggle-groups-name-and-link-what-the-hand-built-strips-left-anonymous.md)); **4b** `TagInput`. `Popover` is not built (no caller) | complete: 4a and 4b ([ADR 0055](../adr/0055-the-tag-input-is-a-wrapped-text-field-and-chips-that-report-and-never-edit-the-list.md)) | 5 | 2 | - |
| 5 | Tables and combobox | Presentational `Table`, keyboard rows, `aria-sort`; supersede the ADR 0009 table exception ([ADR 0056](../adr/0056-a-presentational-table-primitive-replaces-the-dtable-class-and-rows-take-the-keyboard.md)). The alias combobox stays bespoke (S10a): only its input is wrapped | complete | 4 | 1 | - |
