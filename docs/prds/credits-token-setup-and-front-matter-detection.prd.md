...
**Status (2026-09-26):** in delivery (lane train, [implementation plan](implementation-plan.md) section 8). The open questions take the owner's answers (D34, D35) and otherwise the recommendations (D39; Decisions Log). Phases 1-3 are built (Phase 2's host half by lane A, its dialog and Phase 3 by lane C's stream C4, #587/#588). Phase 4 is blocked on lane B (`internal/importer/**`); see the phase table.
...
| 3 | Entry points | Banner on Home and above the Manuscript credits card, "Fill in" on `CreditsEntry`, ask again after Replace manuscript; steady-state docs and the credits PRD amendment | complete (lane C, stream C4, #588: `CreditsSetupBanner`, the "Fill in" action on `CreditsEntry`) - this row was stale; corrected 2026-09-26, no code changed by this correction | - | 2 | - |
| 4 | Import-time metadata (Could) | Markdown YAML front matter and DOCX `Title`/`Subtitle` styles kept as `sourceMetadata` at import, fed into Detect | blocked (2026-09-26): every source of this phase's YAML/style parsing lives in `internal/importer/{markdown,docx,model}.go`, which is lane B's file ownership under the delivery train's lane split (D41, `implementation-plan.md` section 8); lane A owns `internal/manuscript` and `internal/credits`, which only *consume* a `sourceMetadata` block once the importer produces one. Building the consuming half first, with nothing populating it, would be speculative dead code, so nothing was changed here. Recommend a lane B stream pick this phase up | 3 | 1; CS10 | - |
