...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Detection | Fix the `storedPath` join; the front matter parser; EPUB OPF metadata; DOCX property filtering and CS6 precedence; `detected` in `CreditsProjectValues` with source captions in Settings > Credits | partial — pending owner: the host-side detector, the `storedPath` fix, and the additive `detected` field are done and wire-tested; the Settings > Credits source-caption UI and its visual state are left to a UI stream | - | CS3, CS5, CS6, CS8 | - |
