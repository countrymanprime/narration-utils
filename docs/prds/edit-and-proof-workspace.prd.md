...
| 2 | Workspace MVP: listen, follow, flags, click | Route and entry points, header, `useChapterPlayback` (and Tracks' player on it), highlight, auto-scroll, flags, click to seek, keyboard, speed; visual states, aria, guide | complete: lane C (C8), `/tracks/chapter/:chapterId` (the app's first parameterised route), entry links from Tracks' Chapter links table and Home's chapter row, `useChapterPlayback` and the shared `buildPlaylist` (`useTrackPlayback` refactored onto it, fixing its trim bug), karaoke highlight (binary search per item, ADR 0265), auto-scroll with scroll-lock, inline flags with a legend and next/previous (Go to/Loop and flag review are Phases 3 and 4, not shown rather than shown disabled), click-to-seek, keyboard (Space, arrow keys, `[`/`]`), speed (EP17); `state-catalog.ts`/`app.drivers.ts` rows (never, stale, current, playing, flag-selected, standalone) and the guide page. No new dialog or slide-over, so no new aria snapshot. **Known gap:** EP3 B ("Align again") has no host binding yet (only the sidecar's `--align-only` flag, from Phase 1); the header offers Check again in its place | - | 1; EP1, EP2, EP5, EP14, EP17 | - |
