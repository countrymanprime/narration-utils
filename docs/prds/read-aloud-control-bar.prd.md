...
| 6 | REAPER transport state | Armed-track fields on the shared read-only `chapter_track_state` (built here to the sibling's spec if its Phase 4 has not landed), harness tests first; `ReadAloudReaperState`; wire rows; toggle state shown read-only in the bar | partial — the command and its armed fields came with stream B1 ([ADR 0231](../adr/0231-chapter-track-state-is-one-read-only-answer-of-the-transport-the-arms-the-input-device-and-one-tracks-items.md)), plus a harness test for nothing armed and only the named track armed; `ReadAloudReaperState` (host API 56), its contract, golden and mock (`?mockReaperState=`) delivered ([ADR 0249](../adr/0249-read-aloud-asks-reaper-once-whether-the-chapters-linked-track-is-the-one-track-armed.md)); the state in the bar is the UI lane's (stream C5); pending owner: the scripted REAPER run for the `GetPlayState` and `I_RECARM` reads, and switching the command on after the verification pass (#510) | 1, 2, 4 | The sibling's Phase 4 (soft); Integration Phase 11 | - |
