...
| 4 | Input level | Sidecar `level` events and `--meter` mode; Go relay and meter child; bindings; wire contract, golden, mock; meter in the bar and popover | complete — sidecar, host, bindings (host API 54), wire contract and mock ([ADR 0247](../adr/0247-the-input-level-comes-from-the-sidecars-own-capture-and-a-model-free-meter-child-runs-before-start.md)); `useInputLevel` and `InputLevelMeter` (stream C5) delivered the bar's decorative meter and the popover's `role="meter"`; pending owner: the meter moves with their own microphone, and the reader does not stutter at ten level events a second (#510) | 1, 2, 6 | Q6 | - |
| 5 | Pause | Control-channel pause/resume, frozen tracker clock, `paused` state, binding, contract, mock; Pause in the bar | complete — the sidecar's pause and resume, the frozen clock, the host's `paused` flag and auto-stop rule, `TeleprompterPause` (host API 55), the contract and the mock ([ADR 0248](../adr/0248-pause-is-a-pause-and-resume-pair-on-the-control-file-and-a-paused-session-stays-running.md)); stream C5 delivered the bar's Play/Pause toggle (`aria-pressed`) and moved the Space shortcut from Play/Stop to Play/Pause | 6 | 3, Q3 | - |
| 6 | REAPER transport state | Armed-track fields on the shared read-only `chapter_track_state` (built here to the sibling's spec if its Phase 4 has not landed), harness tests first; `ReadAloudReaperState`; wire rows; toggle state shown read-only in the bar | complete — the command and its armed fields came with stream B1 ([ADR 0231](../adr/0231-chapter-track-state-is-one-read-only-answer-of-the-transport-the-arms-the-input-device-and-one-tracks-items.md)), plus a harness test for nothing armed and only the named track armed; `ReadAloudReaperState` (host API 56), its contract, golden and mock (`?mockReaperState=`) ([ADR 0249](../adr/0249-read-aloud-asks-reaper-once-whether-the-chapters-linked-track-is-the-one-track-armed.md)); stream C5 delivered `useReadAloudReaperState` and the bar's always-disabled REAPER indicator with its own Refresh; pending owner: the scripted REAPER run for the `GetPlayState` and `I_RECARM` reads, and switching the command on after the verification pass (#510) | 1, 2, 4 | The sibling's Phase 4 (soft); Integration Phase 11 | - |
| 7 | Record in REAPER | `record_start`, `record_stop`, `arm_only` with harness tests first; scripted REAPER run on a copy; bindings; first-time confirm; per-project toggle; threat model, `SECURITY.md`, bridge docs, ADR; owner-pending real take | pending — no lane has queued the Lua/Go half yet (`record_start`, `record_stop`, `arm_only`, the bridge client and bindings do not exist), so stream C5 (UI, lane C, no Go/Lua) built nothing here: an actionable toggle would have nowhere to send its click. Phase 6's read-only indicator covers everything a fake-state reading can show today | - | 3, 5 (for Pause's REAPER behaviour), 6, Q7, Q8, Q9 | - |
...
| Which track counts for the REAPER state (Phase 6) | The chapter's one confirmed link; no link or several answer `no_link` without asking REAPER | The chapter-to-track name match | Q7's guard must never pick a track by name ([ADR 0249](../adr/0249-read-aloud-asks-reaper-once-whether-the-chapters-linked-track-is-the-one-track-armed.md)) |
