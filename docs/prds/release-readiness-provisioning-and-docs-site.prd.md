...
- [ ] **11. Code signing.** Options: (a) ship unsigned RCs with documented SmartScreen and Gatekeeper warnings and checksums (today). (b) Sign Windows with an Authenticode certificate or a signing service (candidates, cost and free open-source programs: TBD - needs research). (c) Sign Windows and notarize macOS. Recommendation: decide before the first stable; the user owns certificates, costs and secrets, so no agent adds signing steps unasked. Until decided, keep RCs unsigned and say so in release notes.
