...
| 3 | `apps/desktop/app.go` `Host` struct and `configureLocked`, `apps/desktop/bindings.go`, new contract and adapter | Every binding-adding session; the pattern owned by `host-binding-data-race.prd.md` |
