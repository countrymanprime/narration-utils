...
| 8 | Packaged-app smoke test | Non-interactive smoke mode, CI step on the Windows package, verify-installable extended | complete (S16; no ADR, `hostAPIVersion` stays 12. Evidence: `narration-utils --smoke` (`apps/desktop/smoke.go`) unpacks the embedded resources, starts the three frozen sidecars, runs the frozen guide's new `self-check` (CMU dictionary and Piper's espeak-ng data), loads the three catalogs, writes the asset cache and checks the REAPER package, and exits 0/1/2; unit tests break one thing at a time (a missing sidecar, a sidecar that will not start, a guide that lost its data, a missing or empty catalog, a missing launcher, an unwritable cache, a hung sidecar) and each fails by name; a full local `wails build` of the executable passed `--smoke` (9 checks, about 5 s), and failed with exit code 1 naming the sidecar when one was deleted from its unpacked resources. The CI step is in `.github/actions/build-native` (Windows only, before packaging, so `Build (Windows)` and the release build both run it); its first CI run, the `Build (Windows)` job of PR #222 on a real Windows runner, passed all 9 checks. macOS, Linux and a clean machine are not covered) | 5, 6, 12 | 7 | [plan](implementation-plan.md) (S16) |
...
- **Status (S16, 2026-09-21)**: built and produced in CI, not yet run on a clean machine. Facts found: nothing passed `-nsis` in CI and no release ever held an installer; the hosted runner has no `makensis` (Wails only warns and skips, so the build now fails when `-nsis` is requested and none is made); the pinned `choco install nsis --version=3.11.0` step installed `makensis v3.11` and Wails built `narration-utils-windows-x64-setup.exe` (196,676,207 bytes; the zip is 195,290,110) in about a minute; the embedded WebView2 bootstrapper checked out as validly signed by Microsoft Corporation. Decisions are in [ADR 0082](../adr/0082-windows-installs-per-user-from-an-nsis-setup-program-that-wails-builds-and-the-release-carries-beside-the-update-zip.md) (per user, so the in-app updater can replace the program; the uninstaller leaves settings, the asset cache and project folders). **Owner steps left** (phase 16 checklist): run the setup program on a clean Windows machine (SmartScreen "More info", install, Start Menu and desktop shortcut, launch, update panel says it can install, uninstall leaves `%APPDATA%
arration-utils`, `%LOCALAPPDATA%
arration-utils` and project folders), and promote a candidate that has the setup program.
