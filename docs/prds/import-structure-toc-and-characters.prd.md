...
**Source:** user requests of 2026-09-20 (items 3 importer side, 18, 19, and the Characters observations). Citations are `file:line` on branch `claude/narration-utils-planning-00e3c8` at dc9d01a. Nothing here is built yet. Related: [the import review](../architecture/import-review.md) (the dialog; its PRD is delivered and deleted), manuscript-reader-search-and-controls Phase 5 (hiding the result in the reader; delivered, PRD deleted, see [the manuscript guide](../guides/using-the-app/manuscript.md) and [ADR 0090](../adr/0090-the-page-flip-reader-hides-reference-chapters-superseding-the-reader-half-of-adr-0005.md)), the Story Bible entries properties schema (delivered, stack S19a, PRD deleted; see [Manuscript Guide](../utilities/manuscript-guide.md)'s "Entity properties" section for the `create --properties` contract this PRD's Phase 3 must keep stable).
...
| 2 | Contents and Characters in the reader | Decide S1/S2, ADR, coordinate the reader filter | complete | - | 1; reader PRD Phase 5 | - |
...
| 4 | TOC as the chapter list | docx TOC reader and matcher, fallback, Markdown links, notices | partial | - | 1 | - |
...
**Phase 2 - Contents and Characters in the reader.** Goal: neither appears as a readable chapter. Scope: the decision, ADR, and the reader PRD's helper. Success: reader and totals verified on a fixture with a TOC and cast. **Status: complete.** The decision (S1/S2) is settled and recorded in [ADR 0088](../adr/0088-contents-and-characters-stay-reference-hidden-by-the-reader.md); the storage-side classification was already correct (pinned by a new regression test) and needed no code change. The read-through reader view (`Manuscript.tsx`) now hides both too: manuscript-reader-search-and-controls Phase 5 (stack S19b, delivered, PRD deleted) landed and reuses the same `isListableChapter` predicate for the render loop, the default chapter and "Expand all", with a hidden-target deep link becoming a no-op message instead of a jump ([ADR 0090](../adr/0090-the-page-flip-reader-hides-reference-chapters-superseding-the-reader-half-of-adr-0005.md), which supersedes the reader-view clause of ADR 0005).
...
**Phase 4 - TOC as the chapter list.** Goal: the manuscript's own list of chapters. Scope: docx first. Success: proposed chapters equal TOC entries on a TOC fixture; fallback and notice tested. **Status: partial.** Delivered: docx TOC reading (a `"TOC N"`-styled paragraph or a `w:hyperlink` whose anchor cites a `"_Toc"` bookmark Word opens around its target heading - a hand-built OOXML fixture with real bookmarks and an inert field wrapper, per ADR 0089), matching by bookmark then normalised text then position, an 80% match-fraction threshold before the TOC overrides the heading heuristic (only `chapterTitles` changes; `Sections`/grouping are untouched), a "listed N, matched M" notice whenever they differ, and the markdown glued-heading split notice (`markdown.go` previously discarded the flag `docx.go` already reported, S9). **Not delivered, deferred with a follow-up issue:** Markdown TOC recognition (a `[text](#slug)` link list matched to heading slugs) - PRD priority "Should", not "Must"; markdown chapters today come only from its own heading structure. No real user manuscript with a TOC field was available in this run, so verification is against the hand-built fixture only - recorded as **awaiting a real manuscript** (`implementation-plan.md` section 2).
