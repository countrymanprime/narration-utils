...
| 3 | Tooltip, Field, MeterBar | Tooltip family and info icon, `Field`, MeterBar semantics; absorbs the a11y PRD's mechanism (Phases 1 and 3 for Field). Delivered in two pull requests: 3a Tooltip and info icon ([ADR 0049](../adr/0049-hints-are-base-ui-tooltips-that-meet-wcag-1-4-13-and-info-icons-are-buttons.md)), 3b `Field` and `MeterBar` | 3a complete, 3b pending | 2, 5 | 1 | - |
