...
- [x] **Q7. Do Manuscript Guide findings duplicate the Story Bible's review state?** Which conditions become findings is undefined in the docs beyond "unresolved entities and unreviewed pronunciations" (`manuscript-guide.md:31`). Options: (A) findings for entities with `review_state == "needs review"` and for unlocked entities whose pronunciation confidence is `low` or `unknown`; a finding decision never mutates the Story Bible, and the finding resolves itself ("resolved upstream") when the entity becomes reviewed or locked; (B) accept/dismiss route through the Story Bible edit path so there is one state; (C) defer the Guide adapter. Recommendation: A, decided and delivered in Phase 3 (`apps/desktop/internal/guide/findings_adapter.go`). It leaves `edit()` and its lock guard untouched (ADR 0007, ADR 0018) and avoids dismiss meaning "delete an entity"; `Service.SaveFindings` calls only `Entities()` and `findings.Store`, never a mutating sidecar command, proven by a test that configures the service with a nonexistent Python executable and asserts `SaveFindings` still succeeds. "Resolved upstream" reuses Phase 1's merge semantics unchanged: an entity absent from the fresh set (because it became reviewed or locked) is carried forward `NotInLatestRun` rather than deleted, so it drops out of the default (open) `List` on its own. How many findings A produces on a real manuscript is still unknown - this phase had no real manuscript to measure against, only fixtures; TBD, carried forward. ADR 0020's precision-first extraction should keep it small.
...
| 3 | Manuscript Guide to findings | Go adapter over `manuscript_guide.json`; entity and pronunciation conditions per Q7; resolved-upstream handling | complete | 2, 4, 6 | 1 | - |
...
**Phase 3 - Manuscript Guide to findings** — delivered.
...
- **Delivered as**: `BuildFindings(entities, project)` in `apps/desktop/internal/guide/findings_adapter.go` - a pure function (no `Service` reference, so it structurally cannot reach a mutating sidecar command) producing one `CategoryEntity` finding per entity with `review_state == "needs review"` and one `CategoryPronunciation` finding per unlocked entity whose pronunciation confidence is `low` or `unknown` (an entity meeting both gets two findings with distinct ids, so each resolves independently). `Service.SetFindings`/`SaveFindings` follow the same opt-in shape as `transcript.Service` (Phase 2): `SaveFindings` reads `Entities()` and calls `findings.Store.SaveAnalyzerFindings("story-bible", "entities", ...)` once per run (a single scope, matching the Story Bible's one-file-per-book shape, unlike Transcript Compare's per-chapter partitions), and is wired into `Build()`'s success path in `app.go`. "Resolved upstream" needed no new store mechanism: it is Phase 1's existing merge (an id absent from the fresh set is carried forward `NotInLatestRun` rather than deleted), proven here by `TestServiceSaveFindingsResolvesUpstreamWhenEntityBecomesReviewed` and `...BecomesLocked`. `TestSaveFindingsNeverInvokesTheSidecarPythonExecutable` configures the service with a nonexistent Python path and asserts `SaveFindings` still succeeds, proving the adapter path never shells out. No real manuscript was available to this phase (an isolated worktree, no owner present), so the finding count Q7's recommendation asks to measure is not recorded here; TBD, carried forward - only synthetic fixtures were used. `go build`, `go vet`, and `go test ./...` on `apps/desktop` are green; `internal/guide` coverage is 82.5%. The UI package is untouched by this phase (no `contracts/findings.ts` consumer yet - that is Phase 4), so no Playwright/atlas run was needed, and no `hostAPIVersion` bump: no binding was added.
...
| Manuscript Guide findings conditions (decided, Q7) | Findings for needs-review entities, plus unlocked low/unknown-confidence pronunciations; adapter never mutates the Story Bible | Route decisions through `edit()`; defer the adapter | Keeps `edit()`'s lock guard untouched; "resolved upstream" reuses Phase 1's merge, no new mechanism |
