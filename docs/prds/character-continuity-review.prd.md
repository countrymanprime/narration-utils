...
| 2 | Stable characters, dialogue cues, appearance maps | Manuscript Guide: stable ids, rules-based cues with `unknown`, appearance maps, corrections that survive rebuild; Go and TS normalization | blocked (see note below) | 1, 3 | - | - |
| 3 | Region listing and reference approvals | Go: list project regions with identity, approve and revoke per character, sidecar with snapshot and metadata, no analysis | complete | 1, 2, 4 | RD-1 | - |
