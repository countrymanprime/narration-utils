...
| 2a | Text ramp: tokens, primitives, shell | `--non-text`; light `--text-muted` `#625e52`; the `--text-faint` usage ratchet; migrate the primitives, the app shell and `components.css`; delete the `WorkDialog` and `MeterBar` debt entries | pending | No | 1 | - |
| 2b | Text ramp: Home, project picker, Settings, Tracks | Triage and migrate `--text-faint` in those pages | pending | No | 2a | - |
...
| Derived on-tint text, 50% toward `--text` (decided, D5) | Colour-mix option A | Neutral text; fixed tokens | Keeps ADR 0016's coloured text; user colours are not guaranteed (Q4 a) |
