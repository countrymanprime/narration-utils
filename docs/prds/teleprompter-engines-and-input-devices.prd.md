...
- [x] **Where the device, engine and model choices are stored.** Options: (a) global settings file (`%APPDATA%/narration-utils/global-settings.json`) under a new `Teleprompter` section, global scope only; (b) browser storage as the device is today; (c) project settings. Recommendation: (a). These are machine facts (hardware and CPU), Go can validate them, and they survive webview data resets. Migrate the existing browser-storage device once. Purely visual preferences stay in browser storage (the rule PRD 1 uses). This needs a free-text or device kind with validation added to `saveSettings`. **Resolved (Phase 2, 2026-09-22):** (a), for the device only - the `Teleprompter` schema's `input_device` field reuses the existing "text" kind (any string, including empty), global scope only (`saveSettings` rejects a project-scoped write). The pre-Phase-2 browser-storage value is migrated once, on first load, only when the settings value has never been set. Engine and model choices stay out of scope for this phase (phase 3's own "Teleprompter settings section"). **Resolved (Phase 3, 2026-09-22):** (a) extended to `engine` and `model`, both new "choice" kind fields on the same global-only `Teleprompter` schema, with repo defaults (`whisper`, `tiny`) in `config/defaults.json` and `builtinDefaults`. A Settings > Teleprompter category (global scope only) now shows all three fields; the Teleprompter page reads the settings default model on load.
- [x] **Model choices exposed per engine.** Options: (a) Whisper tiny and small (the page offers both today; only tiny has live-lag data) and Moonshine tiny and small (both measured in the spike); (b) every catalog Whisper model plus Moonshine medium. Recommendation: (a). Larger models have no live-latency data, and the Phase 8 protocol adds Whisper small's lag. **Resolved (Phase 3, 2026-09-22):** (a), the Whisper half - the `Teleprompter.model` choice field offers `tiny` and `small` only. The engine choice itself stays limited to `whisper` until Phase 7; Moonshine's `tiny`/`small` model choices appear once Phase 6/7 provisions and wires the engine.
...
- [x] **Shape of the model-management bindings.** Options: (a) copy the five `Whisper*` bindings as `Moonshine*`; (b) add engine-aware `LiveModel*` bindings (catalog, install, install state, cancel, remove keyed by engine plus id) backed by both catalogs, leaving `Whisper*` for Transcript Compare; (c) parameterize `Whisper*`, a breaking change. Recommendation: (b) *(superseded - see Resolved)*. Ids collide across engines (`tiny`, `small`), the existing `WhisperCatalog` returns Transcript Compare's selected model (`apps/ui/src/api/contracts/whisper.ts`), and one UI hook then serves both engines. Each new binding bumps the host API version. **Resolved (Phase 5, 2026-09-22):** none of the three - `release-readiness-provisioning-and-docs-site.prd.md` Phase 3 (the provider registry and six generic `Assets*` bindings, [ADR 0079](../adr/0079-every-downloadable-asset-is-listed-installed-verified-and-removed-through-one-registry-of-providers.md)) landed before this phase, exactly as this PRD's own Phase 5 "Depends (asset registration)" note anticipated. Moonshine is a fourth `assetProvider` (kind `moonshine`) in that registry: no `Moonshine*` or `LiveModel*` binding family, no `hostAPIVersion` bump, no id collision (the install directory is keyed by each catalog entry's own `provider` field, not the kind). The id-collision problem this question raised is solved the same way. See the Decisions Log row below.
...
- [x] **Sharing the install-poll loop.** Options: (a) a `useAssetInstall` hook used by Transcript Compare, the Teleprompter and later Settings; (b) host-side events instead of polling; (c) a shared component. Recommendation: (a). Note the behavior change: Transcript Compare would gain the Teleprompter's unmount guard, so leaving Proofing mid-download no longer auto-starts a compare run; confirm that is wanted. **Resolved (delivered by `release-readiness-provisioning-and-docs-site.prd.md` Phase 1, owner decision D4 "release-readiness Phase 1 owns the shared install-poll hook", verified 2026-09-22):** (a), `apps/ui/src/hooks/useAssetInstall.ts`; both `Transcript.tsx` and `TeleprompterPage.tsx` already use it, and `LocalAssetRow.tsx`, `Guide.tsx` and `GuideDetail.tsx` do too. This PRD's own Phase 4 is reconciled to `complete (delivered elsewhere)` rather than rebuilt.
...
| 3 | Teleprompter settings section | `Teleprompter` schema (engine list limited to Whisper until phase 7), model and device, defaults, Settings category, mock, mobile layout | complete | 5, 9, 10 | 2 | - |
| 4 | Shared asset-install hook | `useAssetInstall` extracted from Transcript Compare and the Teleprompter; unmount behavior unified | complete (delivered elsewhere) | 1, 5, 9, 10 | - | - |
| 5 | Moonshine catalog and provisioning | `moonshine-assets.json`, manager, install/state/cancel/remove, engine-aware bindings, dependency record completed | complete | 1, 2, 3, 4, 9, 10 | release-readiness Phase 3 (soft: aggregated catalog) | - |
...
- **Done (2026-09-22, reconciled by the phase 5 PR, no code changed here):** this PRD's own recommendation for the Open Question "Sharing the install-poll loop" was option (a), `useAssetInstall`. `release-readiness-provisioning-and-docs-site.prd.md` Phase 1 delivered exactly that hook first (owner decision D4: "release-readiness Phase 1 owns the shared install-poll hook") - `apps/ui/src/hooks/useAssetInstall.ts`, with `Transcript.tsx` and `TeleprompterPage.tsx` both already migrated onto it (verified in the codebase, not re-built here). Phase 5 depends on this phase; it is satisfied, so phase 5 could proceed without rebuilding a duplicate hook.
...
- **Done (2026-09-22):** `release-readiness-provisioning-and-docs-site.prd.md` Phase 3 (the provider registry, ADR 0079) is delivered (stack S16), so this phase used it rather than a third bespoke binding family - the "Shape of the model-management bindings" Open Question below is resolved by that fact, not by its own pre-registry recommendation (b) (see the Decisions Log row). `internal/moonshine` mirrors `internal/whisper`'s `Manager` exactly and is registered as a fourth `assetProvider` (kind `moonshine`) in the existing registry: no new `Assets*` binding, and **no `hostAPIVersion` bump** (stayed 14) because the six generic bindings already take `kind` as a free string. `config/moonshine-assets.json` holds the English `tiny` and `small` streaming models (word-timestamp variant, 9 files each, all flat - no `assets` subdirectory support was needed after all); every URL, size and SHA-256 was independently verified 2026-09-22 by downloading and hashing every file once (not fabricated or copied from Moonshine's own CRC32C manifest), and MIT licensing (code and English streaming models) was re-verified at the pinned artifact for D17. `docs/research/local-dependency-evaluation.md` entry 10 and `first-use-dependency-provisioning.md` are updated. Ids (`tiny`, `small`) collide in name with the Whisper catalog; the install directory never collides because it is keyed by each catalog entry's own `provider` field. Tests mirror `whisper/catalog_test.go` plus a hash-mismatch-is-never-launchable case and a real-catalog load assertion. Not built here (phase 6 and 7): `moonshine-voice` is still not pinned in `pyproject.toml`, the sidecar does not yet accept `--model-dir` from this catalog, and the live Teleprompter cannot launch Moonshine - the models are only installable from Settings > Local assets so far.
...
| 5 | new `config/moonshine-assets.json`, new `apps/desktop/internal/moonshine`, `apps/desktop/{assetproviders.go,assetregistry.go,assetcache.go,installjobs.go,jobs.go,smoke.go,layout.go,assets/cache.go,cmd/seed-assets/*}` and their tests, `docs/research/local-dependency-evaluation.md`, `docs/architecture/first-use-dependency-provisioning.md` | ADR and catalog naming; release-readiness Phase 3 (aggregated catalog: registered Moonshine there rather than before it). **As actually landed:** no `hostAPIVersion` bump, no `hostApi.ts`/Wails binding/contract change - the existing generic `Assets*` bindings and `kind: z.string()` schemas already cover a new provider kind, so `apps/ui` was untouched by this phase |
...
| Model-management binding shape (Phase 5, 2026-09-22) | Register Moonshine as a fourth `assetProvider` in the existing asset registry (`release-readiness-provisioning-and-docs-site.prd.md` Phase 3, ADR 0079); no new binding family, no `hostAPIVersion` bump | The PRD's own pre-registry recommendation, engine-aware `LiveModel*` bindings; copying `Whisper*` as `Moonshine*`; parameterizing `Whisper*` | Phase 3 of the release-readiness stack landed first, exactly as this PRD's own Phase 5 "Depends" note anticipated; the registry already generalizes over `kind` as a free string, so a fourth provider needs no binding surface change and the Whisper/Moonshine id collision (`tiny`, `small`) is already solved by keying the install directory on each catalog entry's own `provider` field |
| Moonshine dependency licence re-verified at the pinned artifact (Phase 5, 2026-09-22, owner decision D17) | MIT confirmed both on PyPI's release metadata and by reading the repository's `LICENSE` file directly (GitHub's license detector reports the repository as unrecognized, so the file itself was read); the English streaming models this catalog uses are explicitly covered, AGPL-3.0-or-later compatible | Trusting GitHub's detector alone; trusting the PRD's earlier "per docs" note without re-checking | D17 requires re-verifying, not assuming, a new dependency's licence before adding it |
| Settings home, packaging shape, auto-stop trigger, scroll behavior | Recommendations under Open Questions | See there | Pending the user's answers |
