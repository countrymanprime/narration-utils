...
- **Settings.** Add a `Teleprompter` tool to `fieldSchemas` (`apps/desktop/app.go:673-679`) with `engine` (choice), `model` (choice per engine), `input_device` (new validated text kind), defaults in `config/defaults.json`, a global-only category in `Settings.tsx` (categories at `Settings.tsx:12-22`), and matching `mockFixtures.ts` entries (`mockFixtures.ts:501` pattern). Land after `settings-mobile-layout.prd.md` Phase 1 or record the inherited settings-layout defect for the new selects.
