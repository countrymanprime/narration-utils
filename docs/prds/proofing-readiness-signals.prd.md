...
- **No pickup producer or state exists.** `pickup` and `duplicate_read` are only enum values (`apps/desktop/internal/findings/findings.go:30-31`, listed at `:45`). Nothing emits them. The bridge commands are `prepare_compare`, `inspect_compare_results`, `export_compare_markers`, `jump_to_compare_marker`, `stamp_item_lines`, `read_line_ids`, `create_chapter_regions`, `close` (per the take-review PRD's check of `integrations/reaper/narration_ui_bridge.lua:525-553`).
- **Transcript Compare rows are the nearest thing, and have no "resolved".** `Discrepancy` (`apps/ui/src/api/contracts/transcript.ts:3-19`) is built at `apps/desktop/internal/transcript/service.go:314-321`. `markerState` is `pending | existing | exported` and only records whether a take marker exists near that position in REAPER (`narration_ui_bridge.lua:178-187`, set at `:216` and `:279-281`); an exported row is still a discrepancy. A row disappears only when the narrator re-records and re-runs. Row ids are `<item_index>@<srcpos>` (`:214`), so they change with the item selection.
...
- **"Clean" means one thing today.** The results page says "No discrepancies found." when `rows.length === 0` (`apps/ui/src/components/proofing/Results.tsx:90-93`). Home shows `lastCompleted.rows.length` as a raw discrepancy count, including exported rows and regardless of chapter (`apps/ui/src/components/home/Home.tsx:339-366`, count at `:356`).
...
- **Delivery checks exist only as a library nothing calls.** `measure.Profile` limits four metrics with inclusive `Limit{Min,Max}` bounds (`apps/desktop/internal/measure/profile.go:13-18`, `:23-29`); no profile ships. `Evaluate` returns findings only for limited metrics: an `out_of_range` error, or an `unavailable` info finding when the value is nil, NaN or infinite (`:42-64`, `:90-95`). A compliant report and a report that never ran both produce an empty slice. Findings carry `Source.File` only (`:106`), no chapter or track. `Report` has values but no clipping count or windowed series (`apps/desktop/internal/measure/measure.go:32-49`), and only mono and stereo WAV is read (`apps/desktop/internal/measure/wav.go:173`), so an MP3 render cannot be measured.
- **No chapter-to-audio association exists.** `tracks.Item` has no GUID, `SOFFS`, `PLAYRATE` or take list (`apps/desktop/internal/tracks/tracks.go:18-26`); `parseItem` reads `POSITION`, `LENGTH`, `NAME` and the first `<SOURCE>` (`apps/desktop/internal/tracks/parse.go:68-96`). Nothing records which rendered file belongs to which chapter. Chapter ids are positional `c-%04d` and reset with a new `documentId` on re-import; `resetDerived` deletes derived data (`apps/desktop/internal/manuscript/service.go:403`, `:434-441`).
- **Chapter status is a bare string per chapter id**, changed by a `<select>` in the Home breakdown table (`apps/ui/src/components/home/AudiobookEstimatePanel.tsx:188-211`) through `ManuscriptSetChapterStatus` (`apps/desktop/bindings.go:348`); the enum is `not_started | recording | editing | proofing | finalized` (`apps/ui/src/api/contracts/manuscript.ts:1`, `apps/desktop/internal/manuscript/reader.go:303`).
- **Proofing docs and states cover only the compare flow.** `docs/guides/using-the-app/proofing.md` documents setup and results; the visual catalog has Proofing states `setup-default`, `setup-alt-selection`, `running`, `results-row-expanded`, the EXTRA row and `toast` (`apps/ui/tests/visual/state-catalog.ts:66-81`).
- **Host API version** is `5` in three places: `apps/desktop/app.go:33`, `apps/desktop/app_test.go:39-42`, `apps/ui/src/hostApi.ts:2`.
...
Every phase follows the `CLAUDE.md` workflow: plan (find or open the tracking issue first and put `Closes #<n>` in the PR, `docs/operations/github-workflow.md`), `change-impact-scan` (this PRD touches `apps/desktop/internal/tracks`, `apps/desktop/internal/transcript` and shared UI), TDD (80% coverage on new Go), `full-verification-gate` (`pnpm check`, not `check:fast`), `design-spec-guard` when `primitives/` or `styles.css` change, `feature-cleanup`. UI phases run the Playwright visual suite for the Proofing page and view every PNG at `desktop`, `small-desktop`, `tablet` and `mobile`, plus the atlas when a primitive changes. Phases that change bindings bump the host API version in the three places and regenerate `apps/ui/wailsjs/go/main/Host.{js,d.ts}`. There is no Lua in this PRD, so no manual REAPER checklist; the user supplies REAPER-saved `.rpp` fixtures (TBD) for Phases 2 and 3. Precondition for a meaningful dogfood pass: the D12 corpus chapters from RC and ER; this PRD builds none.
...
| 2 | Comparison identity and currency | Ledger record on Compare ingest from the run manifest; chapter id and `documentId`; item coverage and saved-after-run rule; complete-run resolution | complete | 1, 3, 4 | RD-2, EL-3, EL-1, EL-5 | - |
...
| --- | --- | --- |
| 1 | New `apps/desktop/internal/proofing/*`; `docs/architecture/findings-contract.md` | SR engine package and signal contract, RD-1 store API, EL ledger API; the `accepted` wording in the contract |
| 2 | `apps/desktop/internal/transcript/service.go` and tests, the RD-2 adapter file, `docs/utilities/transcript-compare.md` | RD-2 (same ingest point), RF Phase 2 (event fan-out migrates the transcript service), TR ingest; EL parser |
| 3 | `apps/desktop/internal/tracks/{parse.go,tracks.go,testdata/*}` | TM Phase 8, EL parser phase, TR Phase 2, RF Phase 14 (all edit the parser and fixtures); land after them or rebase |
| 4 | New files in the proofing package; `apps/desktop/internal/manuscript/service.go` (`resetDerived`, line 434) | RD-1, EL, SR each add to the same slice literal (textual conflicts); DX-1 `measure` changes |
| 5 | Proofing package; reads DX-2 `Profile` | DX-2 (`Profile` gains a sample-peak limit; numeric settings kind), DX-4 findings shape |
| 6 | `apps/desktop/bindings.go`, `apps/desktop/app.go` (host API), `apps/desktop/app_test.go`, `apps/ui/src/{hostApi.ts,api/*,components/proofing/*}`, `Host.{js,d.ts}`, `tests/visual/*`, `docs/images/ui/*`, `docs/guides/using-the-app/proofing.md` | Every binding PR (host API number); SR's Home and Proofing summary work; DX-5 Delivery page (file picker); RD-5 Review page; nav-changing PRs regenerate screenshots |
