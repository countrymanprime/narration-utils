...
**Status (2026-09-26):** in delivery (lane train, [implementation plan](implementation-plan.md) section 8). The open questions take the recommended answers (D39; Decisions Log). Phases 1 and 2 are built; Phase 3's host half is built (the verdict on the locate result, its UI half is lane C's stream C5); Phases 4 and 5 are complete host-side (the resume locate now prefers REAPER's live state, ADR 0212; a host poll dismisses the prompt when REAPER plays or records, ADR 0213), with the scripted REAPER check still owed to the owner's verification pass and both phases' UI wiring pending (lane C). See the phase table.
...
| 3 | Reconcile two sources | `teleprompter.Reconcile`, `TeleprompterLocate` gains `lastReading` and `verdict`, `hostAPIVersion` bump, agreement notice and compact choice; ADR | host complete: `teleprompter.Reconcile` and the Go port of the chapter script and sentence bounds (`script.go`), `lastReading` and `verdict` on every locate answer but `asset_required`, `hostAPIVersion` 50, goldens `teleprompter-locate-{agree,disagree,complete,prompter-only}.json`, `teleprompterResumeVerdictSchema`, mock seeds `?mockResume=agree\|disagree\|prompter_only` ([ADR 0206](../adr/0206-resume-reconciles-the-recording-with-the-prompters-last-word-in-the-host-and-asks-only-when-they-disagree.md)). UI pending: the agreement notice, the compact choice and their visual states (lane C, C5) | - | 1, 2; RD1, RD3, RD8 | - |
| 4 | Live DAW state | `chapter_track_state` in `narration_track_state.lua` (harness tests first), `wire.go`, Go client with fallback, cursor-to-source mapping, "in REAPER now" label; threat model; scripted REAPER check (owner sign-off pending) | complete: the Lua command, its harness tests and the Go bridge client (`chapter_track_state`, `internal/bridge/track_state.go`) were already built and merged by the sibling Read Aloud Control Bar and Teleprompter Manuscript Integration streams (ADR 0231, ADR 0249); this phase wires the resume locate to it. `recordedEndFor`/`liveRecordedEnd`/`recordedEndAt` (`apps/desktop/teleprompterlivedaw.go`) prefer the live edit cursor (or the track's live end) over the saved project (RD2), fall back on any refusal or a shared-track region, and answer a new `recording_live` status while REAPER records; `TeleprompterLocate` gains `live: boolean` (`hostAPIVersion` 58) and `withResumeVerdict` labels the verdict's DAW place `teleprompter.DAWSourceLive` accordingly ([ADR 0212](../adr/0212-the-resume-locate-prefers-reapers-live-edit-cursor-over-the-saved-project-and-labels-which-one-answered.md)). Threat model rows 4a and 5j re-read and updated. The scripted REAPER check remains pending: it needs the same owner verification pass `chapter_track_state` itself already awaits (#510) | - | 3; RD2 | - |
| 5 | Follow REAPER while idle | Bounded poll while the prompt shows; play or record dismisses; optional cursor re-sync | complete host-side (RD7); RD6 (cursor re-sync) not built - a "Could" item needing the poll to also carry cursor position, which nothing yet asks for. `resumeWatch` (`apps/desktop/teleprompterresumewatch.go`) polls `chapter_track_state` about once a second while a watch runs, `TeleprompterWatchResume(trackGuid)`/`TeleprompterUnwatchResume()` bindings, a `teleprompter:resumeLive` event (empty payload) fired once and self-stopping, a 10-minute safety timeout ([ADR 0213](../adr/0213-the-resume-prompt-is-dismissed-by-a-host-poll-of-reapers-transport-not-by-the-ui.md)). UI pending: starting/stopping the watch from the dialog's own lifecycle, and subscribing to the dismiss event (lane C) | - | 4; RD6, RD7 | - |
...
| Live DAW position (2026-09-26, Phase 4, ADR 0212) | The resume locate calls the already-built `chapter_track_state` itself, preferring the live edit cursor (or the track's live end) over the saved project whenever REAPER is connected and the track holds no region, falling back to the saved project on any refusal; a live answer is labelled `teleprompter.DAWSourceLive` on the verdict's DAW place | A second, resume-only bridge command; always reading the saved project until Phase 5's poll lands | `chapter_track_state` already exists (built by the control-bar and teleprompter-input streams); reusing it needs no Lua or bridge change, only a host-side caller |
| Dismissal poll (2026-09-26, Phase 5, ADR 0213) | A standalone host poll (`resumeWatch`), independent of the teleprompter session's own state machine, reads `chapter_track_state` about once a second while a dialog watches and fires one `teleprompter:resumeLive` event then stops itself the moment REAPER plays or records; a 10-minute safety timeout covers a dialog that never explicitly stops it | Polling inside `teleprompter.Service`'s session state; a poll driven from the UI calling `TeleprompterLocate` repeatedly | Keeps a risky change out of the well-tested session lifecycle; a lightweight transport-only read costs far less per tick than a full locate |
