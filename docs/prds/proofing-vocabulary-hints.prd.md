...
- **Likely reasons it fails for a real project (from reading; not run):**
  ...
  3. **Weak feedback.** An empty result toasts "No new suggestions — everything found is already accepted." (`Transcript.tsx:148-152`), which is false when nothing was found at all; the toast is a fixed 2.4 s (`layout/Toast.tsx:8-9`) keyed by its text, so a second click gives no new signal (since fixed: an identical message that is showing now restarts its time and the queue keeps several, see [interaction feedback](../architecture/interaction-feedback.md)).
...
- [ ] **V7. Primitive or local component?** (Phase 2; recommendation adopted per D22. Delivered by the primitives stack S10: `TagInput` exists and the setup page's hints box uses it, [ADR 0055](../adr/0055-the-tag-input-is-a-wrapped-text-field-and-chips-that-report-and-never-edit-the-list.md).) A `TagInput` primitive (with stories and an atlas entry) versus a local component. Recommendation: the primitive, per the primitives PRD, since Pill groups and tag inputs recur.
...
| 2 | `Transcript.tsx`, optional new primitive and story, `app.drivers.ts`, `state-catalog.ts`, `proofing.md`, `doc-screenshots.json` | Primitives stack (delivered), Tooltip PRD, test-stability PRD (drivers) |
