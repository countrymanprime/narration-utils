...
**Reconciled 2026-09-21 (stack S08, [implementation plan](implementation-plan.md) section 1).** The owner answered the open questions (D16 and D22, ticked below). Since this was written the repository moved to `apps/desktop`, `apps/ui`, `libs/python`, `sidecars/` and `integrations/reaper` ([ADR 0040](../adr/0040-the-repository-is-laid-out-by-role-and-each-project-is-an-nx-project.md)); the host bindings read services through `h.services()` ([ADR 0041](../adr/0041-host-bindings-read-project-services-through-one-snapshot-accessor.md)); Base UI and the primitives are delivered (ADRs 0047 to 0058) and `@base-ui/react` is a dependency; the coverage ratchet exists ([ADR 0043](../adr/0043-coverage-is-a-ratchet-on-logic-directories-not-a-blanket-80-percent.md)); Knip is at zero ([ADR 0045](../adr/0045-dead-code-is-gated-at-zero-with-reasoned-ignores.md)); the dead `api/client/http.ts` is gone; and the Lua bridge has a test harness and a command registry ([ADRs 0066 to 0068](../adr/0066-the-lua-bridge-is-tested-by-a-harness-under-lua-5-4-and-reaper-api-behaviour-is-checked-in-reaper.md)), so the "no Lua in the MVP" and "Lua has no tests" statements below are stale (owner decision D2). Paths and line numbers in Evidence are those of the original branch. The decision record is [ADR 0069](../adr/0069-payloads-are-validated-with-zod-behind-parsewire-and-a-wrong-shape-fails-loudly.md).
...
- Lua changes beyond what the harness makes cheap (D2): Go validates the events it receives; an event version field and field-order tests are added in phase 7 only if the harness makes them small, otherwise they are a filed follow-up.
...
| Library | Zod 4 behind Standard Schema-typed `parseWire` (decided, D16 and D22) | Valibot, Zod Mini, hand-written, generated, none | Ergonomics, first-party JSON Schema, swappable; size irrelevant on desktop |
| Unknown fields | Lenient reads, strict contract tests (decided, D16 and D22) | Strict everywhere | Forward compatibility with drift detection |
| Schema location | TypeScript schemas plus Go/Python golden files (decided, D16 and D22) | JSON Schema source, Go tags | No new toolchain; payloads lack Go structs |
| Failure UX | By payload class (decided, D16 and D22) | Whole-app error, ErrorBoundary only | ErrorBoundary misses async failures |
| Persisted corrupt files | Per class: log, preserve and notify, or refuse (decided, D16 and D22) | One rule | Data value differs |
| Version rule | Additive: no bump; breaking: bump `hostAPIVersion` (decided, D16 and D22) | Separate payload version | One existing mechanism |
| Host log location | `%APPDATA%\narration-utils\logs\host.log`, next to the other per-user files, 1 MiB with one backup (decided, D22) | Wails runtime logger only | Survives a restart; the recents store already uses that folder |
| A fifth boundary, the in-app update manifest (D14) | `parseWire` accepts any Standard Schema and a JSON string, so a remote manifest can use it unchanged (decided) | A separate validator for remote data | One helper, one failure policy |
