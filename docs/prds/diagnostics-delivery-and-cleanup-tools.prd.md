...
- **Delivered:** phase 2 (#412, the settings number kind and the narrator's own delivery limits; ADR 0155, Proposed); phase 4 (#444, the windowed analyzers `measure.Diagnose`, with findings; ADR 0158, Proposed); phase 1 (the measurement job and its four bindings, `MeasurePickFiles/Analyze/State/Cancel`, with real progress, cancel, a range and the file fingerprint; ADR 0156, Proposed; `hostAPIVersion` 44); phase 5 (the Delivery page and its nav entry: pick files, real progress and Cancel, every value with its unit, "Not measurable" with its reason, a file it could not read with why, and the narrator's limits summarised and applied; no binding change); phase 3 in part (the gated EBU test; the recorded run is the owner's); phase 6 (the Diagnostics tab on the Delivery page and its three bindings, `DiagnosticsAnalyze/State/Cancel`, over the files picked for measuring: each finding with its time, measured value, threshold and source kind, read-only and unreviewed; `hostAPIVersion` 46); phase 7 (the report export, `DeliveryExportReport`: an HTML and a JSON report with the same finding IDs and review states in `<project>/narration-utils/delivery/`, deterministic apart from `generated_at`, without local paths unless the narrator includes them; the host now judges the measurements against the limits, so the page and the report show the same `delivery_qc` findings; `hostAPIVersion` 47).
- **Left:** phase 3's recorded run, and phases 8 to 11.
- **Needs the owner:** review ADRs 0155, 0156 and 0158; the EBU run of phase 3; the observation under Evidence (the narrator's own handoff check on two real chapters, now on the Delivery page) and the second-tool comparison on three real chapters; the REAPER checks of phase 10 (preview and apply) when it is built.
...
| --- | --- | --- |
| DSP agreement with EBU reference files | Every supplied file within the test set's published tolerance: ±0.1 LU for integrated loudness and +0.2/-0.4 dB for true peak (EBU Tech 3341 Table 1, November 2023; the table is `apps/desktop/internal/measure/testdata/ebu/expected.json`) | Go test over the official files, gated on a fixture directory (Phase 3) |
...
- [x] **6. EBU fixtures: commit, fetch, or skip.** Options: (a) commit the official files (size and redistribution terms TBD - needs research). (b) A gated test that reads a directory named by an environment variable and a documented fetch step, with expected values committed. (c) Skip. Recommendation: (b), and record the outcome (pass or discrepancy) in a docs note; open a new ADR only if the DSP has to change a recorded decision. **Settled (2026-09-23, Phase 3):** (b). The terms rule out (a): the set is about 87 MB (v5.0, 70 WAV files), and the EBU's terms of use (July 2019, v1.0) forbid copying, publishing or distributing the sequences and allow use only for internal R&D. The download step is manual, because the EBU site refuses scripted downloads and the terms must be accepted by a person. The test is `NARRATION_EBU_DIR`; the note is [delivery-measurement-validation.md](../architecture/delivery-measurement-validation.md). No DSP decision changed, so there is no ADR. The owner should confirm that this project's use counts as internal R&D and not commercial use.
- [x] **7. Report format and default privacy.** Options: (a) one self-contained HTML plus a JSON with identical finding IDs; (b) Markdown plus JSON; (c) also a zipped "reviewer package". Recommendation: (a) with local paths redacted by default and an explicit "include paths" checkbox; no audio embedded; no zip in the MVP. **Settled (2026-09-23, Phase 7):** (a), with an "include each file's full location" checkbox, off by default; [ADR 0170](../adr/0170-the-delivery-report-is-built-by-the-host-into-the-project-sidecar-without-paths-and-the-host-judges-the-limits.md) (Proposed).
...
| 5 | Delivery page | Page, nav entry, results table, limits summary, states (empty, running, error, unavailable), visual states, docs | complete | 3, 4 | 1, 2 | - |
| 6 | Diagnostics view | Bindings and Diagnostics tab listing findings with ranges and thresholds; read-only | complete | 7, 8 | 1, 4, 5 | - |
| 7 | Report export | HTML and JSON export, redaction, sidecar folder, deterministic output; review state included when the store exists | complete | 6, 8 | 1, 5 | - |
| 8 | Measured recorded duration | Chapter-track matches to recorded seconds on Home, ambiguous-match state, mapping exposed for the Review page's chapter grouping, docs; `tracks.md` kept accurate. The Home half (recorded seconds per chapter, Q10 answered (a) for the confirmed track only) moved to [Actual Recorded](actual-recorded-column.prd.md) (AR7 A; its Phase 2 delivered `recordedSeconds`); what remains here is the Review page grouping | pending | 1-7 | `teleprompter-manuscript-integration.prd.md` Phase 8 | - |
| 9 | Silence cleanup analyzer | `silence_cleanup` findings with classification and suggested action; thresholds in settings; findings view only | partial: the analyzer core is complete (lane B, stream B2; `internal/measure/cleanup.go`, [ADR 0238](../adr/0238-silence-cleanup-candidates-are-classed-in-the-diagnostics-pass-and-raised-apart-from-the-diagnostics-findings.md), Proposed). Pending: the threshold settings keys and saved presets in the layered store, and running `CleanupFindings` in the diagnostics job (lane A, `diagnostics_job.go`, settings rows); the list in the Diagnostics area (lane C, stream C7) | 8 | 4, 6 | - |
...
- **Scope**: fetch script or documented download (source: the EBU loudness test set v5.0 from `tech.ebu.ch`; the terms forbid redistribution, see Open Question 6), test gated on a directory variable, expected-value table committed, docs note with results; new ADR only if a decision changes (re-check `docs/adr/` numbering; take the next free ADR number at merge time, 0027 at `d5cc994`).
...
| Profile work in v1 (D22 default, Open Question 1) | Measurements plus the narrator's own limits; no built-in numeric preset | Vendor-neutral preset; "unvalidated" distributor preset | The PRD's recommendation; ADR 0025 already rules out distributor numbers ([ADR 0155](../adr/0155-settings-gain-a-number-kind-with-a-declared-range-and-delivery-limits-are-the-narrators-own.md), Proposed) |
| Where limits live (D22 default, Open Question 9) | A `Delivery` section of the layered settings with the new generic `number` kind (min, max, step, unit); one limit set per layer; clearing saves null | Named profile files in the sidecar | The PRD's recommendation; the same kind serves recording-coverage Phase 7's thresholds (ADR 0155, Proposed) |
| Which limits Phase 2 offers | Integrated loudness and RMS lowest and highest; sample peak, true peak and noise floor highest only | Every bound of every metric | A peak or noise floor is only ever too high; the analyzer thresholds arrive with their analyzers (Phases 4 and 9), so no setting changes nothing |
| How Phase 6 runs the diagnostics | Its own job and bindings (`DiagnosticsAnalyze/State/Cancel`, `apps/desktop/diagnostics_job.go`) over the same session allowlist as the measurement (ADR 0156), with progress from the bytes read and the narrator saying whether the files are raw recordings or rendered chapters; each file answers `measure.Diagnostics.Summary()` and `Findings()`, and nothing is saved | Run `Diagnose` inside the measurement job and add diagnostics to its payload | Phase 1's measurement payload and ADR 0156 stay as they are; the source kind is a choice the narrator makes for a check, not for a measurement; the findings stay unsaved and unreviewed until the dashboard's store ingests them (Open Question 4) |
| Cleanup classifier (Phase 9, ADR 0238, Proposed) | Silences from the silence map, keeping a 0.15 s hold at each side; breaths as quiet (12 dB or more below the read's 90th-percentile level), noise-like (zero-crossing rate 0.2 or more), 0.12 to 0.9 s runs of 10 ms windows; clicks as bursts of up to 30 ms standing 30 dB or more above silence on both sides. Confidence 0.3 for a pause of 2 s or more or a level within 3 dB of the floor, 0.35 to 0.5 for a breath, 0.6 otherwise, 0.8 for digital silence. Every candidate is an info `silence_cleanup` finding with a `split_and_trim` action that needs confirmation, raised by `CleanupFindings` apart from `Findings` | Spectral-shape features (an FFT per window); raising candidates through `Findings` now; a severity above info | Zero crossings separate noise from voiced sound without an FFT, and the approval gate carries what a heuristic misses (the PRD's accepted risk); a chapter has hundreds of pauses, so they wait for their own list |
| Phase 6's thresholds | ADR 0158's starting values, answered by the host with every job (idle included) and shown in a Thresholds panel and on each finding; not editable yet | Settings keys for them now, as ADR 0158's consequences expected | Phase 9 brings analyzer thresholds into the layered settings with its own; Phase 6 is read-only. ADR 0158 (Proposed) now says so too |
| Where Phase 5 judges a value against a limit | The page, against the effective limits it reads from settings, with `measure.Evaluate`'s rules (inclusive bounds, above checked before below, not measurable never a pass), so a limit changed in Settings re-judges what is on screen | Evaluate in the host when the job ends, adding findings to the job payload | The job payload stays Phase 1's (no binding or `hostAPIVersion` change); the stored `delivery_qc` findings with their IDs arrive with the report (Phase 7), which should replace the page's check with the host's; Phase 6 left it, because its scope is the windowed analyzers and a stored finding needs Phase 7's export or the dashboard's store |
| Who judges a measurement against the limits (Phase 7) | The host: every answer of the measurement job carries each measured file's `delivery_qc` findings from `measure.Evaluate`, judged against the effective Delivery settings when it is read (`judgeMeasureJob`), and a `limitsError` when a hand-edited limit is not a number; the page only shows what the findings say, and the report runs the same `Evaluate`, so both carry the same IDs | Keep the page's own check and evaluate only in the export | One judgement in one place: the page and the report cannot disagree. Judging on read keeps the page's behaviour (a limit changed in Settings re-judges without measuring again); the findings are not saved, since the Review page's ingestion of them is the dashboard's |
| Report export (Phase 7) | `DeliveryExportReport(includePaths)`: the page sends only that choice; the host reports the last measurement and the last check (refused while one runs, with no project, or with nothing measured), writes `delivery-report-<UTC stamp>[-n].html` and `.json` into `<project>/narration-utils/delivery/`, never over an earlier report. Files are named F1, F2, ... by base name; a finding is open until dismissed; review states come from the project's findings store (Delivery's findings are not in it yet, so they read unreviewed); installed assets are listed by kind, id and version from the asset registry; no manuscript excerpt is written (no Delivery finding carries one, so there is no opt-in yet) | Markdown; a zip; a save dialog; opt-in excerpts now | Open Question 7's recommendation (a); the sidecar folder is the PRD's; a fixed folder needs no picker and no new path boundary ([ADR 0170](../adr/0170-the-delivery-report-is-built-by-the-host-into-the-project-sidecar-without-paths-and-the-host-judges-the-limits.md), Proposed) |
...
- New and unverified: the EBU test-set tolerances and terms were researched in Phase 3 ([delivery-measurement-validation.md](../architecture/delivery-measurement-validation.md)), but the run over the official files is not yet recorded; an MP3 decoder option (TBD - needs research); persisted Transcript Compare word timing (TBD - needs research); muted-item handling in the `.rpp` parser (TBD - needs research); the accuracy of the source-to-project time mapping for trimmed, stretched and reversed items (per docs, inference).
