...
- **Left:** phase 3's recorded run, and phases 6 to 11.
...
| Where Phase 5 judges a value against a limit | The page, against the effective limits it reads from settings, with `measure.Evaluate`'s rules (inclusive bounds, above checked before below, not measurable never a pass), so a limit changed in Settings re-judges what is on screen | Evaluate in the host when the job ends, adding findings to the job payload | The job payload stays Phase 1's (no binding or `hostAPIVersion` change); the stored `delivery_qc` findings with their IDs arrive with the Diagnostics view and report (Phases 6 and 7), which should replace the page's check with the host's |
