...
- **Delivered:** phase 2 (#412, the settings number kind and the narrator's own delivery limits; ADR 0155, Proposed); phase 1 (the measurement job and its four bindings, `MeasurePickFiles/Analyze/State/Cancel`, with real progress, cancel, a range and the file fingerprint; ADR 0156, Proposed; `hostAPIVersion` 44); phase 5 (the Delivery page and its nav entry: pick files, real progress and Cancel, every value with its unit, "Not measurable" with its reason, a file it could not read with why, and the narrator's limits summarised and applied; no binding change).
- **Left:** phases 3, 4 and 6 to 11.
- **Needs the owner:** review ADRs 0155 and 0156; the observation under Evidence (the narrator's own handoff check on two real chapters, now on the Delivery page) and the second-tool comparison on three real chapters; the REAPER checks of phase 10 (preview and apply) when it is built.
