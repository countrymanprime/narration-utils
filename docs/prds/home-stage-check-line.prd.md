...
| No ADR expected | The delivering PR notes the change in the parent PRD's Phase 5 and Decisions Log | A new ADR | No ADR records the line or the button; if Q2 A amends the Q5 trigger list, `architecture-decision-records` decides at merge whether that is a recorded decision |
