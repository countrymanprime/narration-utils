...
- **Bootstrap and project switching.** `config` is `shell/app.go:74-86`; `parseConfigArgs` reads only `--project-folder`, `--project-name`, `--daw`, `--session-dir` (`app.go:435-453`), with no project-file argument. `Bootstrap` (`app.go:560-589`, TS type `shared/ui/src/api/contracts/system.ts:14-25`) carries no DAW project file. `ProjectSwitch` always sets `daw="Standalone"` (`shell/bindings.go:195-217`, `:202`). `App.tsx:156` shows `ProjectPicker` when `projectFolder` is empty.
...
- **No projects directory exists.** Nothing in `shell` calls `os.UserHomeDir`. Recents live in `%APPDATA%\narration-utils\recent-projects.json` (`shell/app.go:100-108`), capped at 10, deduped case-insensitively, silently pruned (`shell/internal/recents/store.go:18,52,94`).
- **The only DAW-file association today** is `Tracks.selectedRpp` in the project's `narration-utils/settings.json`, written by `tracksSelect` (`shell/tracks.go:39-56`, save at `:52`). `tracks.Discover` lists top-level `*.rpp` in the project folder only, non-recursively (`shell/internal/tracks/tracks.go:48-64`); `tracksSelect` refuses any path not in that list; `tracksList` errors when none or several are found (`tracks.go:62-80`). The saved value is an absolute path, so moving the folder invalidates it. `shell/media.go:51-64` calls `tracksList` on every `/media` request, so the media allowlist follows whatever the association resolves to.
- **REAPER does not pass the project file.** The launcher passes `--session-dir`, `--project-folder` (the rpp's dirname), `--project-name` and `--daw REAPER` (`shared/reaper/NarrationUtils_Launcher.lua:101-109`, context at `:39-47`); an unsaved project yields an empty folder. The project folder is therefore *defined* as the rpp's folder by the launcher, by Lua `prepare_compare` (`shared/reaper/narration_ui_bridge.lua:97-103,167`, which writes `TranscriptCompare/` next to the rpp) and by `tracksSelect`. `transcript/service.go:77` and `:371-388` only agree with Lua while the project folder equals the rpp folder.
- **Nothing gates or verifies.** Tracks is `alwaysEnabled` (`shared/ui/src/components/layout/AppShell.tsx:18-28`); `isDisabled` (`:52`) tests only the manuscript; `App.tsx` routes gate only on `data.manuscript` and `/tracks` has no gate (`:216`). No bridge command reports the open project: the dispatcher (`narration_ui_bridge.lua:511-560`) has `prepare_compare`, `inspect_compare_results`, `export_compare_markers`, `jump_to_compare_marker`, `stamp_item_lines`, `read_line_ids`, `create_chapter_regions` and `close`; only `prepare_compare` calls `EnumProjects(-1,'')` (`:97`) and never compares it to the app's project.
- **"Can't reach the DAW" is not detectable.** The session dir arrives only through `--session-dir` (`app.go:169-172`); `bridge.New` just `MkdirAll`s `commands/` (`shell/internal/bridge/bridge.go:59-64`). There is no handshake, heartbeat or liveness check, so a stale session dir yields a "connected" client nobody reads. The session dir is minted by the Lua launcher per run (`NarrationUtils_Launcher.lua:67`).
- **No code launches REAPER.** The only `os/exec` uses are the Python sidecar supervisor (`shell/internal/process/supervisor.go`), which on Windows assigns children to a kill-on-close job object (`job_windows.go:14-25`), so REAPER started through it would die with the app. There is no DAW executable setting; Settings "DAW Integration" (`Settings.tsx:179-213`) is static text that says "Connected" even for `Standalone`. Passing a project to REAPER is documented in `docs/research/reaper-automation-surface.md:150` (`reaper.exe <project.rpp>`), not verified here.
- **Settings.** Layers are project (`<project>/narration-utils/settings.json`), global (`%APPDATA%\narration-utils\global-settings.json`) and repo `defaults.json` (`shell/internal/settings/store.go:135-149,42-48`); `fieldSchemas` (`app.go:673-679`) has five tools and validates only `choice` and `color` (`app.go:718-723`). The TS `kind` union already allows `'text'` (`system.ts:7`). A computed default such as `~/NarrationUtils` cannot live in `defaults.json` (`TestBuiltinDefaultsMatchRepoDefaultsFile`, `store_test.go:9`).
...
- Automatically running the launcher script inside REAPER at startup, or modifying REAPER's configuration (`shell/app.go:244` states REAPER is never modified automatically); it stays a spike question.
...
- [ ] **W9. `ProjectCreate` semantics.** It changes from "full folder" to "parent plus name". A signature change bumps `hostAPIVersion` (5 in `shell/app.go:33`, `shell/app_test.go:39-41`, `shared/ui/src/hostApi.ts:2`; regenerate `Host.{js,d.ts}`). Options: change the existing binding, or add `ProjectCreateIn(parent, name)` and retire the old one. Recommendation: new binding, one bump.
- [ ] **W10. Can the app know REAPER is running and which project is open?** Needs a Lua-side heartbeat or `report_project` event and an event route in the transcript-owned reader (`ReadEvents` has one offset, and `Handle` drops events whose second field is not the current run id). `EnumProjects(-1,'')` returns only the active tab and is empty for unsaved projects. Resolved by the Phase 6 spike.
- [ ] **W11. Where is `reaper.exe`?** No setting exists. Options: a global setting with auto-detect, registry lookup, or `ShellExecute` on the `.rpp` (file association). Launching must be detached, not through the sidecar supervisor's kill-on-close job. Resolved by the Phase 6 spike.
- [ ] **W12. Can the launcher script start automatically when REAPER is launched by the app?** Without it, a launched REAPER has no bridge until the user runs the action, and the session dir is minted by the launcher (`NarrationUtils_Launcher.lua:67`). REAPER 6.80+ can accept a script argument (`reaper-automation-surface.md:150`, unverified). This conflicts with "REAPER is never modified automatically". Resolved by the Phase 6 spike and the owner.
...
- [ ] **W18. REAPER-launched with several rpp files in the folder.** The launcher knows the exact rpp but drops it (`Launcher.lua:39-47`); until Phase 5's `--project-file` lands, a "linked" gate would lock Proofing for these users even though REAPER is live. Recommendation: treat a live `--daw REAPER` launch as linked until Phase 5.
...
**Architecture notes**
- New small Go package (for example `shell/internal/project`) owning the manifest (`project.json`: name, `dawProjectFile` {absolute, relative}, created date) with atomic writes, and the projects-directory helpers (default resolution, validation, scan). `Host` reads it under the `h.mu` snapshot pattern (`host-binding-data-race.prd.md`); every attach still goes through `canAttachLocked` (`app.go:467`) before any `MkdirAll`.
...
| `ProjectCreate` rework collides with the host-binding PRD | High | Sequence after that PRD's accessor phase or write the new binding on it; bump `hostAPIVersion` once |
...
| 5 | REAPER launcher passes the file | `--project-file`, second-launch mapping, unsaved-project handling, rpp outside the folder (Lua and transcript paths); manual REAPER sign-off | pending | - | 3 | - |
| 6 | Spike: reachability, verification and launch | Heartbeat or `report_project`, `reaper.exe` discovery and detached launch, auto-running the launcher (W10-W12); docs and ADR only | pending | 1-5 | - | - |
...
| --- | --- | --- |
| 1 | new `shell/internal/project`, `shell/app.go` (`NewHost`/`Startup`), `fieldSchemas`, `Settings.tsx`, `system.ts`, mock | Other `fieldSchemas`/`Settings.tsx` editors (teleprompter, diagnostics, story-bible PRDs) |
| 2 | `shell/bindings.go`, `shell/app.go`, `ProjectPicker.tsx`, new dialog, `hostApi.ts`, `Host.*`, `wailsClient.ts`, `mockApi.ts`, catalog/drivers, `getting-started.md` | `host-binding-data-race.prd.md` phases 1-4 (`ProjectCreate` and `bindings.go`), every phase that bumps `hostAPIVersion` |
| 3 | `shell/internal/{project,tracks}`, `shell/tracks.go`, `shell/media.go`, `Bootstrap`, contracts, mock | Analysis PRDs that cite `Tracks.selectedRpp` (`analysis-evidence-ledger.prd.md:18`), API bump |
| 4 | `AppShell.tsx` (nav and the pill markup, which regenerates every screenshot), `NavButton.tsx`, `App.tsx`, `Home.tsx` (Proofing card), `Transcript.tsx`, `TracksPage.tsx`, `Settings.tsx`, `main.tsx`/mock, catalog/drivers, `doc-screenshots.json`, guides | Nav-item additions (each regenerates every screenshot; see README sequencing), dialog/a11y PRDs, `import-review-redesign.prd.md` and the other `Home.tsx` editors, `proofing-vocabulary-hints.prd.md` (`Transcript.tsx`) |
| 5 | `shared/reaper/{NarrationUtils_Launcher,narration_ui_bridge}.lua`, `shell/app.go`, `shell/internal/transcript` | `reaper-automation-follow-through.prd.md` (Lua dispatcher), `release-artifact-naming.prd.md` Phase 1 (launcher exe name), transcript and take-review PRDs |
| 6 | `docs/` only, scratch REAPER scripts out of tree | ADR numbering |
| 7 | Lua dispatcher, `shell/internal/bridge`, transcript reader, `Bootstrap`, header pill | Same Lua dispatcher PRDs |
| 8 | new launcher package, `fieldSchemas`, `Settings.tsx`, `Bootstrap`, `AppShell.tsx`, mock | `fieldSchemas`/Settings editors, API bump |

Cross-cutting: every phase re-checks `docs/adr/` numbering before writing an ADR and re-checks `hostAPIVersion` at merge time; every `shared/ui` phase runs `visual-catalog-sync`, the Playwright visual suite with PNG review at all four viewports, and `doc-screenshot-sync`; Phases 5, 7 and 8 need manual REAPER verification (`shared/reaper` has no tests); each phase follows `CLAUDE.md`: plan, `change-impact-scan`, TDD, `full-verification-gate`, `design-spec-guard`, `feature-cleanup`.
