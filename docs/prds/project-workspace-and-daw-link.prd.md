...
| 2 | `apps/desktop/bindings.go`, `apps/desktop/app.go`, `ProjectPicker.tsx`, new dialog, `hostApi.ts`, `Host.*`, `wailsClient.ts`, `mockApi.ts`, catalog/drivers, `getting-started.md` | `host-binding-data-race.prd.md` phases 1-4 (`ProjectCreate` and `bindings.go`), every phase that bumps `hostAPIVersion` |
...
| 5 | `integrations/reaper/{NarrationUtils_Launcher,narration_ui_bridge}.lua`, `apps/desktop/app.go`, `apps/desktop/internal/transcript` | `reaper-automation-follow-through.prd.md` (Lua dispatcher), `release-artifact-naming.prd.md` Phase 1 (launcher exe name), transcript and take-review PRDs |
