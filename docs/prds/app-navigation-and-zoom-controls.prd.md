...
- **Wheel and pinch zoom stay on because `Windows` is nil.** Zoom control and pinch zoom are only set when `opts.Windows != nil` (`frontend.go:578-589`: `PutZoomFactor` if `ZoomFactor > 0`, `PutIsZoomControlEnabled(opts.IsZoomControlEnabled)`, `PutIsPinchZoomEnabled(!opts.DisablePinchZoom)`). With nil options WebView2's defaults apply, which have zoom control (Ctrl+wheel) and pinch zoom on. This is what the owner uses. **A trap for this work:** the moment `main.go` passes any `windows.Options` (for example to set a startup `ZoomFactor`), `IsZoomControlEnabled` defaults to `false` and Ctrl+wheel zoom silently stops, unless it is set to `true` explicitly. **On Wails v3** (since [ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md)) the same trap is closed: v3's window option `ZoomControlEnabled` also defaults to off, so `main.go`'s `mainWindowOptions()` sets it to `true` and a Go test holds it; pinch stays WebView2's default. v3 still turns the browser accelerator keys off (`webview_window_windows.go`, `PutAreBrowserAcceleratorKeysEnabled(false)`), so the page still handles Ctrl+0 and Alt+Left itself.
- **Wails v2 has no runtime zoom API.** `pkg/runtime/window.go` has size, position, fullscreen, theme, reload, `WindowExecJS` and print, but nothing for zoom; the only zoom knob is the startup `windows.Options.ZoomFactor` (`pkg/options/windows/windows.go:90-93`). The `edge.Chromium` object that could call `PutZoomFactor` at runtime is private to Wails' internal frontend. (Wails v3 has window zoom methods, and the owner chose to move to it, D29: the shell runs on Wails v3 beta.25 since the [Wails v3 Migration](wails-v3-migration.prd.md), [ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md).) On macOS, `mac.Options.DisableZoom` is the window's green zoom button, not page zoom (`internal/frontend/desktop/darwin/window.go:125`).
...
Zoom should be the webview's real page zoom, not a CSS imitation, because only real zoom reflows the layout through the app's breakpoints (the rail and the drawer), which the design and the visual suite already assume ([ADR 0061](../adr/0061-settings-states-are-also-captured-at-a-390px-reflow-width.md)). Wails v2 cannot change it at runtime; the owner settled the mechanism (Q3, D29): Wails v3's `Window.SetZoom`, on the window [ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md) names `main`.
...
- [x] **Q1. Placement.** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken (in the existing header). Options: (A) in the existing header: Back/Forward at its left, the zoom group at its right before the REAPER pill; (B) a thin browser-style strip above the header holding all of them; (C) Back/Forward in the header, zoom only in Settings > Appearance and the shortcuts. Recommendation: A. B costs 32 to 40 px of height on every page, which the Manuscript and the teleprompter reader need most; C hides the level, which is the thing the owner asked to see.
- [x] **Q2. Which controls.** Answered per D22/D39 (2026-09-25, stream C2): the recommended set taken, no fullscreen toggle added. Recommended set: Back, Forward, Zoom out, zoom level (reset), Zoom in. Rejected: reload, find, print, copy link, address bar (see What We're NOT Building). Does the owner want any other? A fullscreen toggle (F11) for the teleprompter is the one plausible addition; recommendation: not here (its own request if wanted).
- [x] **Q3. How zoom is applied.** Answered by the owner (D29, 2026-09-24): none of A to C; the shell moves to Wails v3 beta and zoom uses its runtime `Window.SetZoom` ([ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md)). The options as first written, kept for the record: Options: (A) **the webview's own zoom, through a small patch to Wails**: a pinned fork of `wailsapp/wails/v2` (a `replace` in `apps/desktop/go.mod`) that adds a runtime zoom get/set and a zoom-changed callback on Windows, offered upstream; real zoom, the same steps and reflow as Ctrl+wheel, but the app carries a fork until upstream takes it; (B) **the webview's own zoom without a fork**: the startup `ZoomFactor` option for persistence, plus Win32 from the host to drive WebView2's own zoom at runtime (for example sending it Ctrl+wheel input); no fork, but fragile and the steps are WebView2's; (C) **CSS zoom on the page root**, applied by the UI: no Go change and works in the browser demo, but the breakpoints do not move (the sidebar would stay at 224 px scaled up at 200% instead of giving way to the rail and drawer), it stacks with any Ctrl+wheel zoom unless native zoom is turned off, and pop-up positioning under CSS zoom needs checking. Recommendation: A, if the spike shows the patch is small (it should be: the `chromium` object already has `PutZoomFactor`); B only if the owner refuses a fork; C last, because it gives a different layout from the zoom the owner uses today.
- [x] **Q4. Persist zoom across launches?** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken. Options: (A) yes, one app-wide level stored in global settings (the host needs it before the window opens, so it is not `localStorage`); (B) no, every launch starts at 100%; (C) remember it, and add a "Default zoom" row in Settings > Appearance as well. Recommendation: A; C only if the owner wants to set it there.
- [x] **Q5. What does Back step through?** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken. Options: (A) pages only (the history the app already records: every nav move and every "go to" link); (B) pages, plus closing an open slide-over or drawer first (phone-style); (C) pages plus in-page positions (previous chapter, previous Story Bible entry). Recommendation: A. B conflicts with modal drawers owning Escape and focus (ADR 0051); C needs every page to write its selection into the URL, a much bigger change.
- [x] **Q6. Zoom range and steps.** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken, narrowed to 100%-200% on Windows by Proposed ADR 0201 (unchanged by this decision; still pending the owner). Options: (A) WebView2's own steps clamped to 50% to 200% (50, 67, 75, 80, 90, 100, 110, 125, 150, 175, 200); (B) the full browser range, 25% to 500%; (C) 67% to 300%. Recommendation: A: 200% meets WCAG 1.4.4 (resize text) and on the 960 px minimum window is 480 CSS px, above the 390 px reflow width the app is checked at; beyond that the known cramped layouts (#148 to #150) are reachable by a stray wheel. Ctrl+wheel is clamped to the same range (the host resets a level outside it). **Under Wails v3 on Windows** the host cannot set a level under 100% (`SetZoom` and `ZoomOut` floor at 1.0) and no zoom-changed event reaches the host; Proposed [ADR 0201](../adr/0201-app-zoom-under-wails-v3-on-windows-runs-from-100-to-200-percent-and-reads-the-level-back-from-the-window.md) narrows the buttons to 100% to 200% and reads the level back from the window, pending the owner.
- [x] **Q7. Native menu entries.** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken (Phase 4/Q7 B not scheduled in this train). Options: (A) none on any platform; the header and shortcuts are the whole surface; (B) on macOS only, the standard app menu with View > Zoom In / Zoom Out / Actual Size and History > Back / Forward (Wails `options.App.Menu`), which macOS users expect; (C) also a Windows menu bar (File/View/History strip above the webview). Recommendation: A now, B later if a macOS build is ever promoted; C adds window chrome for three commands that are already in the header.
- [x] **Q8. Back across a project switch.** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken. Options: (A) Back is disabled at the first page of the current project (earlier entries belong to another project and would open its pages with this project's data); (B) Back crosses the switch like a browser. Recommendation: A.
- [x] **Q9. Is the reset always visible?** Answered per D22/D39 (2026-09-25, stream C2): recommendation A taken. Options: (A) the level readout is always shown and is the reset button (disabled, reading 100%, at 100%); (B) the readout and reset appear only when zoom is not 100%; (C) always show `−`/`+`, show the level and reset only when not 100%. Recommendation: A: a control that appears and disappears moves the header's layout and is harder to find; a disabled "100%" also tells the narrator zoom exists.
- [x] **Q10. Narrow widths.** Answered per D22/D39 (2026-09-25, stream C2): the mockups (`07-reflow-390-header-optionA.webp`, `07a-reflow-390-header-optionA-100.webp`) show option A fitting at the 390 px reflow width, so A is taken (checked again against the actual capture in Phase 1/2). At about 167% zoom on a 1280 px window, or 125% on the 960 px minimum, the shell drops below `md` and the header already holds the menu button, the project name and the pill. Options: (A) keep everything, let the project name truncate further, and shorten the pill to its dot plus an accessible name below `md`; (B) below `md`, fold the zoom group into one "Zoom 150%" menu button (a `Menu` with Zoom in, Zoom out, Reset); (C) below `md`, hide Forward. Recommendation: A if the 390 px capture fits, else B.
...
**Feasibility.** Back/Forward is UI only. Zoom adds a host binding on Wails v3's `Window.SetZoom`/`GetZoom` (Q3, D29); under [ADR 0201](../adr/0201-app-zoom-under-wails-v3-on-windows-runs-from-100-to-200-percent-and-reads-the-level-back-from-the-window.md) it adds no event. Keys arrive in the page because Wails disables WebView2's browser accelerators (`frontend.go:596`), so the page must implement Ctrl+0 and Alt+Left itself; this is the reason the shortcuts are in scope at all.
...
**Zoom (Phases 2 and 3), on Wails v3 (Q3, D29).**
- The host binding `WindowZoom` / `WindowSetZoom(factor)` reads and sets the level with `GetZoom`/`SetZoom` on the main window (`mainWindow()` in `apps/desktop/wailsapp.go`, [ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md)), clamps to the Q6 range and snaps to the step list. v3 exposes no zoom-changed callback, so the UI re-reads `WindowZoom` when the page's `devicePixelRatio` changes (a Ctrl+wheel or pinch), per Proposed [ADR 0201](../adr/0201-app-zoom-under-wails-v3-on-windows-runs-from-100-to-200-percent-and-reads-the-level-back-from-the-window.md), which also sets the range Windows allows (100% to 200% from the buttons). A platform where `GetZoom` fails returns "unsupported" and the UI hides the zoom group.
- `mainWindowOptions()` in `main.go` already sets `ZoomControlEnabled: true` (the trap in Evidence) and leaves pinch on. Phase 3 adds the saved level as the window's startup `Zoom`. The level is written to global settings under a global-only tool (for example `Appearance.zoom`, refused at project scope like `Updates`, `app.go:1207-1212`), debounced, and read before the window is created so the first paint is at the saved level.
- **Wire contracts** (`CLAUDE.md`, `docs/architecture/wire-contracts.md`): a Zod schema for the binding result and the event in `apps/ui/src/api/schemas/`, a golden payload from a Go test in `tests/fixtures/contracts/` (`UPDATE_CONTRACTS=1`), a row in `wireContracts.test.ts`, a mock in `mockApi.ts` (with `?mockZoom=125` for the visual suite), and the event in the table the host's events use if one is added. `hostAPIVersion` bumps to `main`'s value + 1 (a serialization point; 49 when the Wails migration landed). The host binding reads no project services, but still goes on the accessor pattern and into `stressReaders` if it touches `h.services()` for settings.
- (Q3 C, CSS zoom through a `useZoom` hook with native zoom turned off, was not chosen: D29.)
...
**Security and trust boundaries.** Back and Forward only call the history API inside the app's own origin; the main frame never leaves it (Evidence), and `idx` stops Back at the first entry. Record that in the threat model's webview section (row 3, `docs/architecture/threat-model.md:78-84`) as the reason in-app history needs no allow-list. The zoom binding sets a number on the app's own window and reaches no file or network; the Wails v3 dependency itself is recorded in the threat model and `SECURITY.md` by the migration ([ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md)), so no fork and no patched Wails is involved.

**Risks.**
1. Wails v3's `ZoomControlEnabled` defaults to off; `mainWindowOptions()` sets it and a Go test holds it (Evidence). A desktop check in Phase 2 covers it.
2. Wails v3 is a pinned beta ([ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md)); on Windows it cannot set a level under 100% ([ADR 0201](../adr/0201-app-zoom-under-wails-v3-on-windows-runs-from-100-to-200-percent-and-reads-the-level-back-from-the-window.md)). A later beta that changes zoom is a pin change reviewed on its own.
...
**Phase 0 - Verify today on Wails v3**
- **Scope:** on a copy of a project with an isolated app data folder, run the packaged Windows build and record: Ctrl+wheel, pinch, Ctrl+=, Ctrl+-, Ctrl+0 (expected: first two only); quit at 150% and relaunch (does it come back?); mouse buttons 4/5 and Alt+Left/Right after two page moves (do they navigate, and do they bypass the Settings confirm?); whether `preventDefault` on the mouse buttons stops WebView2. The build is the Wails v3 one (the mechanism is settled, D29); no Wails patch is prototyped. Anything needing the owner's hardware (a touchpad pinch, a mouse with side buttons) is marked pending for the owner.
- **Output:** a short note in `docs/research/` with the observations. No product code.
...
- **Verification:** TDD on the hook first; `pnpm check`; `npx playwright test tests/visual/app.spec.ts -g "shell.*history"` and every PNG of the `shell` states under `apps/ui/screenshots/app/`; `pnpm --dir apps/ui run aria`; a scripted desktop run for the mouse buttons (owner-pending if it needs their mouse).

**Phase 2 - Zoom controls**
- **Scope (on Wails v3, D29):** a new `apps/desktop/bindings_window.go` with `WindowZoom`/`WindowSetZoom` on `mainWindow()`'s `GetZoom`/`SetZoom`, their Go tests and contract golden (no `go.mod` change: the migration pinned Wails v3; `ZoomControlEnabled` is already on); `hostAPIVersion` bump in its three places and `Host.{js,d.ts}`; `apps/ui/src/api/` contract, schema, `wailsClient.ts`, `mockApi.ts` (`?mockZoom`), `wireContracts.test.ts`; a `useZoom` hook (level, step up and down, reset, debounced announcement); the header group; Ctrl key handling in the Phase 1 listener; catalog rows; visual row `shell / zoom-level` with reflow; the aria header snapshot updated; the guide.
...
| 2 | Phase 1's UI files again, `apps/desktop/{app.go,app_test.go}`, new `bindings_window.go` and tests, `hostrace_test.go` (if it reads services), `apps/ui/src/{hostApi.ts,api/*,wailsClient.ts,mockApi.ts}`, `api/wireContracts.test.ts`, `tests/fixtures/contracts/`, the generated `apps/ui/wailsjs/**` bindings | `hostAPIVersion` (every PRD adding a binding); the same UI and screenshot collisions as Phase 1 |
...
Cross-cutting: each phase follows `CLAUDE.md`: an issue and `Closes #<n>`; `change-impact-scan` on `AppShell.tsx`, `App.tsx` and (Phase 2) the host bindings; TDD; `full-verification-gate` (`pnpm check`, the visual suite with PNGs looked at for every viewport and the reflow width, `pnpm --dir apps/ui run aria` since the navigation changes); `design-spec-guard` only if a primitive or `styles.css` changes; `feature-cleanup` including the trust-boundary check (the webview).
...
| Zoom mechanism (proposed, Q3 A; superseded by the owner's D29 row below) | The webview's own zoom via a minimal Wails patch | Win32 input; CSS `zoom` | Only real zoom reflows through the breakpoints the design and ADR 0061 assume |
...
| Zoom range (proposed, Q6 A) | WebView2's steps clamped to 50%-200%, narrowed to 100%-200% on Windows (Proposed ADR 0201) | The full browser range; 67%-300% | 200% meets WCAG 1.4.4 without reaching further into the known cramped 390 px layouts (#148-#150) |
| Native menu (proposed, Q7 A) | None on any platform; header and shortcuts are the whole surface | A macOS app menu (Q7 B); a Windows menu bar | No macOS build is promoted yet; a menu bar adds chrome for commands already in the header |
| Back across a project switch (proposed, Q8 A) | Back is disabled at the first page of the current project | Back crosses the switch like a browser | Earlier entries belong to another project and would open its pages with this project's data |
| Reset visibility (proposed, Q9 A) | The level readout is always shown and is itself the reset button | Readout/reset appear only when zoom is not 100% | An appearing/disappearing control moves the header layout and is harder to find |
| Narrow widths (proposed, Q10 A) | Keep everything below `md`, let the project name truncate further and the pill shorten | Fold the zoom group into a menu button (Q10 B); hide Forward (Q10 C) | The reflow mockups (`07-reflow-390-header-optionA.webp`, `07a-...-100.webp`) show option A fitting at 390 px; rechecked against the real capture in Phase 1/2 |
| ADRs expected | One ADR at Phase 2 (next free number at merge, 0171 at this commit): "The app has page-level Back and Forward and real webview zoom in the header, its shortcuts are handled by the page because WebView2's browser keys are off, and the header is captured at the reflow width". The dependency decision is already its own ADR ([ADR 0200](../adr/0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md), the Wails v3 migration), and the Windows zoom range is Proposed [ADR 0201](../adr/0201-app-zoom-under-wails-v3-on-windows-runs-from-100-to-200-percent-and-reads-the-level-back-from-the-window.md) | Fold the dependency into the same ADR | ADRs are immutable; a dependency is a decision someone will want to find by itself |
...
| Zoom mechanism (owner, 2026-09-24, Q3, D29) | Migrate the desktop shell to Wails v3 beta (pinned, starting at `v3.0.0-beta.25`) and use its runtime `Window.SetZoom`; full controls (zoom out, a level readout that resets, zoom in, Ctrl+=/-/0) and the level remembered across launches. Wails v2.16 has no runtime zoom call, only the startup `ZoomFactor` | A patched Wails v2; CSS `zoom`; readout and reset only | The owner accepts a beta for a personal tool; no fork to maintain; CSS zoom breaks layouts (see the overflow PRD) |
| Sequencing (owner, 2026-09-24, D29) | The Wails v3 migration runs first and alone, before any other stack of the 2026-09-24 train, because it regenerates every binding; the owner launches the new build once before the next wave starts | Migrate last | Every other stream touches bindings or events |
