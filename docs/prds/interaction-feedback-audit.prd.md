...
- The host reports the install job as `{id, voiceId, phase, message}` with `phase: "running"` (`shell/app.go:735,777-781`); the job struct has no percent and no error field, and `assets.Store`/`tts.Manager.Install` have no progress reporting at all (`shell/internal/tts/catalog.go:96-102`; no `percent`/`progress` in `shell/internal/tts` or `assets`).
- The UI contract says `phase: 'downloading' | 'success' | 'cancelled' | 'error'` plus `percent` and `error` (`shared/ui/src/api/contracts/tts.ts:26-33`), and `GuideDetail.tsx` keys everything on `'downloading'`: the poll loop (`:152-172`), the title, body, button label and progress bar (`:768-785`), cancel (`:173-184`). The Whisper contract, by contrast, uses `'running'` (`contracts/whisper.ts:24-29`; `TeleprompterPage.tsx:78`).
- The mock returns instant success with `percent: 100` (`shared/ui/src/api/mockApi.ts:521-533`), and no test in `shell/` or `shared/ui/{src,tests}` calls `ttsInstall`/`TtsInstall` (grep), so nothing exercised the mismatch.
...
- Overlapping calls are unsafe: each process does read-modify-write of `manuscript_guide.json`, and `write_json` writes a fixed `.tmp` sibling then `os.replace` (`tools/manuscript-guide/core/manuscript_guide.py:729-734`), so two concurrent writers can lose an update or collide on the temp file (not reproduced). The code comment at `bindings.go:301-304` already says concurrent writers would race.
...
- The uncached pronunciation preview has the same gap: `playPreview` awaits `guidePreview` (Go, then `render-audio` with Piper voice load) with no loading state until sound starts (`shared/ui/src/components/storybible/usePreviewAudio.ts:54-100`).
...
- `NarrationApi` has 72 members across nine contract files (`shared/ui/src/api/contracts/`): Manuscript 19, Story Bible 12, Transcript 11, System 6, Project 5, Teleprompter 5, Tts 5, Whisper 5, Tracks 4. Seven are not request-response (four `subscribe*`, `mediaUrl`, `ready`, `reportClientDiagnostic`), leaving 65.
- Grepping `.<method>(` in non-test, non-story source outside `shared/ui/src/api/` finds **87 call sites in 12 files**: Manuscript.tsx 13, Transcript.tsx 12, GuideDetail.tsx 12, Home.tsx 11, TeleprompterPage.tsx 9, Settings.tsx 8, ProjectPicker.tsx 7, App.tsx 4, Results.tsx 3, Guide.tsx 3, TracksPage.tsx 3, AudiobookEstimatePanel.tsx 2. 64 of the 65 methods are called; `manuscriptReader` has no UI caller (a bound but unused method).
...
- Go-native candidates to measure too (not Python): `tracksList` re-parses the `.rpp` on every call and `authorizedMediaPath` calls it on every `/media` request (`shell/media.go:51-64`); `ManuscriptSearch`/`Chapters`/`Reader` on a full manuscript.
...
- REAPER/Lua-side interactions (`shared/reaper`) - no automated tests, separate manual-verification scope.
...
- **Host API**: bump `hostAPIVersion` (`shell/app.go:33`, `shell/app_test.go:40-41`, `shared/ui/src/hostApi.ts:2`) and regenerate `shared/ui/wailsjs/go/main/Host.*` only when a binding's signature or required payload changes (a new event alone does not; a changed `GuideEdit` signature would).
...
- **Scope**: the `motion-and-animation.md` link (already pointing at this PRD; repoint to the ADR and catalog), ADR index, cross-links, screenshots via `doc-screenshot-sync` (and the matching pages of `docs/guides/using-the-app/`, guarded by `shared/ui/src/docsGuide.test.ts`), GitHub issues for leftover gaps, cleanup. New defects found along the way are GitHub issues (Bug report form, `area:ui` label), per `docs/operations/github-workflow.md`, not entries in a Markdown register.
...
| --- | --- | --- |
| 1 | new harness file under `shell/` (build-tagged test) or `scripts/`, `docs/research/` | Low; additive |
| 2 | new catalog and ratchet test in `shared/ui/src/`, new ADR | ADR numbering; any session adding an API call site (catalog rows) |
...
| 4 | `shell/bindings.go` (`GuideEdit`), `shell/internal/guide/service.go`, `tools/manuscript-guide/core/manuscript_guide.py` and tests, possibly `hostApi.ts` and Wails bindings | host-binding-data-race phase 2 (same functions), briefs phase 10 (`edit()`), any API bump |
| 5 | `App.tsx`, `layout/Toast.tsx`, `Guide.tsx`, `Home.tsx`, `Transcript.tsx`, `shell/app.go` (events), `api/wailsClient.ts` | briefs phases 2 and 3 (they consume or share the hook), release-readiness Phase 1 (job snapshot shape), dialog PRD, `App.tsx` editors (teleprompter PRDs) |
...
Cross-cutting: every phase re-checks `docs/adr/` numbering immediately before writing an ADR; every phase that changes a binding signature or required payload bumps `hostAPIVersion` in `shell/app.go:33`, `shell/app_test.go:40-41`, `shared/ui/src/hostApi.ts:2` and regenerates `Host.{js,d.ts}` (several PRDs bump it: whichever lands second increments again); every `shared/ui` phase runs `visual-catalog-sync`, the Playwright visual suite with PNG review of `shared/ui/screenshots/app/<page>/<state>/<viewport>.png` at all four viewports (`shared/ui/tests/visual/viewports.ts`), and `doc-screenshot-sync` (which now refreshes the split guide under `docs/guides/using-the-app/`; `shared/ui/src/docsGuide.test.ts` guards its links and screenshot embeds); primitives or `styles.css` changes also run the atlas (`pnpm --dir shared/ui atlas`) and `design-spec-guard`; every phase follows `CLAUDE.md`: plan, `change-impact-scan`, TDD, `full-verification-gate` (`pnpm check`, not `check:fast`), `design-spec-guard`, `feature-cleanup`. Nothing merges without the user.
...
- Adjacent findings not owned here: `Supervisor.Run` waits on the process before its pipe readers finish (`shell/internal/process/supervisor.go:98-101`), which can truncate the last stdout line `Create` parses (`CREATED|<id>|<n>`); `manuscriptReader` is bound but has no UI caller. Both are listed in the hand-off message.
