...
| 11 | Per-chapter render configuration | Lua `configure_chapter_render`, Go and UI; configure-only; manual checklist | partial | 13 | 7, 10 | - |
| 12 | Chapter tag embedding | Go ID3 CHAP on rendered MP3 (library TBD - needs research) as a new file; explicit action | complete | 13, 14 | 11 | [ADR 0099](../adr/0099-chapter-tags-are-embedded-with-bogem-id3v2-and-a-hand-built-ctoc-frame.md) |
| 13 | Change-driven re-compare indicator | Lua `project_state`, baseline stored at prepare time, "changed since comparison" label; no auto-run | partial | 11, 12 | 2 | - |
...
- **Status: partial.** Delivered: `integrations/reaper/narration_render.lua` registers `configure_chapter_render` through the Phase 4 registry (`FEATURE_FILES`, `scripts/release/reaper-files.mjs`, `apps/desktop/smoke.go`'s `smokeReaperFiles`). It sets `RENDER_FILE`, `RENDER_PATTERN` (`$region`) and `RENDER_BOUNDSFLAG` (3, "all regions" - the S5 spike's confirmed values), reads `RENDER_TARGETS` back, and reports it on `RENDER_CONFIGURED`; `RENDER_FORMAT` is never written. 9 new harness tests in `integrations/reaper/tests/render_test.lua` (139 total across the whole harness, 0 failed) cover the missing-API and missing-folder errors, the exact keys set, the predicted file names (with and without existing regions), that the output folder is created, and - the load-bearing one - that the command never calls `Main_OnCommand` and never reads `RENDER_STATS`/`RENDER_STATS_SUMMARY` (the S5 spike's footgun: a real render action's ID passed to that getter re-triggers a render). 2 new mutation entries in `mutations.json` (31 of 31 project-wide caught, 0 survivors) pin the empty-folder and missing-API guards. `apps/desktop/internal/renderconfig` (new package, mirroring `pickups.Service`'s pattern: `Configure`/`Drain`/`Snapshot`/`Handle` on the same bridge client, the fan-out's fourth real consumer) wraps the command; 11 Go tests (exact bridge command, the empty-folder and no-bridge errors, the target list and zero-region message, fan-out sharing, error attribution), 2 contract fixtures (`render-config-idle.json`, `render-config-success.json`). `RENDER_CONFIGURED` joined the wire-contract table (`internal/bridge/wire.go`). `hostAPIVersion` 22 to 23 (`RenderConfigConfigure`, `RenderConfigSuggestFolder`, `RenderConfigState`; hand-updated `wailsjs/go/main/Host.{js,d.ts}` in the same shape `wails generate module` produces, since the Wails CLI is not available in this environment - regenerate for real on the next build that has it). UI: `apps/ui/src/components/tracks/RenderConfigDialog.tsx` ("Prepare chapter render…" next to "Link chapters…" and "Pickups…" on the Tracks page, no new nav item), with the full wire-contract chain (schema, contracts, `wailsClient.ts`, `mockApi.ts`, a `wireContracts.test.ts` row, golden fixtures) and a `?mockRenderConfig=success|no-regions|error` seam. On success it shows the predicted file names and "Render is configured — press Render in REAPER (Ctrl+Alt+R or File > Render) to create the files."; nothing in the Lua, Go or TS layers ever calls a render action. Four new visual states (`tracks/render-config-prefilled`, `-success`, `-no-regions`, `-error`) reviewed at desktop, small-desktop and tablet (12 captures, axe gate 0 violations). **Not delivered in this pass**: the manual REAPER checklist step (actually pressing Render after a real configure and confirming one correctly named file per chapter region, and that nothing else in the project changed) needs the owner and a real REAPER session; tracked under Phase 1's checklist scope, since this phase's own scope is the Lua/Go/UI configure path, which the harness and Go tests already prove does not call any render action.*
...
- **Status: complete, with a scope correction from evidence (see [ADR 0099](../adr/0099-chapter-tags-are-embedded-with-bogem-id3v2-and-a-hand-built-ctoc-frame.md)).** Phase 11 never writes `RENDER_FORMAT` (Open Question 7) and the S5 spike's own scratch run recorded that default as WAV, and Phase 11 renders one file *per chapter region*, not one combined file with internal chapters - so "embed CHAP/CTOC in the rendered MP3s" as literally written does not fit what Phase 11 actually produces (each per-chapter file already is one chapter; a single-entry CHAP spanning a whole file adds nothing a filename does not). This phase instead targets the shape ID3v2 chapters are for: a separate, already-rendered *combined* MP3 the narrator names, tagged from Phase 11's known chapter titles and the per-chapter files' durations (summed sequentially into a timeline) - never the per-chapter files themselves. `apps/desktop/internal/chaptertags` (`mp3.go`: a frame-summing MPEG duration estimate; `chaptertags.go`: `BuildTimeline`, `Embed`) uses `github.com/bogem/id3v2/v2` (MIT, AGPL-3.0-or-later compatible per D17) for the tag and its built-in `ChapterFrame`, with the `CTOC` frame body hand-built (the library has no CTOC type) and round-tripped in tests; `Embed` always writes a NEW file (`<name>.chapters.mp3`) beside the narrator's chosen file and never touches it or the per-chapter files. 17 Go tests (round trip via `ChapterFrame`/`UnknownFrame` re-parsing, idempotent re-run, non-MP3 and missing-file rejection, timeline sequencing) plus a real `ffprobe -show_chapters` check against a generated fixture (titles and millisecond-accurate start/end times both matched exactly) on two tiny synthetic MP3 fixtures (`internal/chaptertags/testdata/*.mp3`, generated with ffmpeg, committed); 84.1% coverage (floor added to `scripts/ci/coverage-floors.json` at 84). Host bindings `ChapterTagsPreview`/`ChapterTagsEmbed` (`hostAPIVersion` 23 to 24) read the render-config service's last successful configure for chapter titles and files, reporting which are rendered yet; 10 Go binding tests plus 3 golden contract fixtures. UI: `apps/ui/src/components/tracks/ChapterTagsDialog.tsx` ("Embed chapter tags…" next to "Prepare chapter render…" on the Tracks page, no new nav item), gated behind a destination-path field and an explicit "writes a new file, the original is not changed" confirm checkbox before the action enables; full wire-contract chain (schema, contracts, `wailsClient.ts`, `mockApi.ts`, `wireContracts.test.ts` rows, golden fixtures, `interactionFeedback.catalog.ts` rows) and a `?mockChapterTags=ready|not-rendered` / `?mockChapterTagsEmbedError=1` mock seam. Five new visual states (`tracks/chapter-tags-idle`, `-ready`, `-not-rendered`, `-success`, `-error`) reviewed at desktop, small-desktop and tablet (15 captures, axe gate 0 violations). **Not delivered in this pass**: WAV chapter tagging (out of scope for this ADR; ID3v2 is MP3-only) and a manual REAPER/third-party-player round trip on a real narrator MP3 - needs the owner, tracked as pending, since this phase's own scope (the Go writer, host bindings, UI and fixture-based tests) is fully proven by the fixture round trip and `ffprobe`.
...
- **Status: partial.** Delivered: `integrations/reaper/narration_project_state.lua` registers `project_state` through the Phase 4 registry (`FEATURE_FILES`, `scripts/release/reaper-files.mjs`, `apps/desktop/smoke.go`'s `smokeReaperFiles`); it reads `GetProjectStateChangeCount(0)` and the current project's saved-file path (`EnumProjects(-1, '')`, the same call `prepare_compare` already makes), mutates nothing (no undo block) and reads nothing else. 6 new harness tests in `integrations/reaper/tests/project_state_test.lua` (145 total across the whole harness, 0 failed) cover the missing-API error, the reported count and path, an empty path for a never-saved project, a later, higher count once REAPER records more edits, and that the command never opens an undo block; 2 new mutation entries in `mutations.json` (33 of 33 project-wide caught, 0 survivors) pin the API guard and the real-count read. `apps/desktop/internal/projectstate` (new package, mirroring `renderconfig.Service`'s pattern: `Check`/`Drain`/`Snapshot`/`Handle` on the same bridge client, the fan-out's fifth real consumer) wraps the command and stats the reported project path itself for the saved file's modification time (Lua has no portable file-stat call); a pure `ChangedSince(current, baseline int) bool` comparison is exported for a caller with its own baseline. `PROJECT_STATE` joined the wire-contract table (`internal/bridge/wire.go`). 12 Go tests (exact bridge command, the stat of a real saved file, a never-saved and a missing-file path, fan-out sharing, error attribution, `handleInvalid`, `ChangedSince`), 92.5% coverage (floor added to `scripts/ci/coverage-floors.json` at 92). `hostAPIVersion` 24 to 25 (`ProjectStateCheck`, `ProjectStateChangedSince`, `ProjectStateState`; hand-updated `wailsjs/go/main/Host.{js,d.ts}` in the shape `wails generate module` produces, the same seam Phase 11 used). Deliberately not wired into `apps/ui`: no schema, mock or `wireContracts.test.ts` row, since nothing in the frontend calls these bindings yet, the same deliberate gap Phase 6 (line identity) left for its own UI phase. **Not delivered in this pass, and a scope correction from evidence**: this phase's own text says the baseline is "stored at `prepare_compare`" and the label is "changed since comparison" (Transcript Compare); the task that dispatched this phase instead asked for it to be wired as an additional live hint into `analysis-evidence-ledger.prd.md`'s staleness evaluator (its own Open Question 12, answered (B), names the identical Could-tier design). Neither integration is delivered: `apps/desktop/internal/transcript/service.go`'s `Start` (which calls `prepare_compare`) was not touched, to avoid modifying a complete phase's tested code without a concrete UI consumer driving the change; and `analysis-evidence-ledger.prd.md`'s staleness evaluator (its own Phase 6, `EvaluateChapter`) does not exist yet in code (`apps/desktop/internal/evidence` holds only `identity.go`; that PRD's Phase 6 row reads `pending`), so there is nothing there to add a hint alongside. What ships here is the complete, tested Lua/Go/binding plumbing both integrations need - `Check`, `Snapshot` and `ChangedSince` - left available for whichever lands first: this PRD's own Transcript Compare baseline, or analysis-evidence-ledger's Phase 8 once its own Phase 6 exists. Manual REAPER verification of `GetProjectStateChangeCount`'s real behavior stays pending the owner, tracked under Phase 1's checklist scope.
...
| Chapter tag library and scope (phase 12, ADR 0099) | `github.com/bogem/id3v2/v2` (MIT), CTOC hand-built; targets a separate narrator-named combined MP3, not the per-chapter renders | `sa6mwa/id3v24` wrapper; hand-rolled ID3v2 writer; tagging the per-chapter files themselves | Most-used Go ID3v2 library, one dependency, AGPL-3.0-or-later compatible (D17); per-chapter files are already one chapter each and Phase 11 never guarantees MP3 output |
...
*Status: IN DELIVERY - part 1 (phases 1 to 5) is delivered by stack S09, with the two owner-only checklist steps pending; stack S22 (phases 6 to 25) is in progress: phases 6, 8, 9 and 12 are complete, phases 7, 11 and 13 are partial (see their rows), phases 14 to 25 are queued*
