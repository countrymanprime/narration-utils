...
| 8 | Pickup list: Lua commands | `import_pickups`, `export_pickups`, `next_pickup`, `resolve_pickup`, `count_pickups`; harness tests or manual checklist | pending | 6, 7 | 1, 2, 4 | - |
...
| 10 | Spike S5: render details (approved, D3) | Verify `RENDER_STATS` key format, marker-to-chapter mapping, `$region` naming, current action IDs, dry-run behavior | pending | 6, 7, 8, 9 | D3 | - |
...
*Status: IN DELIVERY - part 1 (phases 1 to 5) is delivered by stack S09, with the two owner-only checklist steps pending; stack S22 (phases 6 to 25) is starting: phase 6 (the line-identity Go client) is complete; phases 7 to 25 are queued*
