...
- **The static project model cannot list takes.** `parseItem` reads `POSITION`, `LENGTH`, `NAME` and the first `<SOURCE>` only (`shell/internal/tracks/parse.go:68-96`); `Item` has no GUID, source offset, playrate or take list (`tracks.go:18-26`); the docs list "only the first occurrence of an attribute per chunk is read" as a known limit (`docs/utilities/tracks.md`, Known limits). The only fixture, `shell/internal/tracks/testdata/basic.rpp`, contains no `TAKE` or `IGUID` line (grep count 0), so multi-take parsing has no sample and its serialization is unverified here (TBD - needs a REAPER-saved multi-take project).
- **`/media` would not serve alternate takes.** `authorizedMediaPath` allows only the source of each item's first `<SOURCE>` (`shell/media.go:51-64` over `parse.go:73`), so a second take that points at a different file is refused today.
...
- **Bridge and settings plumbing** the phases reuse: the bridge client exists only when launched from REAPER (`shell/app.go:169-173`); layered settings resolve project, global, repo default (`shell/internal/settings/store.go:25-50`) with built-in defaults that must mirror `shared/config/defaults.json` (`store.go:38-42` comment).
...
- [ ] **Q4. How is a take created?** Options: (A) add the candidate's source range as a new take on the target item; (B) place the candidate on a fixed lane or a duplicated item; (C) do not create anything, only navigate. Recommendation: decide after the phase 1 spike; provisional A because both docs say "attach an approved candidate as a take in a target item" and the roadmap says "group reviewed duplicate candidates as REAPER takes". The spike must settle: which take becomes active on add, how a candidate longer or shorter than the target item is handled (item length must not silently change), source-offset alignment to the manuscript span start, and one-step undo.
- [ ] **Q5. Where is take provenance stored?** Options: (A) namespaced take extension data (`P_EXT`) plus the finding id in the review sidecar; (B) sidecar only; (C) take name or item notes. Recommendation: A. C is ruled out because item notes and take names belong to the narrator (ADR 0026); B alone drifts when takes move or copy, the very problem ADR 0026 solved for line identity. Needs a new ADR that extends, not supersedes, ADR 0026 (take-level keys), verified in the spike.
- [ ] **Q6. Does this milestone require manuscript line identity stamps?** Options: (A) require stamps before scanning; (B) do not require them: spans come from alignment at analysis time, and stamped ids only pre-select a target item when present; (C) this PRD builds the Go client for stamps. Recommendation: B. It removes a dependency on an unverified, user-run REAPER checklist and on `reaper-automation-follow-through.prd.md`; the item GUID is already the explicit target identity the roadmap demands. If the narrator wants stamps, the Go client belongs to the reaper follow-through PRD.
...
- **Metrics.** `shell/internal/measure` gains a range-limited entry point for WAV sources; clipping and noise from samples, level consistency against neighboring items, pause profile and duration from words. MP3, FLAC and others report "unavailable" rather than a guess (ADR 0025 stance).
...
- **Success signal**: Fixture tests for multi-take, trimmed section, playrate; media route refuses unrelated paths; `tracks` page unchanged.
...
- **Scope**: Bindings (start, state, cancel), scan dialog with scope (Q3) and real progress, candidate group detail in the Review page, navigate to target and candidate using milestone 1's GUID navigation, host API bump, visual states, docs and screenshots (the scan and group-review states extend the Review guide page, `docs/guides/using-the-app/review.md`, added by `RD-5`; `shared/ui/src/docsGuide.test.ts` guards the index and footers).
...
| 2 | `shell/internal/tracks/{tracks.go,parse.go,chunk.go}` and testdata, `shell/media.go`, `docs/utilities/tracks.md` | Teleprompter phase 8 edits the same parse functions for `SOFFS`/`PLAYRATE`; any Tracks page work |
| 3 | `tools/transcript-compare/core/compare.py` (or a new module it imports), `tools/transcript-compare/core/tests/*` | Teleprompter phase 6 may touch `compare.py` for shared matching; keep new logic in a new module |
| 4 | New `shell/internal/*` scan service and adapter, findings store use | Any session changing the store API (`RD-1`), diagnostics findings work |
| 5 | `shell/bindings.go`, `shell/app.go` (host API), `shell/app_test.go`, `shared/ui/src/{hostApi.ts,api/*,components/review/*}`, visual catalog, docs images | `RD-4`, `RD-5`, `RD-7` and every binding phase; host API number collisions |
| 6 | `shared/reaper/narration_ui_bridge.lua`, Go executor, review UI confirm dialog | Every Lua PR; dispatcher chain at `:522-553`; user's REAPER availability for the checklist |
...
| 8 | `shell/internal/measure/*` | `diagnostics-delivery-and-cleanup-tools.prd.md` exposes and extends the same package; agree the range API once |
| 9 | Sidecar mode module, tests | Phase 3 module; keep separate files |
| 10 | Review UI, findings adapter, `docs/roadmap.md`, `shared/config/roadmap.json`, docs | Any milestone-status edit; roadmap files change together and alone |
