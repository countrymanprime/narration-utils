...
- [x] **Q1. What signal detects a repeated read?** Decided (phase 3): **A**, transcript-to-manuscript alignment of every read in scope (`find_repeated_spans` in `sidecars/transcript-compare/core/compare.py`), grouping reads whose aligned manuscript span overlaps, plus exact-copy detection from item source identity (`detect_exact_copies`, resolved source file plus source range). C (Silero VAD) is not evaluated by this phase; B (acoustic similarity) remains deferred per the recommendation.
- [x] **Q2. Where does the repeated-span detector run?** Decided (phase 3): **A**, an additive `--find-repeats` mode of the Python Transcript Compare sidecar (`sidecars/transcript-compare/core/compare.py`), reusing its existing tokenization, homophone/number-word equivalence and manuscript diff (`diff_and_build_markers`) rather than duplicating them, and emitting new additive `SPAN_GROUP`/`SPAN_MEMBER` tagged lines. A minimal Go adapter (`apps/desktop/internal/repeats`) parses that output into `findings.Finding` records; it is not wired into a findings store, scan service or binding yet (phase 4/5).
- [x] **Q3. What is the scan scope?** Decided (phase 4): **B**, the chapter track's items and all takes of those items, plus at most one narrator-designated pickup track (every item on it) or time range (every item on any track whose position overlaps it), never both. `internal/takereview.Scope`/`buildManifest` implement it; `ResolvePickupScope` reads the pickup addition from layered project settings (Q12), the chapter track name is supplied per scan. A is still reachable (the two settings default empty). C remains the documented cross-session "later work".
...
- [x] **Q7. How is side-by-side audition delivered?** Options: (A) in the app, two raw source ranges with the target and each candidate played from their own source plus pre/post roll via `/media`, no REAPER mutation, works standalone; (B) in REAPER, looping the timeline context and temporarily switching the active take (restored afterwards); (C) both. Recommendation: A first. It needs no REAPER state change and respects "no automatic active-take change"; it must be labeled "raw source, no FX or edits" because processing chains may differ (the doc names that risk). B is a later Should once the milestone 1 loop plumbing is proven. A needs take-aware source ranges from the static reader (phase 2) and the `/media` authorization change.
  **Settled:** (A), by Phase 7 (PR #358, commit 818b6d43): in-app raw-source audition over `/media`, no REAPER mutation.
- [x] **Q8. Does the app set the active take?** Options: (A) never, the narrator does it in REAPER; (B) an explicit, confirmed, undoable "Make active" action. Recommendation: A for this milestone (smaller Lua surface, matches the workflow text "choose the active take manually"); B as a later addition using the executor from `RD-8`.
  **Settled:** (A) for this milestone, by Phase 6: `integrations/reaper/narration_take_review.lua` never calls `SetActiveTake`. (B) stays a later addition.
- [x] **Q9. Any composite score at all?** Options: (A) none, per-category evidence only; (B) an optional narrator-weighted composite; (C) a fixed composite. Recommendation: A. The take-intelligence doc lists weights as an input but its MVP is separate evidence; B waits for accumulated narrator decisions to calibrate, C is what the doc warns against.
  **Settled:** (A), by Phase 5: `TakeReviewPanel` shows category, evidence kind, manuscript location and coverage, with no ranking column.
- [x] **Q10. How precisely can divergence be localized inside a take?** Options: (A) diff plus ASR word timestamps per take (ADR 0008's approach); (B) forced alignment (new PyTorch or Kaldi dependency, new ADR superseding 0008). Recommendation: A, with an explicit evaluation gate on the annotated set; escalate to B only with evidence that word-level timing is too coarse to show the divergent sub-span, exactly the condition ADR 0008 states.
  **Answered 2026-09-23:** (A) diff plus ASR word timestamps (ADR 0008), but the evaluation gate runs on synthetic, constructed fixtures instead of an owner-supplied annotated corpus. That is weaker evidence than real annotated chapters; (B) forced alignment, superseding ADR 0008, is considered only if the fixtures show word timing is too coarse. Phase 9 carries it.
- [x] **Q11. How do restarts map onto the fixed category list?** Decided (phase 4): **A**, category `pickup` with `evidence.kind` of `restart`, `pickup` or `exact_copy`, and `duplicate_read` (`evidence.kind` `near_duplicate`) for a near-identical re-read: every member fully covers the span *and* every member's own manuscript match quality clears the Q12 near-duplicate-quality threshold. `internal/repeats.classify` implements this; phase 3's interim implementation had the exact_copy/restart cases backwards (mapped to `duplicate_read`) since Q11 was still open when it was written - this phase corrects that and updates its tests. `Validate` still rejects unknown categories (`findings.go:138`); option B was not adopted.
- [x] **Q12. Where do detection thresholds live?** Decided (phase 4): **A**, layered project settings (`TakeReview` tool: `full_coverage_threshold`, `near_duplicate_quality_threshold`, and the Q3 pickup-scope settings `pickup_track_name`/`pickup_range_start_seconds`/`pickup_range_end_seconds`), conservative built-in defaults mirrored in `config/defaults.json` and `apps/desktop/internal/settings/store.go`'s `builtinDefaults`. `internal/takereview.ResolveThresholds`/`ResolvePickupScope` read them; an unparseable or out-of-range stored value falls back to the built-in default rather than failing a scan.
...
| 3 | Timestamped words and repeated-span detector | Sidecar mode: cached transcription per range, persisted words, alignment to manuscript, span clustering, additive tagged output; fixtures and pytest | complete | 1, 2 | - | - |
| 4 | Pickup and duplicate findings | Go scan service and adapter producing `pickup`/`duplicate_read` findings with evidence and `create_take` suggestion; exact-copy detection from project model | complete | 8, 9 | 2, 3, RD-1 | - |
| 5 | Scan and review surface | Bindings, scan dialog, candidate group detail, navigate to target and candidate via milestone 1 plumbing, host API bump | partial | 8, 9 | 4, RD-4, RD-5, RD-6 | - |
| 6 | Take creation action | Lua `create_take` per spike, Go executor, target-item selection and confirm dialog, provenance, undo; manual checklist | complete | 7, 8, 9 | 1, 5, RD-8 (soft) | - |
| 7 | Side-by-side audition | In-app A/B component over `/media` with loop and pre/post roll, keyboard, "raw source" label, visual states | complete | 6, 8, 9 | 2, 5 | - |
...
- **Delivered**: `internal/repeats.classify`/`ToFindings` corrected for Q11's category mapping (adopted this phase; phase 3's interim mapping had exact_copy/restart backwards) and given a `Thresholds` parameter (Q12) plus a computed `EvidenceVersion` per finding (kind and each member's own coverage/quality, excluding identity, so a re-scan that changes the evidence - not just re-finds the same members - returns any prior decision to unreviewed rather than keeping it silently). New `internal/takereview` scan service (Q3): `buildManifest` assembles the sidecar's `--find-repeats` manifest from the chapter track's items and takes plus an optional narrator-designated pickup track or time range; `Scanner.Scan` runs the sidecar (`SidecarRunner`, real `ProcessRunner` plus a fake for tests), parses and adapts its output, and calls `findings.Store.SaveAnalyzerFindings("take-review", <chapter id>, ...)`; `ResolveThresholds`/`ResolvePickupScope` read Q12's layered `TakeReview` settings (`config/defaults.json`, `apps/desktop/internal/settings/store.go`'s `builtinDefaults`) with a graceful fallback to the built-in default on an unparseable or out-of-range value. No bindings, scan dialog or Review page surface (phase 5).
- **Success signal**: Service tests with a fake sidecar; finding ids stable across re-scans. Met: `internal/repeats` and `internal/takereview` both green (`go build ./...`, `go vet ./...`, `go test ./...`, `golangci-lint run ./...` all clean); `TestScanSavesFreshFindingsIntoTheStoreUnderTheChapterScope` proves the store round-trip; `TestToFindingsIDIsStableAcrossRescansAndDiffersWhenMembersDiffer` and `TestToFindingsEvidenceVersionChangesWhenMemberQualityChangesButIDDoesNot` cover ID stability and evidence-version drift separately.
...
- **Delivered (partial)**: `RD-5`'s own Review page does not exist yet (`review-dashboard-and-findings-adoption.prd.md` has only reached its Phase 3 Guide adapter on a separate branch chain at the time this phase shipped), so this phase does not build against it and does not duplicate a generic findings-browsing UI. Instead it ships take-review's own scan trigger and results view on the Tracks page: `Host.TakeReviewScan(chapterTrackName)` (runs the phase 4 `takereview.Scanner` to completion and saves fresh findings) and `Host.TakeReviewFindings(chapterTrackName)` (reads the store back without scanning), `hostAPIVersion` 25 → 26, full TS wire contracts (`apps/ui/src/api/contracts/takeReview.ts`, `apps/ui/src/api/schemas/takeReview.ts`, golden fixture `tests/fixtures/contracts/takereview-findings.json`, mock and `wireContracts.test.ts` row), and `TakeReviewPanel` (category, evidence kind, manuscript location, coverage - no composite score, per Q9) rendered under the active track. Deferred to a later pass once `RD-5` lands: a scan dialog with real progress/cancel (this binding runs synchronously to completion), the pickup-track/time-range scope selector's own UI (Q3's settings already resolve server-side; there is no picker for them yet), GUID-based navigate-to-target/candidate, and Review-page integration.
- **Success signal**: End-to-end on a real chapter; visual suite and atlas green with four-viewport PNG review. Met for the delivered scope: `apps/desktop/takereview_test.go` and `apps/desktop/contract_test.go`'s `TestContractTakeReviewFindings` exercise the binding end to end against a fixture project with a fake sidecar runner; `apps/ui/src/components/tracks/TakeReviewPanel.test.tsx` and the Playwright visual suite (`tracks / take-review-results` and `tracks / take-review-empty`, all three viewports, axe clean) cover the UI. No primitive or `styles.css` change, so the atlas was not re-run.
...
- **Delivered**: `create_take` (`integrations/reaper/narration_take_review.lua`), a new feature file loaded by the bridge per the spike's exact sequence (`AddTakeToMediaItem`, `SetMediaItemTake_Source`, `D_STARTOFFS`, the three `P_EXT` provenance keys, one `Undo_BeginBlock2`/`Undo_EndBlock2` block, re-resolves the target item and new take by GUID afterward) - covered by `integrations/reaper/tests/take_review_test.lua` against the fake `reaper` (harness). `internal/takereview.CreateTake` (`apps/desktop/internal/takereview/createtake.go`) sends the command over the bridge and waits for `TAKE_CREATED`/`TAKE_STALE`/`ERROR`, writing the request as a one-row payload file (the wire protocol's eight-field cap forced this, the same convention `stamp_item_lines`/`create_chapter_regions` use). `Host.TakeReviewCreateTake` (`hostAPIVersion` 26 -> 27) exposes it; `TakeReviewPanel`'s "Add as take" action lets the narrator pick the target item and the candidate from the finding's own members (never preselected, Q4/Q8) and confirms via `ConfirmDialog` before REAPER does anything.
- **Success signal**: Met. Lua harness (`take_review_test.lua`, 9 cases: missing API, missing payload, stale target, stale candidate, missing source file, successful creation with D_STARTOFFS/provenance/active-take/length assertions, no item-level collision, repeatable). A scripted real-REAPER run (`integrations/reaper/spikes/take_review_smoke.lua`, REAPER 7.80, against a scratch copy of `Challenges_001.rpp` plus its referenced media, never the original folder) proved against the real API: (a) the previously active take stayed active, (b) the target item's `D_LENGTH` was unchanged, (c) the new take's `P_EXT` provenance survived an `Undo`/`Redo` cycle once re-resolved by GUID (the phase 1 spike's specific warning). Notes and item names were never touched (Lua command writes no `P_NOTES`/take name anywhere; harness case). Split/duplicate-of-a-take-provenanced-item (ADR 0098's own flagged open item) was not re-tested here and stays open for a future pass.
...
- **Delivered**: Q7 option A, in-app, no REAPER mutation. `apps/ui/src/components/tracks/useRangePlayer.ts` plays one member's raw source range (`source_file`/`source_start`/`source_length`, already on the wire from phase 5) through the same `/media` route `useTrackPlayback` streams from (`api.mediaUrl`, unchanged - phase 2's `authorizedMediaSource` already authorizes every take's own source), with a fixed `AUDITION_PRE_ROLL_SECONDS`/`AUDITION_POST_ROLL_SECONDS` (1.5s each) lead-in and trail-off and an optional loop back to the pre-roll start; it stops itself at `rangeEnd + post-roll` instead of playing the rest of the source file. `apps/ui/src/components/tracks/AuditionDialog.tsx` runs two independent instances (Read A / Read B, pickers over the finding's own members, defaulting to the first two), starting one always stops the other so a narrator compares rather than layers two streams, each side has its own Play/Pause and Loop control (native `<button>`/`<Checkbox>`, so both are keyboard-operable without a bespoke shortcut layer) and its own load-error state, and the dialog states "Raw source, no FX or edits applied" plus why (REAPER's processing chain is not applied) per the PRD's own labeling requirement. `TakeReviewPanel` gained a per-finding "Audition" action (available whenever a finding has two or more members, independent of `canCreateTake`'s `suggested_action` gate - auditioning never proposes or needs a REAPER action). No new host binding, wire contract or `hostAPIVersion` bump: the feature is pure playback over data and a route the app already has.
- **Success signal**: Met. `apps/ui/src/components/tracks/useRangePlayer.test.tsx` (7 cases: pre-roll seek and clamp to zero, self-stop at the post-roll boundary, loop-back, load-error, `stop()`, a pre-any-play timeupdate is a no-op) and `AuditionDialog.test.tsx` (6 cases: raw-source label, default A/B assignment, playing one side stops the other, play/pause toggle, per-side load-error, dialog close) are green; `TakeReviewPanel.test.tsx` gained a case proving the Audition action opens the dialog without calling `takeReviewCreateTake`. Playwright visual suite state `tracks / take-review-audition` added to `state-catalog.ts`/`app.drivers.ts` and reviewed at `desktop`, `small-desktop` and `tablet` (no phone viewport, ADR 0037) - axe clean, no unintended `sameAs` collapse. No primitive or `styles.css` change, so the atlas was not re-run.
...
- **Scope**: Additive sidecar mode; per-word fidelity for one range against a fixed manuscript span; output consumed by phase 10; evaluate Q10 on synthetic, constructed fixtures (owner decision 2026-09-23: no owner-supplied annotated corpus), for example takes built with a flub, restart or substitution at a known word partway through. The evaluation write-up records that synthetic fixtures are weaker evidence than real annotated chapters (clean synthetic audio may time words better than a real session does).
- **Success signal**: Fixture takes with a flub partway localize to the correct sub-span; evaluation result recorded (adopt A; a move to forced alignment, superseding ADR 0008, is considered only if the fixtures show word timing is too coarse to show the divergent sub-span).
...
| Detection basis (decided, Q1, phase 3) | Transcript-to-manuscript alignment (`find_repeated_spans`) plus exact-copy detection from source identity (`detect_exact_copies`) | Acoustic similarity | Reuses shipped ASR and diff; no new dependency |
| Detector location (decided, Q2, phase 3) | Additive `--find-repeats` sidecar mode (`compare.py`), reusing its tokenization/equivalence/diff, plus a minimal Go adapter (`apps/desktop/internal/repeats`) not yet wired to any store or binding | Go analyzer, new sidecar | Shared tokenization lives in Python; a Go copy would drift (Q2 evidence) |
| Scan scope (decided, Q3, phase 4) | The chapter track's items and all takes, plus at most one narrator-designated pickup track or time range, from layered project settings (`internal/takereview.Scope`/`ResolvePickupScope`) | Chapter track only; whole-project scan | Pickups are often on a separate track or later in the timeline; whole-project multiplies ASR time and invites false matches |
| Category mapping (decided, Q11, phase 4) | Category `pickup` for `evidence.kind` `restart`/`pickup`/`exact_copy`; category `duplicate_read` for `evidence.kind` `near_duplicate` (full coverage and every member's quality above the Q12 near-duplicate-quality threshold) | A new `restart` category | `Validate` rejects unknown categories; phase 3's interim mapping had exact_copy/restart backwards and is corrected here |
| Detection thresholds (decided, Q12, phase 4) | Layered project settings (`TakeReview` tool, `internal/takereview.ResolveThresholds`/`ResolvePickupScope`), conservative built-in defaults in `config/defaults.json` | Sidecar constants | Narrators with repetitive prose need per-project tuning; an unparseable stored value falls back to the built-in default rather than failing a scan |
| Audition (decided, Q7, phase 7) | In-app raw-source A/B first (PR #358) | REAPER-side | No project mutation, standalone; owner confirmed 2026-09-23 |
| Active take (decided, Q8, phase 6) | Narrator sets it in REAPER; `narration_take_review.lua` never calls `SetActiveTake`; an explicit "Make active" action later | App action this milestone | Narrower Lua surface; owner confirmed 2026-09-23 |
| Composite score (decided, Q9, phase 5) | None; `TakeReviewPanel` has no ranking column | Weighted or fixed | Over-trust risk; owner confirmed 2026-09-23 |
| Divergence localization evidence (decided, Q10) | Diff plus ASR word timestamps (ADR 0008), evaluated on synthetic, constructed fixtures | Owner-supplied annotated corpus; forced alignment now | Owner decision 2026-09-23; weaker evidence than real chapters, accepted; forced alignment only if fixtures show word timing too coarse |
