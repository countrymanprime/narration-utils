...
- **The static project model cannot list takes.** `parseItem` reads `POSITION`, `LENGTH`, `NAME` and the first `<SOURCE>` only (`apps/desktop/internal/tracks/parse.go:68-96`); `Item` has no GUID, source offset, playrate or take list (`tracks.go:18-26`); the docs list "only the first occurrence of an attribute per chunk is read" as a known limit (`docs/utilities/tracks.md`, Known limits). The only fixture, `apps/desktop/internal/tracks/testdata/basic.rpp`, contains no `TAKE` or `IGUID` line (grep count 0), so multi-take parsing has no sample and its serialization is unverified here (TBD - needs a REAPER-saved multi-take project).
