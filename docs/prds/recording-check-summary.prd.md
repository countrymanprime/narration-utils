...
| 4 | Move the summary | Mount it in the surface RS7 chooses once the per-row Check button is retired | complete (lane C, stream C7): `RecordingCheck.tsx` is a `SlideOver` instead of a `Dialog`, opened from Home's per-row check-status cell (`daw-chapter-track-auto-sync.prd.md` Phase 6); its one action moved from the dialog's action bar into the body via `RecordingCheckReport`'s new `actionsSlot`; aria snapshot renamed `slide-over-recording-check.aria.yml` ([ADR 0264](../adr/0264-the-recording-check-summary-moves-into-a-slide-over-opened-from-the-rows-own-status-and-the-check-button-retires.md)) | - | 1; `daw-chapter-track-auto-sync.prd.md`, `chapter-track-link-control.prd.md`; RS7 | - |
