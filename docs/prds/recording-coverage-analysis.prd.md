...
- **Sidecar mode.** (As delivered in Phase 3, [ADR 0127](../adr/0127-the-coverage-sidecar-mode-reads-a-json-manifest-keeps-one-words-file-per-item-and-writes-tagged-json-lines.md): the mode is `core/coverage_mode.py` over the model `core/recording_coverage.py`, renamed from `coverage.py` so it does not shadow the `coverage` package; the manifest is JSON with `index`, `itemGuid`, `sourceFile`, `startOffset`, `length`, `wordsFile` and `muted` per item; words files are named inside `--words-dir`; chunking is not supported. The original proposal follows.) New module `sidecars/transcript-compare/core/coverage.py` imported by `compare.py`, entered by an additive flag in the same argparse block as `--extract-hints` (`compare.py:1598-1626`); `run()` and the marker path are untouched. Input: the manifest (`index|source_file|startoffs|length` per item plus a words-cache path per item), the manuscript, a chapter id (the `--chapter-title` bypass at `:1463-1468` is the precedent for skipping name matching), alignment parameters. Behavior: for each item, reuse the words file if it exists, else decode (`decode_segment`, `:386`) and transcribe (`transcribe` or `transcribe_chunked`) and write it atomically (temporary file then rename), so a cancelled run keeps finished items. Then align, then write results.
- **Words file.** (As delivered: times are source seconds and the file records the source range it transcribed, so a words file covering a wider range is reused and sliced to the played range, which suits EL's whole-source cache, Q5 A; ADR 0127.) One JSON per item: schema version, words as `(word, start, end)` with times relative to the played range start, and the parameters that shaped them (model, language, hotwords hash from `vocabulary_hints.txt` at `compare.py:1478-1485`, VAD flag, chunk parameters). The file name is the cache key computed by Go (D8: source identity, played range, analyzer version, parameter hash), so the sidecar never interprets key semantics. Storage location, retention and `resetDerived` handling are EL's (proposed `narration-utils/analysis/`).
- **Output.** A results file separate from the compare results, one JSON payload per line after a tag: `COVERAGE|{chapter summary}`, `COVERAGE_PARAGRAPH|{...}`, `COVERAGE_REGION|{...}`, and (as delivered) `COVERAGE_ITEM|{...}` per manifest item, which lists muted items (Q10) and labels each item's words with their model and language (Q13). JSON payloads avoid the field-count hazard ADR 0008 records for `MARKER`. Existing readers ignore unknown tags (`narration_ui_bridge.lua:200-204`).
...
| 3 | Sidecar coverage mode | `--coverage` flag, per-item words files, `COVERAGE|` output, progress and cancel; hand-made manifests in tests | complete | 1 | 2 | [ADR 0127](../adr/0127-the-coverage-sidecar-mode-reads-a-json-manifest-keeps-one-words-file-per-item-and-writes-tagged-json-lines.md) |
...
- **Delivered 2026-09-23**: `compare.py --coverage` runs `sidecars/transcript-compare/core/coverage_mode.py` (ADR 0127, Proposed): a JSON manifest checked field by field, one words file per item inside `--words-dir` (reused when its source range covers the played range, else decoded, transcribed with one lazily loaded Whisper model and written atomically), the chapter by id with title and subtitle as optional heading tokens (Q11), muted items listed and skipped (Q10), and `COVERAGE`, `COVERAGE_ITEM`, `COVERAGE_PARAGRAPH` and `COVERAGE_REGION` lines with JSON payloads written atomically; counts only, no verdict. Progress is TRANSCRIBE seconds over seconds to transcribe (ADR 0015); exit codes 0, 1 and 2. `core/coverage.py` is renamed `core/recording_coverage.py`. `tests/test_coverage_mode.py` (fake transcriber, a fake Whisper model over a generated WAV, and the CLI as a subprocess) covers the all-cached run with no transcription call, one missing words file transcribed once, trims narrowing and widening a range, cancel mid-run keeping the finished file with exit 2, a bad manifest with exit 1, and identical bytes for identical inputs; the golden marker test is unchanged and green. Threat-model row 4e added.
...
| 2 | New `sidecars/transcript-compare/core/recording_coverage.py` and tests, `docs/utilities/transcript-compare.md` | TR phase 3 and TM phase 6 also add or touch modules in the same folder; keep new logic in new files |
| 3 | `compare.py` `main()` argparse block (next to `--extract-hints`), `recording_coverage.py`, `libs/python/tests` | TM phase 6, TR phases 3 and 9 edit `compare.py`; the addition is a few lines, rebase is trivial |
