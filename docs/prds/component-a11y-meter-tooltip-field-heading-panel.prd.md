...
- Not in the old register: a tooltip cannot be dismissed with Escape, and it is `pointer-events-none`, so it is not hoverable (WCAG 1.4.13 asks for dismissible, hoverable, persistent).
...
- `Field.tsx` (38 lines): no `aria-invalid`, `aria-describedby`, hint or error slot. One consumer (`GuideDetail.tsx`); its control classes are duplicated in `ScopedSetting.tsx` (`controlClass`) and `AddNoteDialog.tsx`. Assumption - needs validation: any current form actually needs an inline error (no consumer validates inline today).
- `Heading.tsx`: always `<h1>`; 7 consumers (`Home`, `Manuscript`, `Transcript`, `Settings`, `Guide`, `TeleprompterPage`, `TracksPage`); the optional subtitle `<p>` has no overflow-wrap. The old register's claim that a long unbroken subtitle overflows at 390px is plausible but unproven (`LongSubtitleWraps` uses spaces); add a story with an unbroken token before asserting it.
- `Panel.tsx`: a bare `<section>` with no name or slot; 4 consumers (`AudiobookEstimatePanel`, `Transcript`, `TeleprompterPage`, `TracksPage`). Callers hand-build an `<h2>` inside it (`Panel.stories.tsx` shows the pattern).
...
- `TooltipTarget`: `useId` per instance; the tooltip element takes that id; `aria-describedby` only while that target is the active one; the child is cloned so the attribute lands on the focusable element; positioning uses the child's rect. The disabled-button case keeps a `tabindex="0"` wrapper (the `proofing/disabled-button` driver selects it) but gains `role="group"` and a name from the tooltip text.
- `MeterBar`: required `label`; `role="img"` `aria-label` = label + ": " + segments' tooltip strings; existing `WithCaptionAndLegend` story and `AudiobookEstimatePanel` updated. ADR 0006 (caller controls segment order) is untouched.
- Tests: extend `Tooltip.test.tsx` (id now unique), add a small helper that asserts every `aria-describedby`/`aria-labelledby` id resolves; stories gain `play()` for keyboard reach. `InfoIconShowsTooltip` currently uses `fireEvent.focusIn` because the icon is not a tab stop; switch it to `userEvent.tab()`.
