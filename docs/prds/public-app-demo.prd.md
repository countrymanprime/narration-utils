...
**Architecture notes**
- Build: add a `demo` build invocation to `pages.yml`'s existing `build` job, after the current Storybook and docs-site steps, e.g. `pnpm --dir apps/ui exec vite build --mode mock --base /narration-utils/demo/ --outDir dist-demo`, then `cp -r apps/ui/dist-demo tools/docs-site/build/site/demo` (mirrors the existing `cp -r apps/ui/storybook-static tools/docs-site/build/site/storybook` at `pages.yml`'s "Add Storybook beside the docs" step). Resolve D3 (CLI flag vs. mode config) before writing this.
