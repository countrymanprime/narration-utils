...
- **The sentence is half right.** "Excluded from audiobook totals and Proofing" holds (`AudiobookEstimatePanel.tsx:63`, `tools/transcript-compare/core/compare.py:784`, `manuscript_guide.py:211`, `chapter_script.py:65`, `TeleprompterPage.tsx:151`, `shell/app.go:596`) but is narration-only, so Front Matter is excluded the same way and the sentence implies otherwise. "Stays readable" is ADR 0005 (reference material is filtered at the listing layer only, `isListableChapter`, `shared/ui/src/state.ts:7`); the ADR does not contain the sentence. It appears in no test, doc or story.
- **No grouping primitives exist.** `Panel` is a bare `<section>` (`Panel.tsx:3-4`), `Heading` is always `<h1>` (`Heading.tsx:3`), `Field` has no select variant (`Field.tsx:1`). There is no Disclosure or Badge; the only disclosure pattern is a hand-built chevron button with `aria-expanded` (`AudiobookEstimatePanel.tsx:92-100`), badge-like chips are inline classes (`EntitySummary.tsx:18`, `Home.tsx:350-357,378-382`, `Results.tsx:51`), and the uppercase legend classes are repeated in 8 files. `docs/design/design-system.md:19-26` lists only Dialog, Button, Highlight, SlideOver, Pill and MeterBar. `Tooltip` is a portaled "i" icon at `z-[1000]`, hover after 1000 ms, not keyboard-reachable (known defect, `component-a11y-meter-tooltip-field-heading-panel.prd.md`); no Tooltip sits inside a dialog today, and a native-top-layer `Dialog` (dialog PRD Phase 2) would hide it.
...
- The modal dialog overhaul (Escape, focus trap, native `<dialog>`): the dialog PRD's job.
...
- **Coordination**: `Home.tsx` is also edited by briefs PRD phases 2, 3 and 5, the dialog PRD (Phase 4 changes the `ConfirmDialog` API and `Home.tsx` call site; Phase 3 touches the import `cancel` logic), the Tooltip/Panel PRD, the interaction audit (phase 5) and `chapter-stage-recommendations.prd.md` (phase 5). Extracting the component in Step 1 shrinks that surface, and the citations to `Home.tsx:217/243/275` in the briefs PRD go stale.
...
| First Tooltip inside a dialog is clipped or hidden (z-index, top layer) | Medium | Verify against the dialog PRD status; fall back to a visible inline hint for keyboard users |
...
| 2 | `shell/internal/importer/model.go` and tests, `contracts/manuscript.ts`, review component, `doc-screenshots.json`, `home.md`, briefs PRD | Briefs PRD phase 5 (importer, contract), any change to `newDraft` |
| 3 | review component, possible new `Disclosure` primitive (`components/primitives/`, story, `docs/ui/`), Tooltip use, catalog/drivers, screenshots | Dialog PRD phases 2-4 (`ConfirmDialog` API, top layer), Tooltip/Panel PRD (icon becomes a button), palette PRD (tokens) |

Cross-cutting: `visual-catalog-sync`, the Playwright visual suite with PNG review of `shared/ui/screenshots/app/<page>/<state>/<viewport>.png` at all four viewports, `doc-screenshot-sync`, the atlas and `design-spec-guard` when a primitive is added; ADR numbering re-checked at merge time (0037 at dc9d01a); each phase follows `CLAUDE.md`: plan, `change-impact-scan`, TDD, `full-verification-gate`, `design-spec-guard`, `feature-cleanup`. No Lua, so no manual REAPER step.
