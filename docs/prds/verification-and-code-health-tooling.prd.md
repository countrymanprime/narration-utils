...
| Properties | At least 8 Python, 6 fast-check, 3 Go fuzz targets; known `validate` TypeError fixed | Test counts; the RED run recorded in the Phase 4 PR (Python: 21 delivered, and `validate` fixed plus two more `load_file` defects) |
...
| 4 | Hypothesis properties | pytest plus `hypothesis`, `validate` fix, tracker, chapter script and compare properties | complete | Yes | - | [plan](implementation-plan.md) |
...
**Phase 4 - Hypothesis.** *Delivered:* `hypothesis==6.168.0` (MPL-2.0, test-only and never imported by product code, so AGPL-compatible); 21 property tests in four files (`validate` and `load_file` 4, script tracker 9, chapter script 1, compare 7), with generators that derive the heard words from the script and corrupt one field of a valid manuscript so the jump, skip and id-lookup branches are reached (checked with `hypothesis.event` statistics); a derandomized `gate` profile and a manual `explore` profile in `libs/python/hypothesis_profiles.py`, loaded by a `conftest.py` in each tests folder that uses Hypothesis. The RED run found three defects in `narration_common.manuscript`, all fixed: `validate` raised `TypeError` on unhashable ids (known), and `load_file` let `UnicodeDecodeError` (a file that is not UTF-8) and `RecursionError` (pathologically nested JSON) escape instead of `ManuscriptError`. A 3000-example `explore` run of every property found nothing else, including `words_match` symmetry, which held. A review pass also noted that `validate` still accepts a chapter without a `title` or a paragraph without an `index`, which `chapter_script.py` and `compare.py` then index directly; that is outside this phase (the properties cover `validate`'s own promise) and is recorded here rather than fixed. Added gate time is about 1 to 2 s per Python project. *Original scope:* Goal: Python text and alignment logic under properties. Scope: `hypothesis` in `pyproject.toml`/`uv.lock`, a settings profile (derandomized), test files beside the existing ones, the `validate` guard in `manuscript.py`. Success signal: the `validate` property fails first (RED), passes after; suite time added recorded; `.hypothesis/` gitignored.
...
| `manuscript.validate` and `load_file` strictness (phase 4) | Ids (`chapters[].id`, `paragraphs[].chapterId`) must be strings; `load_file` reports non-UTF-8 and over-nested files as `ManuscriptError` and leaves `validate`'s own messages untouched | Coerce ids, catch `Exception` | The Go importer writes string ids, so a non-string id is corruption; catching everything would hide real bugs |
