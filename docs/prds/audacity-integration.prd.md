...
| Could | An Audacity-side launcher artifact (Question 3, if (a) is chosen later) | 10 |
| Won't (this cycle) | Take management, pickup lists, per-chapter render, non-Windows support | - |

### MVP Scope

Phases 1 through 7 (spikes, the adapter boundary extraction, the pipe client, config plumbing, import and mark-reviewed). Export (Phase 8) and dashboard integration (Phase 9) follow immediately after; nothing here needs a new transport, new take-marker semantics, or REAPER's bridge.
...
2. The narrator opens the app standalone (Question 3 default) with the Audacity project's folder, and enables Audacity scripting if not already on (a one-time, documented step).
...
- **Label mutation (Phases 6, 7).** `SetLabel`/`GetInfo: type=labels`/`Export2` per the scripting reference (unverified - Phase 2 spike proves or corrects this); finding identity is encoded in the label text per Open Question 4's recommendation, never inferred from position alone.
...
| 10 | Audacity-side launcher artifact (conditional) | Only if Open Question 3 is answered (a) after narrator feedback; a macro file analogous to `NarrationUtils_Launcher.lua`, added to release packaging | pending | - | 3, Question 3 | - |
...
**Phase 10 - Audacity-side launcher artifact (conditional)**
- **Goal:** close the gap Question 3 leaves open only if narrator feedback shows the standalone launch path is real friction.
- **Scope:** a macro file or equivalent, added to release packaging the way `verify-installable.mjs` lists REAPER's Lua files.
- **Success signal:** TBD - this phase does not start until Question 3 is revisited with real usage evidence.
...
| 10 | `scripts/release/verify-installable.mjs` (conditional) | Same file every Lua-and-launcher-adjacent PRD touches; add independently and rebase |
