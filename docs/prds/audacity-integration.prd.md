...
**Re-scope (2026-09-26, benchmark recommendation 9):** the [audiobook studio benchmark](../research/audiobook-studio-benchmark.md) recommendation 9 ("re-scope the adapter spike for Audacity 4.0, which rebuilt its punch-and-roll. Check that release first *(single source)*") asked this PRD's Audacity-4.x scope to be re-checked against the current release. It was checked in this session (WebSearch/WebFetch, 2026-09-26) rather than resting on the benchmark's own single secondary source; see Question 8 below and the updated Evidence and "What We're NOT Building" rows. **Outcome: no phase changes.** The version-affected material is confined to Question 8 (new), one dated confirmation added to the existing Audacity-4.x exclusion row, and a pre-flight check added to Phase 1's own spike scope; every phase's dependencies, scope and status are otherwise unchanged from the 2026-09-23 text above. Part of the [agent train](../operations/agent-train.md) (stream N-D2, wave 4), see #509.
...
- ~~Whether Audacity ships a project-open/macro mechanism comparable to a REAPER Action-list launcher script is unverified.~~ **Settled 2026-09-23 from Audacity's manual and source ([launcher feasibility note](../research/audacity-launcher-feasibility.md)):** nothing Audacity loads can start a program. No macro or scripting command does it, Audacity compiles Nyquist's `system` as a stub, and Audacity 4.0 has no macros or scripting pipe at all. The launcher is therefore an installer Start Menu entry (Phase 10).
...
- ~~How a narrator gets from "opened Audacity" to "the app's workspace is open," with no REAPER-style launcher script mechanism. Method: spike S-A3.~~ Settled by desk research instead of a spike ([launcher feasibility note](../research/audacity-launcher-feasibility.md)); Question 3.
...
- **Audacity 4.x support** - the integration targets Audacity 3.x only for now (owner decision 2026-09-23). Audacity 4.0.0 (2026-09-03) ships without the Macro Manager and the scripting pipe (its release notes, "Compatibility notes"), and every pipe-based phase (1, 2, 4, 6-8) needs that pipe. Revisit when an Audacity 4 release brings it back ([launcher feasibility note](../research/audacity-launcher-feasibility.md)). **Re-checked 2026-09-26 (recommendation 9, Question 8):** still true. No release after 4.0.0 is published as of this date ([audacity/audacity releases](https://github.com/audacity/audacity/releases)). The tracked reimplementation for the Audacity 4.1 milestone ([issue #11783](https://github.com/audacity/audacity/issues/11783), closed as a duplicate of [#9407](https://github.com/audacity/audacity/issues/9407), open, "Ready for dev") is the **Macro Manager only** - a GUI feature to build, edit and run macros against the open project or a batch of files - and neither issue mentions `mod-script-pipe` or any scripting API; it does not restore the transport Phases 1, 2, 4 and 6-8 depend on. A third-party fork, `Audacity4-MCP`, adds an unofficial scripting bridge by compiling a modified Audacity, which repeats exactly the "native module, rebuilt per minor release" risk the [launcher feasibility note](../research/audacity-launcher-feasibility.md) already found not recommended, and is not a build this app should depend on.
...
- [x] **7. Packaging.** Does a released build need to ship an Audacity-side artifact (a macro file, a Nyquist plug-in) the way `scripts/release/verify-installable.mjs` lists REAPER's Lua files by name? Depends on the answer to Question 3. Recommendation: revisit once Question 3 is answered; if (b) is chosen, no new release artifact is needed for the first slice. **Answered 2026-09-23:** yes, no longer moot because Question 3 is (a): the release ships the macro file, and Phase 10 lists it in the release check (`scripts/release/verify-installable.mjs`, which reads REAPER's file list from `scripts/release/reaper-files.mjs`) the way REAPER's Lua files are listed. **Revised by the owner 2026-09-23, following Question 3's revision:** no Audacity-side artifact ships and `verify-installable.mjs` is unchanged. The installer (`apps/desktop/build/windows/installer/project.nsi`) carries the "Narration Utils for Audacity" shortcut, `scripts/release/installer.test.mjs` pins it and its exact arguments, and the uninstaller removes it.
- [ ] **8. Does Audacity 4.x change this PRD's scope, now that a release has actually shipped and its follow-on work is tracked (recommendation 9, re-checked 2026-09-26)?** Options: (a) no change: stay Audacity 3.x only, and re-check Audacity's own release page immediately before Phase 1's spike session runs (not only at PRD-write time), since the spike needs a real 3.x install with `mod-script-pipe` present and would silently fail its premise on a narrator who has since upgraded. (b) Widen scope now to plan for the incoming 4.1 Macro Manager ([issue #11783](https://github.com/audacity/audacity/issues/11783)/[#9407](https://github.com/audacity/audacity/issues/9407)) as a second, macro-batch-only adapter path (no live navigate/mark-reviewed, only an offline import/export cycle a narrator runs by hand). (c) Depend on the unofficial `Audacity4-MCP` fork's bridge. Recommendation: **(a)**. Nothing in 4.1's tracked work restores `mod-script-pipe` or any scripting API (Evidence, re-checked 2026-09-26); the Macro Manager is a GUI batch-processing feature with no pipe-equivalent transport this PRD's Go client could drive, so (b) has no real transport to build against yet and would be speculative scaffolding (CLAUDE.md's own rule against building for a shape that does not exist); (c) repeats the native-module risk the launcher feasibility note already rejected, and depending on an unofficial fork is a supply-chain risk this app's local-only, narrator-trusted model does not take elsewhere. Phase 0's own "reconcile this PRD and the roadmap" step is where a future re-check belongs if Audacity ever does restore a scripting transport.
...
2. The narrator starts "Narration Utils for Audacity" from the Start Menu (Question 3, Phase 10) and picks the Audacity project's folder in the app, or opens the app standalone, and enables Audacity scripting if not already on (a one-time, documented step).
...
| Audacity 4.x has no scripting pipe or macros, so the adapter works only for narrators on Audacity 3.x | High (4.0.0 shipped 2026-09-03 without them) | Target Audacity 3.x only for now (owner decision 2026-09-23); the Audacity guide says so; revisit when a 4.x release restores the pipe ([launcher feasibility note](../research/audacity-launcher-feasibility.md)) |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 0 | Reconcile this PRD and the roadmap | Tick this table's phases as they land; correct `docs/roadmap.md` and `config/roadmap.json` (already done by D23 in the same PR that adds this PRD) | complete | - | - | none | - |
| 1 | Spike S-A1: `mod-script-pipe` reachability | Scripted, isolated run: **first re-check Audacity's own release page for a version past 4.0.0 that restores the scripting pipe (Question 8, recommendation 9)**, confirm the owner's Audacity install is 3.x, then enable scripting, open both pipes from a small Go program, send a known command, read the reply, measure latency, and record what happens when scripting is off or Audacity is closed | pending | 2 | owner has Audacity installed (3.x, re-confirmed at spike time) | none | - |
| 2 | Spike S-A2: label identity and drift | Add a label, move/split the underlying clip, re-scan labels, record whether the added label is still identifiable and by what signal | pending | 1 | 1 (same session) | none | - |
| 3 | `DAWAdapter` boundary extraction | Behavior-preserving Go refactor: define the interface from the REAPER bridge's existing operations; REAPER bridge becomes its first implementation; no consumer-visible change | complete | - | - | `review` (`ReviewSession`/`dawadapter.Review`, predates the formal `dawport` PRD but is the same seam `daw-port-and-capabilities.prd.md` later generalizes) | - |
| 4 | Go pipe client | Connect, send, receive, timeout and explicit "Audacity not reachable" error path; tests against recorded pipe transcripts (Question 6) | pending | 5 | 1, 3 | none | - |
| 5 | `--daw Audacity` config plumbing | Config parsing accepts `"Audacity"`; settings-store smoke test proves no REAPER-specific assumption is tripped | complete | 4 | - | none | - |
| 6 | Import findings as labels | Idempotent label creation from findings (Question 5 scope), preview and approval before any write, one narrator-triggered action | pending | 7 | 2, 4 | `markers` (`MarkerWriter`-equivalent, label creation) | - |
| 7 | Navigate and mark reviewed | Selecting a finding moves Audacity's selection to its label; marking reviewed updates the label in place, never duplicates | pending | 6 | 6 | `navigate` (`Navigator`-equivalent) | - |
| 8 | Export reviewed labels | Writes the reviewed set to a hand-off file matching the reviewed findings exactly | pending | 9 | 7 | none | - |
| 9 | Dashboard integration | Findings sourced from an Audacity project show only Audacity-appropriate actions (no take-management controls); visual states and docs | pending | 8 | 7, `DAWAdapter` (3) | UI: existing primitives only (predates `studio-ui-primitives.prd.md`) | - |
| 10 | Audacity launcher shortcut | The installer's Start Menu entry "Narration Utils for Audacity" starts the app with `--daw Audacity`; the narrator picks the project in the app and the picker keeps the launch an Audacity one (Question 3, revised); pinned by `scripts/release/installer.test.mjs`, removed by the uninstaller (Question 7, revised) | complete (the installed shortcut on a real machine is the owner's check, with the release-readiness clean-machine install) | 8, 9 | 5 | none | - |
...
- **Scope:** **first, re-check `github.com/audacity/audacity/releases` for a release past 4.0.0 that restores the scripting pipe (recommendation 9, Question 8); if one exists, stop and re-plan this phase before spending the owner's session on it.** Otherwise, a throwaway Go program (not shipped) that opens the named pipes, sends one documented command, reads the reply, and records latency and error behavior with scripting off and with Audacity closed, against a confirmed Audacity 3.x install.
- **Success signal:** a written result in `docs/research/` naming the release re-check's outcome, exact latency, framing, and every observed failure mode.
...
**Phase 10 - Audacity launcher shortcut**
- **Goal:** the narrator reaches the app's workspace in Audacity mode in one click, as REAPER narrators do from the Action list (Question 3, owner decision 2026-09-23, revised the same day).
- **Why not a macro:** nothing Audacity loads can start a program ([launcher feasibility note](../research/audacity-launcher-feasibility.md)), and Audacity 4.0 has no macros.
- **Scope:** `apps/desktop/build/windows/installer/project.nsi` always creates `$SMPROGRAMS\Narration Utils for Audacity.lnk`, which starts `narration-utils.exe` with exactly `--daw Audacity` (no project path, nothing else), and the uninstaller deletes it. `ProjectSwitch` keeps an Audacity launch's `daw` instead of resetting it to `Standalone`, so the project the narrator picks opens as an Audacity project ([ADR 0145](../adr/0145-the-audacity-launcher-is-an-installer-start-menu-entry-and-a-picker-switch-keeps-an-audacity-launch.md)). No Audacity-side artifact ships and `verify-installable.mjs` is unchanged (Question 7, revised). The Audacity guide documents the shortcut.
- **Success signal:** `installer.test.mjs` fails if the entry, its exact arguments or its removal changes; `apps/desktop/audacity_test.go` proves a picker switch after `--daw Audacity` stays Audacity (and still opens no REAPER bridge). Installing the setup program and starting the shortcut on a real machine is the owner's step, together with the release-readiness clean-machine install.
...
| 10 | `apps/desktop/build/windows/installer/project.nsi`, `scripts/release/installer.test.mjs`, `ProjectSwitch` in `apps/desktop/bindings.go` | The installer definition is shared with release-readiness; add independently and rebase |
...
| Launcher (Question 3) | An installer Start Menu entry "Narration Utils for Audacity" that starts the app with `--daw Audacity`; the narrator picks the project in the app (Phase 10) | An Audacity macro (first answer, not buildable); Nyquist plug-in; the exporter side channel; a native module; standalone launch only | Owner decision 2026-09-23, revised the same day after the [launcher feasibility note](../research/audacity-launcher-feasibility.md) showed no Audacity-side artifact can start a program; [ADR 0145](../adr/0145-the-audacity-launcher-is-an-installer-start-menu-entry-and-a-picker-switch-keeps-an-audacity-launch.md) |
...
| Packaging (Question 7) | No Audacity-side artifact; the installer carries the shortcut, pinned by `installer.test.mjs`, and the uninstaller removes it | Ship a macro file checked by `verify-installable.mjs` (first answer) | Owner decision 2026-09-23, revised with Question 3 ([launcher feasibility note](../research/audacity-launcher-feasibility.md)) |
| Audacity versions | Audacity 3.x only for now; the pipe-based plan stays | Wait for Audacity 4 to restore the pipe; support 4.x some other way | Owner decision 2026-09-23: Audacity 4.0.0 dropped the Macro Manager and the scripting pipe ([launcher feasibility note](../research/audacity-launcher-feasibility.md)) |
| Audacity 4.x re-check (2026-09-26, Question 8, recommendation 9) | No scope change; stay Audacity 3.x only; add a release re-check as the first step of Phase 1's own spike | Widen scope for the incoming 4.1 Macro Manager; depend on the unofficial `Audacity4-MCP` fork | Re-verified directly (not resting on the benchmark's single secondary source): no release past 4.0.0 exists yet, and the tracked 4.1 Macro Manager work ([#11783](https://github.com/audacity/audacity/issues/11783)/[#9407](https://github.com/audacity/audacity/issues/9407)) is GUI batch macros with no scripting-pipe equivalent |
...
**Re-scope research (2026-09-26, recommendation 9):** checked live via WebSearch/WebFetch rather than the benchmark's own single secondary source. [audacity/audacity releases](https://github.com/audacity/audacity/releases) lists no version past 4.0.0 (2026-09-03) as of this date. [Issue #11783](https://github.com/audacity/audacity/issues/11783) ("Automation Epic: Macro Manager"), closed as a duplicate of [#9407](https://github.com/audacity/audacity/issues/9407) (open, "Ready for dev," milestone Audacity 4.1), tracks a from-scratch Macro Manager reimplementation ("not compatible" with Audacity 3 macros) that lets a narrator build, edit and run macros against the open project or a batch of files - a GUI/batch feature, not the `mod-script-pipe` transport this PRD's Go client needs, and neither issue mentions scripting. A third-party project, `Audacity4-MCP`, compiles a modified Audacity to add an unofficial scripting bridge for AI assistants; it is not an official Audacity feature and repeats the native-module risk the [launcher feasibility note](../research/audacity-launcher-feasibility.md) already rejected, so this PRD does not depend on it.

---

*Generated: 2026-09-21*
*Re-scoped: 2026-09-26 - Question 8 added, "Audacity 4.x support" row and Phase 1's own scope updated with a dated release re-check, for benchmark recommendation 9. No phase's dependencies, scope or status changed beyond that.*
