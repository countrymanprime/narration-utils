...
- **Wails.** `wails.json` `frontend:dir` becomes `../ui`; `frontend:build` still runs `scripts/copy-wails-frontend.mjs`, whose source and destination paths change. `go -C shell` becomes `go -C apps/desktop` in `scripts/quality.mjs`, `_quality.yml`, `build-native/action.yml`, `setup-toolchain/action.yml` (go.sum cache path).
