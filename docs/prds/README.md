...
| [Repo Layout by Role](repo-layout-by-role.prd.md) | Infrastructure | 5 | None; sourced from [`docs/research/repo-layout-alignment.md`](../research/repo-layout-alignment.md); supersedes the `shell/`-directory non-goal of [Release Artifact Naming](release-artifact-naming.prd.md) |
...
- **Next free ADR number.** `0027` when the PRDs were written; `0037` once ADRs 0027-0036 had merged, and `0039` once ADRs 0037 and 0038 (visual suite) had merged (the Base UI ADR is planned as the next one, written by foundation Phase 1 after its code lands). Numbers can collide with open pull requests, so re-check `docs/adr/` at merge time before numbering.
...
- **`ProjectCreate` and the projects directory.** [Project Workspace](project-workspace-and-daw-link.prd.md) Phase 2 replaces `ProjectCreate` with `ProjectCreateIn` (a `hostAPIVersion` bump) while the host binding data race PRD rewrites `bindings.go`; sequence it after that PRD's accessor phase.
- **The layout move.** [Repo Layout by Role](repo-layout-by-role.prd.md) Phase 3 renames `shell/`, `shared/*` and `tools/*` (nearly every tracked path), so it needs a window with no other PRD phase in flight; after it lands, every path cited in the other PRDs and in this file is rewritten by its codemod. Its Phase 1 (cleanup) and Phase 2 (guard test and path constants) touch `scripts/quality.mjs`, `shell/app.go` and `prepare-resources.py`, so sequence them against the host-binding, test-stability and release PRDs. [Release Artifact Naming](release-artifact-naming.prd.md) edits `wails.json`, `shell/package.json` and the launcher, so land it before or after Phase 3, not during.
