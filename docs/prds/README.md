...
| [Repo Layout by Role](repo-layout-by-role.prd.md) | Infrastructure | 5 | None; sourced from [`docs/research/repo-layout-alignment.md`](../research/repo-layout-alignment.md); supersedes the `apps/desktop/`-directory non-goal of [Release Artifact Naming](release-artifact-naming.prd.md) |
...
- **`apps/desktop/bindings.go` and the host binding fix.** The host binding data race PRD rewrites nearly every binding body, so its Phase 1 (the `h.services()` accessor and guard test) merges first, Phases 2 and 3 follow back to back, and PRDs that add bindings meanwhile build on the accessor or rebase onto those phases.
