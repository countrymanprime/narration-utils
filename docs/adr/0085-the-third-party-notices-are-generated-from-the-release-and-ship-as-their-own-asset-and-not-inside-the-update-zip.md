...
- The project is AGPL-3.0-or-later (ADR-0039, owner decision D17), so the release must carry the licences of what it contains, the AGPL text and a source offer.
- The docs security and hygiene PRD recommended the notices as a release asset and a copy inside the update zip (Open Question 15 a).
- The in-app updater unpacks exactly one entry, `narration-utils.exe`, and refuses a zip that holds anything else, so an installed client would refuse every update to a two-file zip.

## Considered options

1. Notices generated from the release, shipped as their own release asset and not inside the update zip
2. A copy of the notices inside the update zip as well (the PRD's recommendation)
3. Change the updater's rule so it accepts a second entry in the zip

## Decision outcome

**Chosen option: notices generated from the release, shipped as their own release asset and not inside the update zip**, because a copy inside the zip would strand every installed client, whose updater refuses a two-file zip, and changing that rule weakens the hostile-archive defence for the sake of a text file.
...
- **Good:** Every release has a checkable inventory of what it ships and where its source is, and the check runs on every pull request's Windows build.
- **Bad:** A narrator who unpacks only the portable zip does not get the notices with it; they are one click away on the same release page, and the setup program's copy is an open follow-up. This is the part to review.
- **Neutral:** A licence the generator cannot name, or a component without a licence text, fails the build until a person records it (three are recorded, [#246](https://github.com/countrymanprime/narration-utils/issues/246)).
- **Neutral:** Putting the notices in the zip later needs the updater change above and a new ADR that supersedes this one.

### Confirmation

`scripts/licenses/notices.py` refuses to write a report it cannot trust; the notices are verified by `assets.mjs verify` (a Windows release without them, or with a changed copy, is not promotable); and every pull request's Windows build runs the generator.

## Pros and cons of the options

### A copy inside the update zip

- Bad, because an already-installed client would refuse every update to a two-file zip with "does not hold exactly the program", so the change would strand them.

### Change the updater's rule

- Bad, because it is a change to the hostile-archive defence for the sake of a text file.
- Bad, because the notices would still be stale next to an updated program.
