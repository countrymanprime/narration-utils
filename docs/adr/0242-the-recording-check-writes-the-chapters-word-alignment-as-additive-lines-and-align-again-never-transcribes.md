...
- The workspace's text, flags and click-to-seek come from one alignment, so they can't disagree with the Home check or the Review page's findings.
- A 15,000-word chapter adds about 1.5 MB to its results file, well under the host's 16 MB limit.
- Lane A still has to build the stored artifact and its read binding: join `item` to the item and take GUIDs from the manifest and the saved project, and treat a report with no token lines as "align again". Nothing reads the new lines until then.
- The hyphen fuse and merged number words make one token stand for two words, or two tokens for one. `w` names the first word, and a screen highlights up to the next token's word.
- Changing a line's shape later needs a new tag, or a schema version bump coordinated with the host reader.
