...
- One switch, read by the host on every request, gates every unverified command. No UI, binding or Lua change is needed to switch a verified command on: it is one line in `actions.go`.
- A forged command file (threat row 5a) is not stopped by the switch, since the Lua does not know it. That is the same boundary as every other command: the folder ACL, not the app, protects the command folder.
- The Settings page shows the row as soon as the host offers it; its tooltip and any wording around it are lane C's (the Settings UI), and the mock gains the row with it.
