...
- The owner wants to tell downloads of different versions apart at a glance.
- The in-app updater finds its zip by an exact, compiled-in name.
- Every release so far is a release candidate.

## Considered options

1. Every asset carries the bare version, and older installed copies are reinstalled once by hand
2. Keep the status quo: unversioned asset names, with the version only inside the program (D14)
3. Keep the update zip unversioned
4. Publish an unversioned copy beside the versioned zip for a transition

## Decision outcome

**Chosen option: every asset carries the bare version, and older installed copies are reinstalled once by hand**, because the owner asked to see the version at a glance and, since every release so far is a release candidate, chose a one-time manual reinstall over a transition.
...
- **Good:** Downloads of different versions can be told apart by name, and the checksum file names which version it is for.
- **Bad:** **An installed copy from before this change cannot update itself to a release made after it.** It finds no asset with the name it knows and treats the release as unusable, so it silently reports no update. It has to be updated once by hand (run the new setup program, or replace the program from the new zip); from then on the updater finds the versioned names. Accepted by the owner because every release so far is a pre-release.
- **Bad:** A release candidate published before this change carries unversioned names, so `verify` finds none of its files and promote refuses it; the next candidate can be promoted (the same rule as candidates without attestations or without a setup program).
- **Neutral:** The build folder and the release now name the setup program and the notices differently; `assets.mjs` is the one place that maps one to the other.
- **Neutral:** Changing the name pattern again means changing `assets.mjs` and `update.Platform.AssetName` in one pull request and a new ADR that supersedes this one; the tests of both hold the pattern.

### Confirmation

The tests of `scripts/release/assets.mjs` and `update.Platform.AssetName` hold the pattern, and `verify <dir> <version>` refuses a file named for any other version.

## Pros and cons of the options

### Every asset carries the bare version, and older installed copies are reinstalled once by hand

- Good, because downloads of different versions can be told apart by name.
- Bad, because an installed copy from before the change cannot update itself to a release made after it.

### Keep the status quo: unversioned asset names, with the version only inside the program (D14)

- Bad, because three downloads of three versions sit in a Downloads folder with nothing to say which is which.
