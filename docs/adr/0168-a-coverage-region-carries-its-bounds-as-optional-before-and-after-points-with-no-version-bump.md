...
- **Date:** 2026-09-23
- **Deciders:** the owner
- **Related:** Amends [ADR-0127](0127-the-coverage-sidecar-mode-reads-a-json-manifest-keeps-one-words-file-per-item-and-writes-tagged-json-lines.md) (the `COVERAGE_REGION` payload).
...
- The re-check window has to be bounded by the words the first pass matched on either side of the gap.
- One point cannot say where a gap starts and ends.
- A read title is optional text, and a chance match of a heading word could land anywhere in the audio.
- A time exactly where one item ends and the next starts falls into the next item.
- An `AnalyzerVersion` bump would make every stored result stale with `analyzer_changed` for no change in what it measures.

## Considered options

1. Optional `before` and `after` bounds from anchors with body text, with no version bump
2. Bumping `RESULT_SCHEMA_VERSION`, `AnalyzerVersion` or `wordsVersion`
3. Letting a read title bound a head

## Decision outcome

**Chosen option: optional `before` and `after` bounds from anchors with body text, with no version bump**, because the keys are additive and the verdict is the same bytes as before, so a bump would only make every stored result stale.
...
- **Good:** The re-check planner (cascade Phase 4) can bound each window in one item's source seconds without re-aligning, and a region split across two items reads as `before` in one and `after` in the next.
- **Neutral:** A result made before this change has no bounds. A reader that needs them, the planner, must treat a missing bound as the edge of the item's played range, as it would for a head or a tail, or run the check again. Nothing forces that re-run.
- **Neutral:** The UI and the stage recommendations do not show the bounds yet; `position` still names the place in the Home dialog and the signal's evidence.
- **Neutral:** The bounds are data the host reads from a sidecar's file ([threat model](../architecture/threat-model.md) row 4e): they are labels until the planner uses them, and the planner must check them against the manifest's items before it turns one into a slice of audio.
- **Neutral:** The owner accepted this on 2026-09-23 on the workflow the check assumes for now: each chapter is recorded on its own track, so the end of one chapter never runs into the next. A head or tail window that runs to the edge of the item's played range therefore stays inside the chapter's own recording, and the cost of not bounding it by a read title is only a little more audio to re-check.
- **Neutral:** Changing what bounds a region (for example, letting a read title bound a head) means a new ADR that supersedes this one, and bumping `AnalyzerVersion` if the verdict could change.
...
### Bumping `RESULT_SCHEMA_VERSION`, `AnalyzerVersion` or `wordsVersion`

- Bad, because an `AnalyzerVersion` bump would make every stored result stale with `analyzer_changed` for no change in what it measures.

### Letting a read title bound a head

- Bad, because a chance match of a heading word could land anywhere in the audio, and a window bounded by it could leave out the audio where a misheard first paragraph really is.
