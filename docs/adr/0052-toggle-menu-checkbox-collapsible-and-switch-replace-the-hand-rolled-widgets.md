...
- The four widgets had none of the behaviour a screen reader or a keyboard expects.
- Settings will need on/off controls, and a `Switch` is the right semantics for a setting that takes effect at once.
- The alias combobox's matching and active-index logic is domain code and the riskiest to regress.

## Considered options

1. Convert the four call sites to Base UI `Toggle`, `Menu`, `Checkbox` and `Collapsible` primitives and add `Switch`
2. Also convert the alias combobox
3. Also convert the six native `<select>`s
4. Radio semantics for the single-choice `Pill` groups

## Decision outcome

**Chosen option: convert the four call sites to Base UI `Toggle`, `Menu`, `Checkbox` and `Collapsible` primitives and add `Switch`**, because the hand-written widgets had none of the keyboard or screen-reader behaviour expected, and the owner chose to convert these four and add `Switch` while leaving the riskier alias combobox bespoke.
...
- **Good:** The four converted widgets announce their state and work from the keyboard the way the platform documents, and nothing outside `primitives/` imports the library. What is not converted (the alias combobox, `NavButton`, the lock button, the native selects) is unchanged.
- **Neutral:** `Pill` reports `aria-pressed` on the groups that use it as a single choice (theme, text size, log verbosity), where radio semantics would be more accurate; group semantics are deferred (foundation PRD question 7). The `Menu` popup is not modal, so the page behind stays scrollable and clickable as it was.
- **Neutral:** The visible changes are the token-styled checkbox and the category menu's popup being portalled (same look, positioned by the library). The visual-suite states that show a chip, the disclosure or the entry header were compared pixel by pixel with the state before this work and are identical (the disclosure's top border colour needed an explicit token: Tailwind 4's default is `currentcolor`).
- **Neutral:** `Checkbox` and `Switch` label presses are proved in RTL tests; the Storybook jsdom runner cannot press a label (Base UI builds a `PointerEvent` that its window rejects), so their stories press the control itself.
- **Neutral:** `Switch` is unused code until Settings adopts it; Knip sees it through its story and test.
- **Neutral:** To change any of this, write a new ADR that supersedes this one.

### Confirmation

Each primitive has a story and tests; the visual-suite states that show a chip, the disclosure or the entry header were compared pixel by pixel with the state before this work.

## Pros and cons of the options

### Also convert the alias combobox

- Bad, because its matching and active-index logic is domain code and the riskiest to regress.

### Radio semantics for the single-choice `Pill` groups

- Good, because radio semantics would be more accurate for a single choice.
