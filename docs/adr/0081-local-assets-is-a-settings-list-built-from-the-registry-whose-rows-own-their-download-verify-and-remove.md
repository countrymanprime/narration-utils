...
- There was no place to see everything the app keeps on the computer, to verify or repair an asset, or to follow a download that a page had started.
- The registry (ADR-0079) already lists every asset with its state, sizes, links and running job, and the install is one job with real bytes (ADR-0077).
- The owner's decisions: the cache folder is shown and never changed or emptied (Q3 a), and removing a voice or model is a danger confirm (D12).

## Considered options

1. A Global "Local assets" Settings list built from `AssetsList`, whose rows own their download, verify and remove
2. Keep the status quo: a first-use question per asset and a remove button beside one selected item
3. A "clear the cache" button, or a way to move the cache folder from the app

## Decision outcome

**Chosen option: a Global "Local assets" Settings list built from `AssetsList`, whose rows own their download, verify and remove**, because there was no place to see, verify, repair or follow every asset, and the registry already lists each one with its state and running job.
...
- **Good:** One place to see, verify, repair and remove assets; the older per-page remove buttons still work (they call the same removal) and can be dropped when their pages move over.
- **Neutral:** The first Verify on a large model reads every byte and can take seconds; the button says it is working and cannot be pressed twice.
- **Neutral:** There is no "clear the cache" button and no way to move the folder from the app (owner decision Q3 a); to add either, write a new ADR that supersedes this one.
- **Bad:** The row's state is derived from the list plus its own job, so a second window that starts a download is seen only when the list next reloads.

### Confirmation

The mock reaches every state without a host (`?mockAssets=installing|checking|damaged`, `download-fails`), and the visual suite captures each at every viewport including the 390 px reflow width. `ProgressBar` has stories and a test.

## Pros and cons of the options

### A "clear the cache" button, or moving the folder

- Bad, because the owner decided the cache folder is shown and never changed or emptied (Q3 a).
