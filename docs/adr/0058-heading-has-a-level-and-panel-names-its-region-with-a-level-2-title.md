...
- `Heading` always rendered an `<h1>`, so a section title could not be a heading of its own level.
- A `Panel` had no accessible name and was not a landmark, and some hand-built titles were a `div`, not a heading.
- A long unbroken subtitle overflowed sideways.

## Considered options

1. `Heading` with a `level`, and `Panel` with a `title` rendered as a level-2 heading that names its region
2. Keep the status quo: an `<h1>`-only `Heading` and a bare `<section>` `Panel` with hand-built titles

## Decision outcome

**Chosen option: `Heading` with a `level`, and `Panel` with a `title` rendered as a level-2 heading that names its region**, because without them a section title could not be a heading of its own level and a panel had no name a screen reader could list.
...
- **Good:** The four screens above now offer a named region and a real heading to a screen reader, and a page can put a section title at the right level.
- **Neutral:** Every titled `Panel` is a landmark. That is the point for a few named prompts, but a page that titles many panels would list many regions; a panel that should not be a landmark should stay untitled.
- **Neutral:** The Tracks player, and any other panel whose title is data, has no name yet; naming it is a call-site decision, not a primitive change.
- **Neutral:** `Panel` has one heading level. A panel inside a section that has its own `<h2>` would need a `level` prop, which is a new decision.
- **Neutral:** To change any of this, write a new ADR that supersedes this one.

### Confirmation

`Heading.test.tsx` and `Panel.test.tsx` and stories that run in the atlas; each adopted call site has a test that the region is named.
