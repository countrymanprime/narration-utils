...
- The built-in ACX profile lists head and tail room tone, which the app did not measure.
- The PRD asked for a definition of "speech starts" that agrees with ACX Check, using the silence map's floor (−50 dBFS).
- ACX asks for room tone, not digital silence, at the edges.
- A custom copy saved before Phase 5 would have said "not checked" forever.

## Considered options

1. Time the edges against the fixed silence floor, and take how a rule is checked from its built-in
2. Make the floor a narrator setting or a profile rule

## Decision outcome

**Chosen option: time the edges against the fixed silence floor, and take how a rule is checked from its built-in**, because the PRD asked for a "speech starts" that agrees with ACX Check using the silence map's floor, and a copy saved before Phase 5 would never have been checked.
...
- **Good:** Both room-tone rules are judged on every measured WAV, and their findings carry the ids ADR 0179 defined, so nothing already reviewed changes id.
- **Neutral:** A render with speech in its first 50 ms has 0 s of head room tone and misses ACX's minimum, which is the point. A render whose reading never rises above −50 dBFS (a whisper at a very low level) reads as having no edges; the rule is then "not measurable", not met.
- **Neutral:** The window and floor are the app's, not ACX Check's. Phase 0 of the PRD (the comparison with Audacity's ACX Check, owner-run) may move them; that change is a new ADR and a new `AnalyzerVersion`.
- **Neutral:** A custom copy can no longer freeze an old check. The cost is that a future build that stops checking a rule also stops for every copy; that is intended.
- **Neutral:** Superseding this needs a new ADR, for example to make the floor a profile rule or a narrator setting.
...
### Make the floor a narrator setting or a profile rule

- Bad, because the floor is what separates room tone from reading, not a delivery limit.
