...
- Take review left an explicit, confirmed "Make active" for later.
- The edit and proof workspace's EP7 (recommendation A, adopted by owner decision D39) asks that "Use this take" make the chosen take the item's active take in REAPER.

## Considered options

1. A `set_active_take` bridge command, by GUID, in one undo step
2. Keep the status quo: take review's rule that the app never sets the active take

## Decision outcome

**Chosen option: a `set_active_take` bridge command, by GUID, in one undo step**, because the workspace's "Use this take" makes the chosen take the item's active take (EP7, adopted by owner decision D39).
...
- **Neutral:** Take review's "never sets the active take" now holds for take review only: the workspace's confirmed "Use this take" is the one way the app changes what an item plays.
- **Good:** One Undo in REAPER restores the previous take, per the reference; the verification pass confirms it.

### Confirmation

The verification pass, row A6, including that one Undo in REAPER restores the previous take.
