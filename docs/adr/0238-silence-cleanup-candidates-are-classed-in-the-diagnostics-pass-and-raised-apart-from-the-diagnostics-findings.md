...
- Cleanup and the diagnostics cannot disagree about what is silent, and cleanup adds no second read of the file. The cost is one more meter in the pass and about 16 bytes per 10 ms window held until the end (about 6 MB for an hour).
- Zero crossings stand in for spectral shape: no FFT, and a soft "s" or "f" can read as a breath (confidence at most 0.5, and never cut without approval). A breath quieter than the silence floor is silence, not a breath.
- The finding shape and the `split_and_trim` action are the contract the UI list and the future REAPER action will read. Changing them needs a new ADR and the findings contract kept in step.
- Superseding this needs a new ADR, for example to add spectral features or to raise cleanup candidates with the other diagnostics.
