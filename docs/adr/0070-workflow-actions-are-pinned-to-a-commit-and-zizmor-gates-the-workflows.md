...
- A release is an unsigned executable built and published by GitHub Actions, so everything the workflows run is part of the trust path.
- A moved action tag is a real attack (76 of 77 tags of `aquasecurity/trivy-action` were force-pushed to credential-stealing code on 2026-03-19).
- Checkouts left the token in `.git/config` for every later step, `promote-release.yml` held `contents: write` at workflow level, and nothing checked the workflows themselves.
- The owner approved the hardening (owner decisions D7, D11, D14 and D22).

## Considered options

1. Pin every external action to a full commit SHA, and gate the workflows with zizmor
2. Keep the status quo: mutable tags, stored checkout credentials and no check on the workflows

## Decision outcome

**Chosen option: pin every external action to a full commit SHA, and gate the workflows with zizmor**, because everything the workflows run is part of the trust path of an unsigned release, and a moved action tag is a real attack.
...
- **Neutral:** A repointed action tag cannot change what a job runs; the cost is that an action update is a reviewed pull request (Dependabot opens it, a week after release).
- **Neutral:** An unpinned line, a checkout that keeps its credential, or a workflow with no `permissions` fails the `zizmor` check with the line named. The check is a pull-request check but not a required one (no ruleset requires a status check, D11), so the maintainer reads it before merging.
- **Neutral:** The owner-only setting "Require actions to be pinned to a full-length commit SHA" makes GitHub itself refuse an unpinned action; it is enabled after this lands and one Dependabot run has passed (listed in [GitHub workflow](../operations/github-workflow.md)).
- **Neutral:** Changing any of this means a new ADR that supersedes this one.

### Confirmation

`.github/workflows/zizmor.yml` runs on every pull request and fails on a finding (its `unpinned-uses`, `artipacked`, `excessive-permissions`, `template-injection`, `dangerous-triggers` and `dependabot-cooldown` audits). `pinact run --verify --check` re-checks each SHA against its comment, and pnpm's `minimumReleaseAge` check fails CI on a lockfile entry younger than three days.

## Pros and cons of the options

### Keep the status quo: mutable tags

- Bad, because a moved action tag is a real attack: on 2026-03-19 76 of 77 tags of `aquasecurity/trivy-action` were force-pushed to credential-stealing code.
