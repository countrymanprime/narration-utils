...
- A narrator with an existing check sees a verdict computed with the new share at once. The threshold is applied on
  read, so nothing is transcribed and no result goes stale. The alignment parameters did not change, so no stored check
  goes out of date.
- A narrator's own saved values are untouched. Only the built-in defaults changed.
- A skipped phrase of 3 words or fewer can pass. This was already true of the Proposed values, and it is now stated
  as the check's resolution on the Settings page's rule and in the recording check page.
- The measurements, the sensitivity of each setting and the time budget (about 10 s of CPU per audio minute with
...
- Recalibrating is one command: `coverage_calibration.py synthetic`, run over a corpus that has more cases. A pytest
