...
- A folder's name says its role, the test rule is one sentence, and the top level cannot drift without `pnpm check` failing.
- CI runs what a change affects, and a new project needs a `project.json`; `scripts/ci/projects.test.mjs` fails if a Python, Go, Lua or `apps/` TypeScript file is not covered by a project lint target, and keeps the root project's dependency list complete.
- Open branches and worktrees conflict less than a rewrite would: git follows the renames. After merging `main` into one, run `node scripts/ci/apply-path-map.mjs` to rewrite the path text inside files, then `pnpm install`.
- REAPER users who registered `shared/reaper/NarrationUtils_Launcher.lua` as an action must register `integrations/reaper/NarrationUtils_Launcher.lua` instead. Packaged builds use `resources/reaper` and are unaffected.
- Local, untracked files (`.claude/skills`, ignored runtime folders such as the legacy `shared/python/.runtime`) are not moved by git; the move's author updated `.claude/skills` locally.
- The `area:shell` label keeps its name; the labeler points it at `apps/desktop/**`.
- A future move edits `scripts/ci/layout.json`, the Go `layout` constants and the `prepare-resources.py` constants, runs `apply-path-map.mjs`, and supersedes this ADR.
