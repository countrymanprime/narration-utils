...
- The teleprompter needs word-level speech recognition while the narrator is still talking.
- Prototyping two local engines gave different strengths and no clear winner.
- The user wants to evaluate both against real manuscript scrolling and animation before choosing a default.

## Considered options

1. Interchangeable engines behind one engine-independent event contract, with no default chosen yet
2. Whisper (`faster-whisper` tiny, re-decoded every 0.5s) as the engine
3. Moonshine streaming (Small and Tiny) as the engine

## Decision outcome

**Chosen option: interchangeable engines behind one engine-independent event contract, with no default chosen yet**, because prototyping gave no clear winner and the user wants to evaluate both engines in the real UI before choosing a default.
...
- **Good:** Adding or swapping an engine is one adapter; the tracker, host and UI do not change.
- **Good:** The two engines can be compared on identical input and identical downstream logic.
- **Bad:** Engine timestamps cannot drive per-word animation pacing; the UI must pace itself.
- **Bad:** Moonshine is not a project dependency and is not bundled in releases, so its results cannot ship until provisioning and packaging are done.
- **Neutral:** The confirmation rule adds roughly one second of lag to `word` events, which is why the tracker also follows `partial` events speculatively.
...
- Good, because words are confirmed by agreement between consecutive decodes.
- Bad, because confirmed words lag by a median of about 1.2s.

### Moonshine streaming

- Good, because it processes at about 0.26x (Small) and 0.17x (Tiny) real time and emits partials with word timestamps every 0.5s.
- Bad, because its final line was identical to its last partial in 27 of 27 lines (it never corrects itself).
- Bad, because its word timestamps are often out of order.
