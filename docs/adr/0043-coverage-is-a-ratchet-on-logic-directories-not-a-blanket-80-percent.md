...
- A blanket 80% would fail today, and the cheapest way to pass it is padding tests on glue code.
- Glue and launch code sits far below 80%, and UI components are already covered by the visual suite and the atlas, which coverage does not see.
- Coverage of the code where a regression is silent should not be able to fall.

## Considered options

1. A ratchet on logic directories (PRD question 1, option c)
2. The owner's global rule: a blanket 80% everywhere

## Decision outcome

**Chosen option: a ratchet on logic directories**, because a blanket 80% cannot be applied as written and would reward padding tests on glue code.
...
- **Good:** Coverage of the code where a regression is silent (parsers, offset and alignment math, state, findings, analyzers) cannot fall without a red gate, and rises as tests are added.
- **Neutral:** Glue code is not pushed towards padding tests. The cost is that "the project has 80% coverage" is no longer a true sentence, and that someone must decide which directories are logic; the list in the floors file is that decision, and a reviewer of a new logic directory checks it is added.
- **Neutral:** Go coverage differs by a tenth of a point between runs on the root host package, which is not gated. Gated floors are rounded down so ordinary reruns cannot flake.
- **Bad:** A change that drops a gated directory below its floor (for example by moving tested code to an ungated directory) is caught only if the old path has an entry: the "no coverage data" failure covers renames and deletions.
- **Neutral:** To change the policy (blanket 80%, or a different ratchet), write a new ADR that supersedes this one.

### Confirmation

`scripts/ci/coverage-gate.mjs`, run by the `test` target of each project with entries, fails a gated directory below its floor or with no coverage data; `scripts/ci/coverage-gate.test.mjs` validates that an entry below 80 carries a written `reason`.

## Pros and cons of the options

### A blanket 80% everywhere

- Good, because "the project has 80% coverage" would be a true sentence.
- Bad, because it would fail today.
- Bad, because the cheapest way to pass it is padding tests on glue code.
