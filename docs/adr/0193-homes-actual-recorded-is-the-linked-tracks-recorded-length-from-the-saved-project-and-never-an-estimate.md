...
- **Status:** Accepted
- **Date:** 2026-09-24
- **Related:** Supersedes the "The recorded column names its source" clause of [ADR-0130](0130-the-home-recording-check-opens-on-the-stored-result-runs-only-on-a-press-and-labels-the-recorded-length-measured-or-estimated.md); Q12 of `docs/utilities/recording-coverage.md`
...
- The owner reported that Home's Actual recorded column never showed a recorded time.
- Before a check it was the estimated finished length times a share taken from the status the narrator picked, and even the "measured" case was a word share times the same word-count estimate, not a duration.
- Honesty (ADR 0015), even though the headline number drops for users who have not linked tracks.

## Considered options

1. The linked track's recorded length from the saved project, or a dash with a reason
2. Keep the status quo: a share from the chapter's status (`RECORDED_FRACTION`), or the recording check's word share, times the word-count estimate, captioned "measured" or "estimated from status"

## Decision outcome

**Chosen option: the linked track's recorded length from the saved project, or a dash with a reason**, because the old value was never a recorded time, only a guess from the narrator's status or a word share times an estimate.
...
- **Neutral:** A narrator who has linked no tracks yet sees an all-dash column and a dashed headline stat; this is accurate, not broken, and each dash explains itself on hover or focus.
- **Good:** The number can never be gamed by picking a status, and never regresses when a check goes stale (there is no check dependency left to go stale).
- **Neutral:** Home's chapter-track link button and REAPER Automation's chapter-track sync (sibling features) become the way a narrator turns dashes into times; this ADR does not change how a link is made, only what the column does with one.
- **Neutral:** A future change that wants to show a *raw* take time (including trimmed retakes) alongside the current length, instead of only the union of unmuted playing-lane items, needs a new ADR: this one fixes the definition to the union, matching what the recording check already reads.
...
### Keep the status quo: a share from the chapter's status (`RECORDED_FRACTION`), or the recording check's word share, times the word-count estimate, captioned "measured" or "estimated from status"

- Bad, because it never showed a recorded time.
- Bad, because picking a status changed the number.
