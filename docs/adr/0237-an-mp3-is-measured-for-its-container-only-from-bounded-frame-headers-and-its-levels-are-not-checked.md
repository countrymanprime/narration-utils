...
- ACX asks for each uploaded file to be an MP3 at 192 kbps or more, constant bit rate.
- The app measures the WAV render, so `acx.format` has read "not checked".
- The PRD asked for the format to be read from the MP3's frame headers without decoding it.
- Decoding belongs to the diagnostics PRD's DX-2 and is not built.
- The MP3's bytes are untrusted input parsed in the host.

## Considered options

1. Measure an MP3's container only, from bounded frame headers, and leave its levels unchecked
2. Decode the MP3 to measure its levels (the diagnostics PRD's DX-2)

## Decision outcome

**Chosen option: measure an MP3's container only, from bounded frame headers, and leave its levels unchecked**, because the PRD asked for the format to be read from the frame headers without decoding, and decoding belongs to DX-2, which is not built.
...
- **Good:** A narrator can measure the MP3s they will upload beside the WAV renders. One job judges both, and the book's channel rule compares them.
- **Neutral:** No level is ever claimed for an MP3: loudness, peaks and room tone after encoding stay unmeasured until DX-2 decodes MP3.
- **Neutral:** A file whose headers lie is read as what they claim: a verdict can be wrong, but nothing is written, run or fetched (threat model row 6j). The fuzz test `FuzzReadMP3` guards the parser's bounds.
- **Neutral:** The picker still offers WAV first. Offering "MP3 audio (*.mp3)" in its filter is a one-line change in `apps/desktop/measure_job.go` (lane A); until then, "All files" picks an MP3.
- **Neutral:** Superseding this needs a new ADR, for example when DX-2 decodes MP3 and measures levels on it.

### Confirmation

The fuzz test `FuzzReadMP3` guards the parser's bounds.

## Pros and cons of the options

### Decode the MP3 to measure its levels (the diagnostics PRD's DX-2)

- Bad, because it is not built.
