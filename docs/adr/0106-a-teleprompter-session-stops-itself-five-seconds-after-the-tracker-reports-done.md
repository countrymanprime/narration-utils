...
- The auto-stop works whichever view is open, or none, because it lives in the host.
- A narrator who pauses at the end for more than 5 seconds and then re-reads is stopped first; they press Start
...
- The timer is injectable (`Service.afterFunc`), so the tests drive it with a fake clock and never sleep through the
  delay.
- The browser mock mirrors the host (same delay and messages), so a mock session that replays to the end stops itself
