...
- A red property is a bug in the code or the property, never a bad draw, and a run costs a fixed one to three seconds per project.
- The gate does not search: bugs outside the fixed examples appear only when someone runs the explore profile. Nothing schedules that run; if targets start finding things it should become a scheduled job.
- The fixed seeds mean a new property can pass on 100 examples and still be wrong; the review habit that caught vacuous generators in phases 4 and 5 (measure reach, mutate the code and see the property fail) stays necessary.
- To change this (random seeds in the gate, `-fuzztime` in CI), write a new ADR that supersedes this one.
