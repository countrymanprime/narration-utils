...
- Opening a result never costs a transcription, and a check can never start by accident from the table.
- A stale result stays readable but no longer counts as measured, so the headline "Actual recorded" can drop when the project changes. That is the intended honesty.
- The dialog repeats nothing from Phase 7: when the signal lands, its met or not met can sit beside these counts without changing them.
- Relinking a chapter whose track is missing takes a trip to Tracks. Fixing that in place means a replace operation on the mapping store, which is a separate change.
