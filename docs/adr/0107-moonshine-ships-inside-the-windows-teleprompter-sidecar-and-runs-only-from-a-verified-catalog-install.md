...
- The shipped program must use the English `tiny` and `small` models pinned in the hashed catalog (ADR 0079).
- The owner chose to freeze `moonshine-voice` into the existing sidecar, falling back to a second executable only if the size cost was unacceptable.
- The PRD scopes Moonshine to Windows, and `moonshine-voice` 0.1.5 has no macOS Intel wheel.
- PyInstaller cannot see the package's `ctypes` DLL loads, so a freeze without them still starts and answers `--help`.
- A model directory without the attention decoder degrades word timings silently instead of failing.

## Considered options

1. Freeze `moonshine-voice` into the existing Windows `manuscript-teleprompter` sidecar, running only from a verified catalog install
2. A second executable for Moonshine
3. Keep the status quo: Moonshine only from a developer's `uv` environment, downloading its model into the library's own cache

## Decision outcome

**Chosen option: freeze `moonshine-voice` into the existing Windows `manuscript-teleprompter` sidecar, running only from a verified catalog install**, because the owner chose it, and a second executable would carry its own Python runtime, numpy and PyAV and cost more than the measured growth.
...
- **Bad:** The Windows sidecar grows by 24,058,709 bytes (255,370,066 to 279,428,775, 243.5 MiB to 266.5 MiB, +9.4%; the two
...
- **Good:** The shipped program has no path to Moonshine's servers: the one download of a Moonshine model is the host's, from
  the pinned catalog URLs, verified by SHA-256.
- **Bad:** `moonshine-voice` is the first dependency with an environment marker. A Linux or macOS developer cannot run
  `--engine moonshine` from `uv sync` alone; they add it themselves or wait for a cross-platform decision (a new ADR).
- **Neutral:** The wheel carries its own ONNX Runtime 1.23, separate from the `onnxruntime` package faster-whisper uses. A session
  loads one engine, so the two never share a process today; a change that loads both in one process must check which
  DLL each gets.
- **Neutral:** Moving to another `moonshine-voice` version means re-checking its licence (D17), its wheel list and its DLL names, and
  updating the catalog if the manifest test fails.

### Confirmation

`scripts/release/verify-installable.mjs` requires both DLLs in a Windows tree; `narration-utils --smoke` runs `manuscript-teleprompter --check-moonshine` and fails the build if the native library cannot answer; a test holds the sidecar's file list, the catalog and the pinned library's own manifest to each other, so a layout or version change fails in CI.

## Pros and cons of the options

### A second executable

- Bad, because it would carry its own Python runtime, numpy and PyAV and cost more than freezing into the existing sidecar.

### Keep the status quo

- Bad, because the library downloaded its model from Moonshine's servers into its own cache, checked only by CRC32C.
