...
- **Date:** 2026-09-21
- **Deciders:** the owner
- **Related:** Amends [ADR-0023](0023-visual-suite-capture-contract-and-storybook.md) (what the capture gate fails on)
...
- Page-level accessibility (labels, landmarks, dialog roles) was checked only by looking at the PNGs.
- The owner's decision D22 (question 6): measure a report-only baseline first, then gate with a debt ratchet.
- The first baseline should not be dominated by defects the dialog and palette work was about to fix.

## Considered options

1. Run axe on every captured app state, and fail any violation not declared as debt
2. Keep the status quo: axe runs on Storybook stories only (`tests/atlas/atlas.spec.ts`)

## Decision outcome

**Chosen option: run axe on every captured app state, and fail any violation not declared as debt**, because page-level accessibility was otherwise checked only by looking at the PNGs, and the owner's decision D22 was to measure a report-only baseline and then gate with a debt ratchet.
...
- **Good:** A new page or state that ships an unlabelled control, a page with no landmark, or an unnamed dialog fails the suite where it is introduced, at every viewport, with the selector.
- **Bad:** The suite's cost is one axe run per capture: the full run of 235 captures took 1.9 to 2.2 minutes with axe (two runs) and 1.6 minutes with `UI_AXE=0` on the development machine (four parallel workers), about 0.3 to 0.6 minutes more. `UI_AXE=0` skips it for a quick local loop.
- **Neutral:** The debt list starts at 19 pairs for five real defects. An entry must be on a driven row and is checked at each viewport it names, so a responsive change that stops one rendering at one width fails as "no longer holds" there and the entry gets a `viewports` list. Fixing one deletes its entries and lowers `MAX_AXE_DEBT_RULES`; raising the cap needs a maintainer's decision, like a raised `MAX_UNDRIVEN`.
- **Neutral:** The suite runs the light theme by default. `UI_THEME=dark UI_AXE=1` measures the dark theme by hand; the dark palette's contrast is also guarded by `paletteContrast.test.ts`.
- **Neutral:** Adopting repositories add `axe-core`, measure with `UI_AXE=1`, and declare `axeDebt` (kit CHANGELOG 0.3.4, "Adopt by hand").
- **Neutral:** To change any of this (gate on a stricter rule set, run axe in dark by default, drop the gate), write a new ADR that supersedes this one.

### Confirmation

The gate itself: a violation the debt list does not declare fails the capture, and a declared rule that is no longer reported fails too. `MAX_AXE_DEBT_RULES` in `src/visualSuite.test.ts` caps the (state, rule) pairs and may only fall, and on CI `checkAxeModeForCi` fails a run that sets `UI_AXE=0` or `UI_AXE=1`.

## Pros and cons of the options

### Keep the status quo: axe on Storybook stories only

- Bad, because page-level accessibility, the labels, landmarks and dialog roles of the pages a person actually sees, was checked only by looking at the PNGs.
