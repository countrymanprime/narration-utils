...
- Critical path of a pull request that touches the UI, estimated from the step times above: about 5 minutes (the Windows
...
- Job slots per CI run go from 12 to 14: five Playwright shards and the merge against four folded jobs and the kit's
...
- A job name a person or an agent watches changed: `ui-visual` is now the merge (the shards carry the captures and the
...
- Requiring `CI passed` is not enough on its own: `ci.yml` skips documentation-only pull requests (`paths-ignore`), so a
  required `CI passed` would wait forever on them until that trigger changes.
- Going back to one job per suite is removing the matrix and `UI_VISUAL_SHARDED`; the whole-run checks then run in the
  teardown as before.
