...
- **The UI is built** (`ResumePrompt.tsx`): `agree` is a one-line notice ("Continuing at '…' — REAPER and your last reading agree") with Change and Start from the top, `disagree` a two-way choice of cards (REAPER / Last reading, each named, timed, worded and quoted), `daw_only` keeps the resume offer Phase 1 already shipped, and `prompter_only` a one-line "Your last reading stopped at '…' … Continue there?" (RD8), never taken silently. Change reveals the same two-way choice an outright disagreement shows, so the narrator can still pick differently after an agreed preset.
