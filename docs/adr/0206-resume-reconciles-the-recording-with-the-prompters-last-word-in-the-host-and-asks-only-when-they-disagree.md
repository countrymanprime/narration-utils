...
- **Status:** Accepted
- **Date:** 2026-09-25
- **Related:** Amends ADR-0112 decision 3 (a choice is no longer the only thing that sets where Start begins)
...
- The owner asked for resume to keep going when the track and the prompter match, and to ask only when they do not.
- D39 takes the PRD's recommended answers RD1 (c), RD3 (a) and RD8 (a).
- Comparing in the UI would put the rule in TypeScript, untested against the Go types, and make each later surface re-derive it.

## Considered options

1. One pure host function, `teleprompter.Reconcile`, decides the resume verdict
2. Compare the two places in the UI
3. Keep the status quo: ADR 0112's confirm for every resume

## Decision outcome

**Chosen option: one pure host function, `teleprompter.Reconcile`, decides the resume verdict**, because comparing in the UI would put the rule in TypeScript, untested against the Go types, and make each later surface re-derive it.
...
- **Good:** Agreement needs no click, and a finished chapter never offers a resume, whatever surface asks.
- **Bad:** The Go tokenising must stay in step with the sidecar's. A change to `load_chapter_script` or `sentence_bounds` changes it here too. A mismatch shows as token counts that differ, and then that source is dropped rather than misplaced.
- **Neutral:** Phase 4 adds a live DAW source. `ResumePlace.source` already says `saved`, so the live one adds a value, not a field.
- **Neutral:** The rule in the prompt's UI (Phase 3 UI, lane C) is presentation only. Changing what counts as agreement means changing `Reconcile` and this ADR.

### Confirmation

The goldens `teleprompter-locate-{found,agree,disagree,complete,prompter-only}.json` and the schema `teleprompterResumeVerdictSchema`.

## Pros and cons of the options

### Compare the two places in the UI

- Bad, because it would put the rule in TypeScript, untested against the Go types.
- Bad, because each later surface would re-derive it.
