...
- **Windows creates the release.** The prerelease workflow (`.github/workflows/prerelease.yml`) builds Windows and, in that same job, creates the GitHub prerelease with the installer.
...
- **One asset per platform**, named `narration-utils-<platform>.<ext>`, each with a `.sha256` beside it: `windows-x64` is the NSIS installer (`.exe`), `macos-arm64` is a zip of the app bundle (`.zip`), `linux-x64` is a tarball of the binary (`.tar.gz`). The table, including which platforms are `required`, lives in `scripts/release/assets.mjs`, which also packages and verifies them. The raw Wails executable is not published separately.
