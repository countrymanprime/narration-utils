...
- A boundary that an ADR states cannot be crossed without a red check that links back to the reason.
- The rules are scoped to imports. A sidecar's own network use (the ephemeral `uv` environment of ADR 0021 point 5 downloads packages), `os/exec` running `curl`, a third-party client under another name, and a module name built at run time are not seen, so the network rule is a fence on the Go host, not proof that analysis is local.
- ADR 0032 is still Proposed, so the analyzer and `net/http` rules enforce a proposed decision; if the owner changes it, the rules change with it.
- Renaming a Go package or moving `media.go` means editing the globs and module paths in the config, which is visible in the same diff.
