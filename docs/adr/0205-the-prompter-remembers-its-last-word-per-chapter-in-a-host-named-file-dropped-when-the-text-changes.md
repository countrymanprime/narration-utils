...
- The owner asked for resume to compare where the recording is with where the prompter last was, and to ask only when the two disagree.
- The host kept the last position only in memory for the running session; nothing was kept per chapter.
- ADR 0111 decision 1 rejected word-to-time anchors from past sessions as a way to place the recorded tail; a last reading position is used only as a check.
- D39 takes the PRD's recommended design for Phase 2.

## Considered options

1. The host remembers the last position per chapter in a host-named file, dropped when the text changes
2. Keep the status quo: the last position kept only in memory for the running session

## Decision outcome

**Chosen option: the host remembers the last position per chapter in a host-named file, dropped when the text changes**, because resume needs the prompter's last position per chapter to compare with the recording, and the host kept it only in memory for the running session.
...
- **Good:** Phase 3 can reconcile the DAW position with the prompter's own position and ask only when they disagree.
- **Neutral:** A narrator who reads on without recording leaves a reading ahead of the recording. Phase 3's rule (the DAW word wins on agreement, RD3) is what keeps that from moving the start.
- **Neutral:** A reading survives a Replace manuscript on disk but no longer matches (the document id is in the hash). `resetDerived` does not delete the folder, which Phase 12's anchors will share.
- **Neutral:** Using the stored position to place the recorded tail, rather than to check it, would reopen ADR 0111 decision 1 and needs its own ADR.

### Confirmation

The golden `teleprompter-reading.json`, written by `TestContractReading`, and a `wireContracts.test.ts` row for `teleprompterReadingSchema`.
