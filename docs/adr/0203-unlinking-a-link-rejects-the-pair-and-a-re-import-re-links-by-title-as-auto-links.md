...
- If removing an auto-link by Unlink or clear is not a rejection, the next sync links the same pair again at once, and Unlink does nothing the narrator can see.
- A manuscript re-import renumbers every chapter id; ADR 0100 and Q9 of the evidence ledger PRD chose "cleared, then re-suggested from stored titles".
- With sync, re-suggesting can become re-linking, and those links need a label.

## Considered options

1. Clearing a link records a rejection whatever its origin, and a re-import carries old links forward for sync to re-link as auto-links
2. Clearing a link without recording a rejection
3. Carry a hand-made link across a re-import as `manual`

## Decision outcome

**Chosen option: clearing a link records a rejection whatever its origin, and a re-import carries old links forward for sync to re-link as auto-links**, because a clear that is not a rejection lets the next sync re-link the pair at once, and a re-import renumbers every chapter id.
...
- **Neutral:** An unlinked chapter stays unlinked across syncs. To get the same pair back, the narrator links it by hand, which also lifts the rejection.
- **Neutral:** A link the narrator made by hand before a re-import comes back labelled Auto-linked. The alternative, keeping `manual`, would make sync the author of a link it then refuses to undo. The owner may prefer that. A later ADR can carry the origin across instead (`CarryOver.Links` keeps it).
- **Neutral:** A rejection is keyed by chapter title, so it outlives a re-import. A rejection whose chapter was renamed simply stops matching anything.
- **Neutral:** If the owner rejects this, a superseding ADR changes the `Clear`/`ClearChapter` rejection and the origin that `chaptersync.Build` writes for `previous-link` matches.
...
### Clearing a link without recording a rejection

- Bad, because the next sync links the same pair again at once, and Unlink does nothing the narrator can see.

### Carry a hand-made link across a re-import as `manual`

- Bad, because it would make sync the author of a link it then refuses to undo.
