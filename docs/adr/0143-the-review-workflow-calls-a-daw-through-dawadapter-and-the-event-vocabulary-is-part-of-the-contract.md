...
- A second DAW should be a second implementation rather than `if daw == "REAPER"` branches in the host and UI.
- The seam must preserve behaviour, and come before any Audacity code lands.
- Owner decision Q2: the Audacity pipe client goes in a new Go package behind that interface.
- Avoid colliding with the parallel lanes also editing `internal/bridge` and the transcript service.

## Considered options

1. Wrap only the review commands, and keep the events as the bridge already defines them
2. A fully DAW-neutral interface with its own result types

## Decision outcome

**Chosen option: wrap only the review commands, and keep the events as the bridge already defines them**, because it changes no behaviour and no line on the bridge, where a DAW-neutral interface would rewrite the service's state machine.
...
- **Good:** The REAPER path is unchanged on the wire (`internal/dawadapter/reaper_test.go` pins each command line byte for byte) and every existing bridge, transcript and harness test passes unmodified.
- **Neutral:** The Audacity client (PRD Phase 4 onward) implements four request methods and translates its pipe replies into the `COMPARE_*` events, instead of the transcript service growing a second code path. That translation is the cost: an adapter must produce REAPER-shaped events, including fields such as `projectTime` and `itemIndex` that Audacity will fill from its own model.
- **Neutral:** "Mark reviewed" and "export the reviewed set" (PRD Phases 7 and 8) have no REAPER command today; `InspectFindings` (a finding already carried as a marker) is the only reviewed-state signal. Those phases add methods to `Review` with a REAPER implementation, or a narrower interface, when they land.
- **Neutral:** A future change that makes the events DAW-neutral types, or that moves another service behind an adapter, supersedes this ADR.

### Confirmation

`internal/dawadapter/reaper_test.go` pins each command line byte for byte, and every existing bridge, transcript and harness test passes unmodified.

## Pros and cons of the options

### Wrap only the review commands, and keep the events as the bridge already defines them

- Good, because it changes no behaviour and no line on the bridge.

### A fully DAW-neutral interface with its own result types

- Bad, because it would move the `COMPARE_*` field parsing out of the service and into each adapter, which is a rewrite of the service's state machine, not an extraction.
- Bad, because it would collide with the parallel lanes also editing `internal/bridge` and the transcript service.
