...
- A new command is a new file plus one line in `FEATURE_FILES` and one in `reaper-files.mjs`, and harness tests; PRDs that add commands no longer edit one chain.
- The bridge loads three more files, so a packaging mistake is possible in a new way; the installer check and its test cover it.
- The protocol did not change. Attaching a run ID to `ERROR` events is a separate change (the event fan-out) that sits on top of this.
- To go back to a single file, or to discover feature files by scanning the folder, write a new ADR that supersedes this one.
