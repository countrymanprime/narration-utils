...
- **Date:** 2026-09-25
- **Deciders:** the owner
- **Related:** Supersedes the "a suggestion is never treated as a link until the narrator confirms it through `MappingStore.Confirm`" clause of [ADR-0100](0100-analysis-evidence-is-two-hash-keys-one-ledger-record-per-run-and-a-narrator-confirmed-track-map.md)
...
- The owner asked that, once a DAW project is hooked up, chapters sync to tracks by name without a click per chapter.
- Owner decision D25: a confident two-way match links, labelled "Auto-linked" and undoable; a fuzzy match never links; hand-made links are never overwritten.

## Considered options

1. A confident two-way match is a real link, labelled auto, undoable, and never made over a manual one
2. Keep the status quo: ADR 0100's store of narrator-confirmed links only, where the matcher only suggests

## Decision outcome

**Chosen option: a confident two-way match is a real link, labelled auto, undoable, and never made over a manual one**, because the owner decided (D25) that auto-links are real links, so chapters sync to tracks by name without a click per chapter.
...
- **Bad:** A wrong auto-link sends a recording check to the wrong audio until the narrator undoes it. The bounds are the two-way rule, fuzzy matches never linking, a rejected pair never coming back, and ADR 0100's `mapping_changed` staleness, which already invalidates results on any relink.
- **Neutral:** The wire payloads that carry a `TrackMapping` (`ChapterTrackMapList/Confirm/Clear`, `ChapterTrackSet`, `ChapterTrackUnlink`, `ChapterTrackLinks`) gain two additive fields. `hostAPIVersion` does not move. The goldens, the Zod schema and the mock change with them.
- **Neutral:** The first sync, with no snapshot, reports no new, changed or missing tracks. It is the consent preview, not a change.
- **Neutral:** The snapshot fingerprint notices an item added, moved, trimmed, muted or switched to another take. It does not notice a source file rewritten in place. Phase 6's freshness uses the recording check's own fingerprint (`evidence.ComputeChapterFingerprint`), which does.
- **Neutral:** Letting a fuzzy match link, or letting sync replace a manual link, would need a new ADR that supersedes this one.

### Confirmation

The goldens, the Zod schema and the mock of the wire payloads that carry a `TrackMapping` change with the two additive fields.
