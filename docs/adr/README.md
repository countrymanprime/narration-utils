...
| [0127](0127-the-coverage-sidecar-mode-reads-a-json-manifest-keeps-one-words-file-per-item-and-writes-tagged-json-lines.md) | The coverage sidecar mode reads a JSON manifest, keeps one words file per item, and writes tagged JSON lines | Proposed (amended, proposed, by ADR-0168) |
...
| [0168](0168-a-coverage-region-carries-its-bounds-as-optional-before-and-after-points-with-no-version-bump.md) | A coverage region carries its bounds as optional `before` and `after` points, with no version bump | Proposed (amends ADR-0127) |
