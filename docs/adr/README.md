...
| [0027](0027-windows-gates-and-creates-the-release.md) | Windows gates pull requests and creates the release; macOS and Linux are optional, separate builds | Accepted (its asset names are superseded by ADR-0197) |
...
| [0082](0082-windows-installs-per-user-from-an-nsis-setup-program-that-wails-builds-and-the-release-carries-beside-the-update-zip.md) | Windows installs per user from an NSIS setup program that Wails builds and the release carries beside the update zip | Accepted (its asset names are superseded by ADR-0197) |
...
| [0085](0085-the-third-party-notices-are-generated-from-the-release-and-ship-as-their-own-asset-and-not-inside-the-update-zip.md) | The third-party notices are generated from the release and ship as their own asset, not inside the update zip | Accepted (its asset names are superseded by ADR-0197) |
...
| [0100](0100-analysis-evidence-is-two-hash-keys-one-ledger-record-per-run-and-a-narrator-confirmed-track-map.md) | Analysis evidence is two hash keys, one ledger record per run, and a narrator-confirmed track map | Accepted (its "never a link until confirmed" clause is superseded by ADR-0202) |
...
| [0127](0127-the-coverage-sidecar-mode-reads-a-json-manifest-keeps-one-words-file-per-item-and-writes-tagged-json-lines.md) | The coverage sidecar mode reads a JSON manifest, keeps one words file per item, and writes tagged JSON lines | Proposed (amended by ADR-0168) |
...
| [0168](0168-a-coverage-region-carries-its-bounds-as-optional-before-and-after-points-with-no-version-bump.md) | A coverage region carries its bounds as optional `before` and `after` points, with no version bump | Accepted (amends ADR-0127) |
...
| [0190](0190-a-manuscript-cards-whole-header-is-its-disclosure-button-in-fixed-columns.md) | A Manuscript card's whole header is its disclosure button, in fixed columns | Accepted |
| [0191](0191-a-chapters-name-is-title-em-dash-subtitle-in-source-casing-through-one-formatter-and-one-primitive.md) | A chapter's name is "Title — Subtitle" in source casing, through one formatter and one primitive | Accepted |
...
| [0196](0196-the-read-aloud-control-bar-lives-in-a-dialog-footer-slot-and-space-plays-or-stops-a-session.md) | The read-aloud control bar lives in a Dialog footer slot, and Space plays or stops a session | Proposed (amends ADR-0094, ADR-0119) |
| [0197](0197-every-release-asset-name-carries-the-bare-version.md) | Every release asset name carries the bare version | Accepted (supersedes the asset names of ADR-0027, ADR-0082, ADR-0085) |
| [0200](0200-the-desktop-shell-runs-on-wails-v3-beta-pinned-at-v3-0-0-beta-25.md) | The desktop shell runs on Wails v3 beta, pinned at v3.0.0-beta.25 | Accepted (changes how ADR-0082's setup program is built) |
| [0201](0201-app-zoom-under-wails-v3-on-windows-runs-from-100-to-200-percent-and-reads-the-level-back-from-the-window.md) | App zoom under Wails v3 on Windows runs from 100% to 200% and reads the level back from the window | Proposed |
| [0202](0202-a-confident-two-way-match-is-a-real-link-labelled-auto-undoable-and-never-over-a-manual-one.md) | A confident two-way match is a real link, labelled auto, undoable, and never made over a manual one | Accepted (supersedes part of ADR-0100) |
| [0203](0203-unlinking-a-link-rejects-the-pair-and-a-re-import-re-links-by-title-as-auto-links.md) | Unlinking a link rejects the pair, and a re-import re-links the old links by title as auto-links | Proposed |
| [0204](0204-the-recording-check-result-carries-the-hosts-judgement-by-the-stage-signals-rule.md) | The recording check result carries the host's judgement, by the stage signal's rule | Accepted (supersedes part of ADR-0130) |
| [0205](0205-the-prompter-remembers-its-last-word-per-chapter-in-a-host-named-file-dropped-when-the-text-changes.md) | The prompter remembers its last word per chapter, in a host-named file dropped when the text changes | Accepted |
| [0206](0206-resume-reconciles-the-recording-with-the-prompters-last-word-in-the-host-and-asks-only-when-they-disagree.md) | Resume reconciles the recording with the prompter's last word in the host, and asks only when they disagree | Accepted (amends ADR-0112) |
| [0207](0207-a-chapters-kind-can-change-after-import-and-removing-it-from-recording-is-a-reclassification-never-a-delete.md) | A chapter's kind can change after import, and removing it from recording is a reclassification, never a delete | Accepted |
| [0208](0208-credits-values-are-asked-for-once-per-project-keyed-on-project-state-and-never-saved-without-the-narrator.md) | Credits values are asked for once per project, keyed on project state, and never saved without the narrator | Accepted (amends the credits PRD's C3 seeding) |
| [0209](0209-chapter-sync-asks-once-per-project-and-with-consent-every-link-path-runs-one-sync.md) | Chapter sync asks once per project, and with consent every link path runs one sync | Accepted |
| [0210](0210-chapter-sync-watches-the-saved-rpp-and-reapers-edit-counter-and-sends-reaper-nothing.md) | Chapter sync watches the saved .rpp and REAPER's edit counter, and sends REAPER nothing | Accepted |
| [0211](0211-a-changed-chapter-is-rechecked-in-the-background-only-on-mains-power-with-reaper-quiet-and-not-recording.md) | A changed chapter is re-checked in the background only on mains power, with REAPER quiet and not recording | Accepted (supersedes part of ADR-0130) |
| [0230](0230-reaper-commands-built-before-the-verification-pass-are-refused-by-the-host-while-the-experimental-switch-is-off.md) | REAPER commands built before the verification pass are refused by the host while the experimental switch is off | Accepted |
| [0231](0231-chapter-track-state-is-one-read-only-answer-of-the-transport-the-arms-the-input-device-and-one-tracks-items.md) | chapter_track_state is one read-only answer of the transport, the arms, the input device and one track's items | Proposed |
| [0232](0232-arming-for-a-recording-remembers-the-narrators-arms-makes-no-undo-point-and-the-app-stops-only-the-recording-it-started.md) | Arming for a recording remembers the narrator's arms, makes no undo point, and the app stops only the recording it started | Proposed |
...
| [0237](0237-an-mp3-is-measured-for-its-container-only-from-bounded-frame-headers-and-its-levels-are-not-checked.md) | An MP3 is measured for its container only, from bounded frame headers, and its levels are not checked | Proposed |
| [0238](0238-silence-cleanup-candidates-are-classed-in-the-diagnostics-pass-and-raised-apart-from-the-diagnostics-findings.md) | Silence cleanup candidates are classed in the diagnostics pass and raised apart from the diagnostics findings | Proposed |
| [0239](0239-host-and-sidecar-chapter-names-follow-the-display-rule-through-one-helper-per-language-and-matching-keys-keep-their-own-form.md) | Host and sidecar chapter names follow the display rule through one helper per language, and matching keys keep their own form | Proposed |
| [0240](0240-a-markdown-table-of-contents-is-the-first-list-of-in-file-heading-links-matched-as-a-docx-one-is.md) | A Markdown table of contents is the first list of in-file heading links, matched as a docx one is | Proposed |
| [0241](0241-a-subtitle-set-apart-from-its-heading-is-read-as-the-subtitle-and-an-epub-heading-that-starts-no-chapter-is-kept-as-text.md) | A subtitle set apart from its heading is read as the subtitle, and an EPUB heading that starts no chapter is kept as text | Proposed |
| [0242](0242-the-recording-check-writes-the-chapters-word-alignment-as-additive-lines-and-align-again-never-transcribes.md) | The recording check writes the chapter's word alignment as additive lines, and align-again never transcribes | Accepted |
