...
- The vocabulary hints keep their layout and gain a group name, a named typing box, a screen-reader note that a chip is only suggested (`aria-description`), and focus that does not fall to the page. Two texts are darker: the suggested chips and the "No hints yet" line now use `--text`, because on the box's `--surface-2` background `--text-muted` reached 4.31:1 and `--text-faint` 2.5:1 and the atlas's axe check failed the new stories (the debt list is at its cap and may only shrink). Nine of 282 captures differ, all in the Proofing setup box (the empty line and the dashed chips); the palette stack may refine the colours.
- Enter that ends an input-method composition does not add a term.
- A second use (a term list elsewhere) has the component; a use that needs a fixed list of choices does not, and would be a Combobox wrap decided then.
- To change any of this (Backspace removal, a Combobox), write a new ADR that supersedes this one.
