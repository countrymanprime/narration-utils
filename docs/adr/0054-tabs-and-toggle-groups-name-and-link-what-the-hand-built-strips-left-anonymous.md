...
- The tab-like strips had no `tablist`, `tab` or `tabpanel` roles, no `aria-selected`, no link between a label and the content it swaps, and no arrow keys.
- The chip groups had no group name, and pressing the label text activated the info icon inside it.
- Choosing a scope in Settings can ask to discard unsaved edits.

## Considered options

1. A `Tabs` primitive on Base UI Tabs, and a library-free `ToggleGroup` of `Pill`s with a required label
2. Radio semantics for the single-choice chip groups

## Decision outcome

**Chosen option: a `Tabs` primitive on Base UI Tabs, and a library-free `ToggleGroup` of `Pill`s with a required label**, because the hand-built strips and chip groups had none of the tab or group semantics, and Base UI supplies the roles, the links and the arrow keys.
...
- **Neutral:** The strips announce as tabs and the chip groups as named groups, and arrow keys work along a tab strip. Two things differ in the captures, 13 Settings states (39 PNGs) and no others: the selected category's text is `--accent-strong`, not `--accent` (5.5:1 on its tint, where `--accent` reached 4.03:1 and the atlas's axe check failed the new `Sidebar` story; `NavButton` has the same fault recorded as debt and the palette stack fixes tokens), and `settings/reset-override` loses its accidental hover tint (above). Every Story Bible and chip-group capture is identical.
- **Bad:** A tab strip needs its panel under the same `Tabs` root, so a page that wants tabs restructures its root. The `contents` root is used once, and where it is used the panel is a `div`, not a `section`.
- **Neutral:** The Story Bible list section is now `role="tabpanel"` named by the selected tab; a screen reader hears "All, tab panel" (the selected tab's name) when it enters the list.
- **Neutral:** To change any of this (radio semantics, another activation default), write a new ADR that supersedes this one.
...
### Radio semantics for the single-choice chip groups
