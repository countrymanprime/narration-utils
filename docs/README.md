...
| [Going to and looping a finding in REAPER](architecture/reaper-navigation.md) | Bridge and Go client implemented; not yet in the app | Navigating to a finding by item GUID with the stale rule, looping its context and restoring the narrator's time selection and repeat on Stop, the verification record and the owner's checklist. |
