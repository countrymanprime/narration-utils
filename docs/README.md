...
| [Windows release, setup program and in-app update](operations/ci-and-releases.md#the-windows-setup-program) | Implemented (Windows); macOS and Linux are previews | A per-user NSIS setup program and an update zip on each GitHub release, both unsigned, with build provenance; the app checks for a newer release once a day at most and replaces itself only after a confirmed click ([in-app update](architecture/in-app-update.md)). |
