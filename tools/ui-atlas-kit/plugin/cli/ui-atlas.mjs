...
export async function docs(dir) {
  ...
  writeFileSync(join(outDir, 'atlas', 'index.md'), ['# Component atlas', '', 'Generated by `ui-atlas docs` from the Storybook build. Do not edit by hand.', '', ...inventory.map((c) => `- [${c.name}](${c.name}.md): ${c.stories.length} ${c.stories.length === 1 ? 'story' : 'stories'}`), ''].join('\n'));
...
export function sync(dir, flags = {}) {
  ...
  if (!flags.check && !flags['dry-run'] && config.kit !== KIT_VERSION) writeFileSync(join(dir, 'ui-atlas.config.json'), JSON.stringify({ ...config, kit: KIT_VERSION }, null, 2) + '\n');
...
async function main() {
  ...
  if (cmd === 'init') {
    ...
    if (flags.check || flags['dry-run']) {
      ...
      console.log('note: scaffold files (playwright.config.ts, .storybook/*, tests/visual/app.drivers.ts, ...) are yours and are not compared; see CHANGELOG.md for scaffold changes to adopt by hand');
      if (result.drift.length && flags.check) process.exitCode = 1;
