...
// the list may only shrink (src/atlasCoverage.test.ts), and a story may only be listed for the rules
// named here - anything else it violates still fails.
export interface A11yDebt {
  // Story title (e.g. 'Primitives/Dialog'); matches every story of that component.
  title: string;
  rules: string[];
  reason: string;
}
